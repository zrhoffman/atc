...
class CidrAddress @JvmOverloads constructor(
    ...
    val netmaskLength: Int = address.getAddress().size * 8
    ...
    val isIpV6: Boolean
        get() {
            return hostBytes!!.size > 4
        }

    public override fun compareTo(other: CidrAddress): Int {
        var mask: ByteArray? = maskBytes
        var len: Int = netmaskLength
        if (netmaskLength > other.netmaskLength) {
            mask = other.maskBytes
            len = other.netmaskLength
        }
        val numNetmaskBytes: Int = Math.ceil(len.toDouble() / 8).toInt()
    ...
    public override fun equals(o: Any?): Boolean {
        ...
        val that: CidrAddress = o as CidrAddress
    ...
    companion object {
        ...
        fun fromString(cidrString: String?): CidrAddress {
            val hostNetworkArray: Array<String> = cidrString!!.split("/").toTypedArray()
            val host: String = hostNetworkArray.get(0).trim({ it <= ' ' })
            ...
            try {
                netmaskLength = hostNetworkArray.get(1).trim({ it <= ' ' }).toInt()
    ...
    init {
        val addressBytes: ByteArray = address.getAddress()
        if (address is Inet4Address && (netmaskLength > 32 || netmaskLength < 0)) {
            throw NetworkNodeException("Rejecting IPv4 subnet with invalid netmask: " + cidrString)
        } else if (address is Inet6Address && (netmaskLength > 128 || netmaskLength < 0)) {
            throw NetworkNodeException("Rejecting IPv6 subnet with invalid netmask: " + cidrString)
        }
        hostBytes = addressBytes
        maskBytes = ByteArray(addressBytes.size)
        for (i in 0 until netmaskLength) {
            maskBytes.get(i / 8) = maskBytes.get(i / 8) or (1 shl (7 - (i % 8)))
        }
    }
}