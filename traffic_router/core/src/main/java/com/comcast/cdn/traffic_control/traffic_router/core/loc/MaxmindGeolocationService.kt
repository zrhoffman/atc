...
org.apache.log4j.Loggerimport java.io.Fileimport java.lang.Exception
class MaxmindGeolocationService constructor() : GeolocationService {
    ...
    @Throws(GeolocationException::class)
    public override fun location(ip: String?): Geolocation? {
        if (databaseReader == null) {
            return null
        }
        val response: CityResponse? = getCityResponse(databaseReader, ip.split("/").toTypedArray().get(0))
    ...
    public override fun reloadDatabase() {
        if (databaseReader != null) {
            databaseReader.close()
        }
        if (databaseFile != null) {
            val reader: DatabaseReader? = createDatabaseReader(databaseFile)
            if (reader != null) {
    ...
    @Throws(IOException::class)
    public override fun verifyDatabase(databaseFile: File?): Boolean {
        return createDatabaseReader(databaseFile) != null
    }

    public override fun setDatabaseFile(databaseFile: File?) {
    ...
    private fun createDatabaseReader(databaseFile: File?): DatabaseReader? {
        ...
        if (databaseFile.isDirectory()) {
            LOGGER.error(databaseFile.toString() + " is a directory, need a file")
            return null
        }
        LOGGER.info("Loading MaxMind db: " + databaseFile.getAbsolutePath())
        try {
            val reader: DatabaseReader? = DatabaseReader.Builder(databaseFile).build()
            getCityResponse(reader, "127.0.0.1")
            return reader
        } catch (e: Exception) {
            LOGGER.error(databaseFile.getAbsolutePath() + " is not a valid Maxmind data file.  " + e.message)
            return null
        }
    }
    ...
    fun destroy() {
        ...
        try {
            ...
            LOGGER.warn("Caught exception while trying to close geolocation database reader: " + ex.message, ex)
        }
    }

    fun createGeolocation(response: CityResponse?): Geolocation? {
        val latitude: Double = response.getLocation().getLatitude()
        val longitude: Double = response.getLocation().getLongitude()
        ...
        if ((geolocation.getCity() == null) && (geolocation.getPostalCode() == null) && response.getSubdivisions()
                .isEmpty()
        ) {
    ...
    companion object {
        private val LOGGER: Logger? = Logger.getLogger(MaxmindGeolocationService::class.java)
    }
}