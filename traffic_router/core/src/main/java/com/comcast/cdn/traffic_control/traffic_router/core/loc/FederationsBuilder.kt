...
class FederationsBuilder constructor() {
    @Throws(JsonUtilsException::class, IOException::class)
    fun fromJSON(jsonString: String?): List<Federation> {
        val federations: MutableList<Federation> = ArrayList()
        val mapper: ObjectMapper = ObjectMapper()
        val jsonObject: JsonNode = mapper.readTree(jsonString)
        val federationList: JsonNode? = JsonUtils.getJsonNode(jsonObject, "response")
        for (currFederation: JsonNode? in federationList!!) {
            val deliveryService: String? = JsonUtils.getString(currFederation, "deliveryService")
            val mappings: MutableList<FederationMapping?> = ArrayList()
            val mappingsList: JsonNode? = JsonUtils.getJsonNode(currFederation, "mappings")
            val federationMappingBuilder: FederationMappingBuilder = FederationMappingBuilder()
            for (mapping: JsonNode in mappingsList!!) {
                mappings.add(federationMappingBuilder.fromJSON(mapping.toString()))
            }
            val federation: Federation = Federation(deliveryService, mappings)
