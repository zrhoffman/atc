...
class UDP : AbstractProtocol() {
    ...
    override fun run() {
        while (!isShutdownRequested) {
            try {
                val buffer = ByteArray(UDP_MSG_LENGTH)
     ...
     */
    inner class UDPPacketHandler
    /**
     * This method is package private for unit testing purposes.
     *
     * @param packet
     */ internal constructor(private val packet: DatagramPacket) : SocketHandler {
        ...
        override fun run() {
            ...
            try {
                ...
                val request = ByteArray(packet.length)
                System.arraycopy(packet.data, 0, request, 0, request.size)
