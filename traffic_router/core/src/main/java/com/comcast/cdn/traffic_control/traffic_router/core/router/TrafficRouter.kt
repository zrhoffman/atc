...
org.apache.log4j.Loggerimport org.springframework.context.ApplicationContextimport org.xbill.DNS.Nameimport org.xbill.DNS.Typeimport org.xbill.DNS.Zoneimport java.lang.Exceptionimport java.net.URLimport java.net.UnknownHostExceptionimport java.util.*import java.util.function.Predicateimport

java.util.regex.Matcherimport java.util.regex.Pattern
/**
...
class TrafficRouter constructor(
    ...
    private fun setDsStates(dsStates: JsonNode?): Boolean {
        ...
        val dsMap: MutableMap<String?, DeliveryService?>? = cacheRegister.getDeliveryServices()
    ...
    private fun setCacheStates(cacheStates: JsonNode?): Boolean {
        ...
        val cacheMap: MutableMap<String?, Cache?>? = cacheRegister.getCacheMap()
        if (cacheMap == null) {
            return false
        }
    ...
    fun getCachesByGeo(
        ...
        val geoEnabledCacheLocations: MutableList<CacheLocation?>? =
            filterEnabledLocations(getCacheRegister().getCacheLocations(), LocalizationMethod.GEO)
        val cacheLocations1: MutableList<CacheLocation?>? = ds.filterAvailableLocations(geoEnabledCacheLocations)
        val cacheLocations: MutableList<CacheLocation?>? =
            orderLocations(cacheLocations1, clientLocation) as MutableList<CacheLocation?>?
    ...
    fun selectTrafficRoutersMiss(zoneName: String?, ds: DeliveryService?): MutableList<InetRecord?>? {
        ...
        val trafficRouterLocations: MutableList<TrafficRouterLocation?>? =
        ...
        while (edgeTrafficRouters.size < limit && !exhausted) {
            ...
            for (location: TrafficRouterLocation? in trafficRouterLocations) {
                ...
                if (!orderedNodes.containsKey(location.getId())) {
                    orderedNodes.put(
                        location.getId(),
                        consistentHasher.selectHashables(location.getTrafficRouters(), zoneName)
                    )
                }
                val trafficRouters: MutableList<Node?>? = orderedNodes.get(location.getId())
    ...
    fun selectTrafficRoutersLocalized(
        ...
        val trafficRouterLocations: MutableList<TrafficRouterLocation?>? = orderLocations(
            getCacheRegister().getEdgeTrafficRouterLocations(),
            clientGeolocation
        ) as MutableList<TrafficRouterLocation?>?
        for (location: TrafficRouterLocation? in trafficRouterLocations) {
            val trafficRouters: MutableList<Node?>? =
                consistentHasher.selectHashables(location.getTrafficRouters(), zoneName)
    ...
    private fun getEdgeCaches(request: DNSRequest?, ds: DeliveryService?, track: StatTracker.Track?): DNSRouteResult? {
        ...
        try {
            val inetRecords: MutableList<InetRecord?>? =
                federationRegistry.findInetRecords(ds.getId(), CidrAddress.Companion.fromString(request.getClientIP()))
    ...
    private fun nsRecordsFromNodes(ds: DeliveryService?, nodes: MutableList<Node?>?): MutableList<InetRecord?>? {
        ...
        val limit: Int = if ((getEdgeDNSRoutingLimit() > nodes.size)) nodes.size else getEdgeDNSRoutingLimit()
        var ttl: Long = getEdgeNSttl()
    ...
    fun multiRoute(request: HTTPRequest?, track: StatTracker.Track?): HTTPRouteResult? {
        ...
        for (steeringResult: SteeringResult? in steeringResults) {
            ...
            var caches: MutableList<Cache?>? = selectCaches(request, ds, track)
    ...
    fun singleRoute(request: HTTPRequest?, track: StatTracker.Track?): HTTPRouteResult? {
        ...
        val caches: MutableList<Cache?>? = selectCaches(request, deliveryService, track)
        if (caches == null || caches.isEmpty()) {
    ...
    fun getCoverageZoneCacheLocation(
        ...
        if (cacheLocation != null && cacheLocation.getBackupCacheGroups() != null) {
            for (cacheGroup: String? in cacheLocation.getBackupCacheGroups()) {
                val bkCacheLocation: CacheLocation? = getCacheRegister().getCacheLocationById(cacheGroup)
        ...
        var availableLocations: MutableList<CacheLocation?>? =
            cacheRegister.filterAvailableCacheLocations(deliveryServiceId)
    ...
    fun consistentHashForCoverageZone(
        ...
        val caches: MutableList<Cache?>? = selectCachesByCZ(deliveryService, coverageZoneCacheLocation, requestVersion)
        if (caches == null || caches.isEmpty()) {
            return null
        }
        val pathToHash: String? = buildPatternBasedHashString(deliveryService, request)
    ...
    fun consistentHashMultiDeliveryService(
        ...
        // Pattern based consistent hashing
        val pathToHash: String? = buildPatternBasedHashString(deliveryService, request)
        val steeringTargets: MutableList<SteeringTarget?>? =
    ...
    fun consistentHashDeliveryService(
        ...
        val availableTargets: MutableList<SteeringTarget?>? = steering.getTargets().stream()
    ...
    private fun getClosestCacheLocation(
        ...
        val orderedLocations: MutableList<CacheLocation?>? =
            orderLocations(cacheLocations, clientLocation) as MutableList<CacheLocation?>?
    ...
    private fun selectCaches(
        ...
        val caches: MutableList<Cache?>? = getSupportingCaches(location.getCaches(), ds, requestVersion)
        if (caches.isEmpty()) {
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug(
 ...
 * @return A zone containing records of type qtype that contains qname. This can be null
    ...
    fun configurationChanged() {
        ...
        val configurationListenerMap: MutableMap<String?, ConfigurationListener?>? = applicationContext.getBeansOfType(
    ...
    init {
        ...
        if (cacheRegister.getConfig() != null) {
            ...
            val geolocations: JsonNode? = cacheRegister.getConfig().get("maxmindDefaultOverride")
