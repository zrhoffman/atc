...
class SignedZoneKey constructor(name: Name, records: MutableList<Record?>) : ZoneKey(name, records) {
    var signatureExpiration: Calendar? = null
    var kSKExpiration: Calendar? = null
    var zSKExpiration: Calendar? = null
    val signatureDuration: Long
        get() {
            return signatureExpiration!!.getTimeInMillis() - getTimestamp()
        }

    // force a refresh when we're halfway through our validity period
    val refreshHorizon: Long
        get() {
            return getTimestamp() + Math.round(signatureDuration.toDouble() / 2.0) // force a refresh when we're halfway through our validity period
        }
    val earliestSigningKeyExpiration: Long
        get() {
            if (kSKExpiration!!.before(zSKExpiration)) {
                return kSKExpiration!!.getTimeInMillis()
            } else {
                return zSKExpiration!!.getTimeInMillis()
            }
        }
}