...
org.apache.log4j.Loggerimport org.xbill.DNS.Nameimport org.xbill.DNS.Recordimport java.util.function.Predicate
class SignedZoneKey constructor(name: Name?, records: MutableList<Record?>?) : ZoneKey(name, records) {
    ...
    fun setMinimumSignatureExpiration(signedRecords: MutableList<Record?>?, defaultExpiration: Calendar?) {
        val minSignatureExpiration: OptionalLong? = signedRecords.stream()
            .filter(Predicate({ r: Record? -> r is RRSIGRecord }))
            .mapToLong(ToLongFunction({ r: Record? -> (r as RRSIGRecord?).getExpire().getTime() }))
            .min()
    ...
    fun getSignatureDuration(): Long {
        return minimumSignatureExpiration.getTimeInMillis() - getTimestamp()
    }

    fun getRefreshHorizon(): Long {
        return getTimestamp() + Math.round(getSignatureDuration() as Double / 2.0) // force a refresh when we're halfway through our validity period
    }
    ...
    companion object {
        private val LOGGER: Logger? = Logger.getLogger(SignedZoneKey::class.java)
    }
}