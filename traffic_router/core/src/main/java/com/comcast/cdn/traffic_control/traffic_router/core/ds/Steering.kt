...
class Steering {
    ...
    @JsonProperty
    var isClientSteering = false

    @JsonProperty
    var targets: List<SteeringTarget?>? = ArrayList()

    @JsonProperty
    var filters: List<SteeringFilter>? = ArrayList()
    fun getBypassDestination(requestPath: String?): String? {
        for (filter in filters!!) {
            if (filter.matches(requestPath) && hasTarget(filter.deliveryService)) {
                return filter.deliveryService
            }
        }
        return null
    }
