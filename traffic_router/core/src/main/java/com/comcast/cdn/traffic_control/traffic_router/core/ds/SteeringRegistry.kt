...
class SteeringRegistry {
    private var registry: Map<String?, Steering> = HashMap()
    private val objectMapper = ObjectMapper(JsonFactory())
    fun update(json: String?) {
        val m: Map<String, List<Steering>>
        try {
            ...
            if (old == null || old != newSteering) {
                for (target in newSteering.targets) {
                    if (target.geolocation != null && target.geoOrder != 0) {
                        ...
                        LOGGER.info("Steering " + newSteering.deliveryService + " target " + target.deliveryService + " now has geolocation [" + target.latitude + ", " + target.longitude + "]")
                    } else if (target.weight > 0) {
                        LOGGER.info("Steering " + newSteering.deliveryService + " target " + target.deliveryService + " now has weight " + target.weight)
                    } else if (target.order != 0) { // this target has a specific order set
                        LOGGER.info("Steering " + newSteering.deliveryService + " target " + target.deliveryService + " now has order " + target.order)
                    } else {
                        LOGGER.info("Steering " + newSteering.deliveryService + " target " + target.deliveryService + " now has weight " + target.weight + " and order " + target.order)
                    }
                }
            }
        }
    ...
    operator fun get(steeringId: String?): Steering? {
        return registry[steeringId]
    }

    val all: Collection<Steering>
        get() = registry.values
