...
class StringProtector constructor(passwd: String) {
    ...
    fun decrypt(property: String?): String {
        val bytes: ByteArray = decrypt(base64.decode(property))
    ...
    companion object {
        private val SALT: ByteArray = byteArrayOf(
            0xde.toByte(), 0x33.toByte(), 0x10.toByte(), 0x12.toByte(),
            0xde.toByte(), 0x33.toByte(), 0x10.toByte(), 0x12.toByte()
        )
    }
    ...
    init {
        ...
        val key: SecretKey = keyFactory.generateSecret(PBEKeySpec(passwd.toCharArray()))
        encryptor = Cipher.getInstance("PBEWithMD5AndDES")
        encryptor.init(Cipher.ENCRYPT_MODE, key, PBEParameterSpec(SALT, 20))
        decryptor = Cipher.getInstance("PBEWithMD5AndDES")
        decryptor.init(Cipher.DECRYPT_MODE, key, PBEParameterSpec(SALT, 20))
    }
}