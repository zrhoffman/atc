...
open class Node : DefaultHashable {
    ...
    fun getIpAddresses(ttls: JsonNode?): List<InetRecord>? {
        return getIpAddresses(ttls, true)
    }

    fun getIpAddresses(ttls: JsonNode?, ip6RoutingEnabled: Boolean): List<InetRecord>? {
        if (ipAddresses == null) {
            return null
        }
    ...
    fun addCapabilities(capabilities: Set<String>?) {
    ...
    open fun setDeliveryServices(deliveryServices: Collection<DeliveryServiceReference>) {
    ...
    fun setIpAddress(ip: String?, ip6: String?, ttl: Long) {
        ...
        if (ip != null && !ip.isEmpty()) {
            ...
            LOGGER.error(fqdn + " - no IPv4 address configured!")
        }
        if (ip6 != null && !ip6.isEmpty()) {
            ...
            LOGGER.error(fqdn + " - no IPv6 address configured!")
        }
    }
