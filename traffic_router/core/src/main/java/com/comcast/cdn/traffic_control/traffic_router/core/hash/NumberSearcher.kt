...
object NumberSearcher {
    fun findClosest(numbers: Array<Double>, target: Double): Int {
        val index: Int = Arrays.binarySearch(numbers, target)
        if (index >= 0) {
            return index
        }
        val biggerThanIndex: Int = -(index + 1)
        ...
        val smallerThanIndex: Int = biggerThanIndex - 1
        val biggerThanDelta: Double = Math.abs(numbers.get(biggerThanIndex) - target)
        val smallerThanDelta: Double = Math.abs(numbers.get(smallerThanIndex) - target)
