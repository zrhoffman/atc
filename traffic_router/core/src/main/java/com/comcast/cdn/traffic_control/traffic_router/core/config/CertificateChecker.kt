...
class CertificateChecker constructor() {
    fun getDeliveryServiceType(deliveryServiceJson: JsonNode): String? {
        val matchsets: JsonNode = deliveryServiceJson.get("matchsets")
    ...
    fun certificatesAreValid(certificateDataList: List<CertificateData>, deliveryServicesJson: JsonNode?): Boolean {
        val deliveryServiceIdIter: Iterator<String> = deliveryServicesJson!!.fieldNames()
        var invalidConfig: Boolean = false
    ...
    fun hasCertificate(certificateDataList: List<CertificateData>, deliveryServiceId: String?): Boolean {
        return certificateDataList.stream()
            .filter(Predicate({ cd: CertificateData -> (cd.getDeliveryservice() == deliveryServiceId) }))
            .findFirst()
            .isPresent()
    }

    private fun deliveryServiceHasValidCertificates(
        certificateDataList: List<CertificateData>,
        deliveryServicesJson: JsonNode?,
        deliveryServiceId: String?
    ): Boolean {
        val deliveryServiceJson: JsonNode = deliveryServicesJson!!.get(deliveryServiceId)
        val protocolJson: JsonNode = deliveryServiceJson.get("protocol")
        if (!supportsHttps(deliveryServiceJson, protocolJson)) {
            return true
        }
        val domains: JsonNode? = deliveryServiceJson.get("domains")
        if (domains == null) {
        ...
        for (domain: JsonNode in domains) {
            val domainStr: String? = domain.asText("").replace("^\\*\\.".toRegex(), "")
            if (domainStr == null || domainStr.isEmpty()) {
                continue
            }
            for (certificateData: CertificateData in certificateDataList) {
                val certificateDeliveryServiceId: String? = certificateData.getDeliveryservice()
    ...
    private fun supportsHttps(deliveryServiceJson: JsonNode, protocolJson: JsonNode): Boolean {
        if (!("HTTP" == getDeliveryServiceType(deliveryServiceJson))) {
            return false
        }
        return optBoolean(protocolJson, "acceptHttps")
    }

    companion object {
        private val LOGGER: Logger = Logger.getLogger(CertificateChecker::class.java)
    }
}