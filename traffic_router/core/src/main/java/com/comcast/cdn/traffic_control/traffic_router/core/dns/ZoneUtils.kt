...
object ZoneUtils {
    ...
    private val sdf = SimpleDateFormat("yyyyMMddHH")
    fun getMaximumTTL(records: List<Record?>?): Long {
        var maximumTTL: Long = 0
        for (record in records!!) {
            if (record!!.ttl > maximumTTL) {
                maximumTTL = record.ttl
            }
        }
        return maximumTTL
    }
    ...
    fun getSerial(jo: JsonNode?): Long {
        synchronized(sdf) {
            ...
            return sdf.format(date).toLong() // 2013062701
        }
    }
