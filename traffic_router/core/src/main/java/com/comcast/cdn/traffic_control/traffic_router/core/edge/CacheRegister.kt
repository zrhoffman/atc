...
class CacheRegister constructor() {
    ...
    private var edgeTrafficRouterCount: Int = 0
    ...
    fun getCacheLocationById(id: String?): CacheLocation? {
        for (location: CacheLocation? in configuredLocations.values) {
    ...
    fun getAllEdgeTrafficRouters(): MutableList<Node?>? {
        ...
        for (location: TrafficRouterLocation? in getEdgeTrafficRouterLocations()) {
    ...
    fun setConfiguredLocations(locations: MutableSet<CacheLocation?>?) {
        ...
        for (newLoc: CacheLocation? in locations) {
    ...
    fun setEdgeTrafficRouterLocations(locations: MutableCollection<TrafficRouterLocation?>?) {
        ...
        for (newLoc: TrafficRouterLocation? in locations) {
            edgeTrafficRouterLocations.put(newLoc.getId(), newLoc)
            val trafficRouters: MutableList<Node?>? = newLoc.getTrafficRouters()
    ...
    fun getDeliveryServiceMatchers(deliveryService: DeliveryService?): MutableSet<DeliveryServiceMatcher?>? {
        return deliveryServiceMatchers.stream()
            .filter(Predicate({ deliveryServiceMatcher: DeliveryServiceMatcher? ->
                (deliveryServiceMatcher.getDeliveryService().getId() == deliveryService.getId())
            }))
    ...
    fun getDeliveryService(request: Request?): DeliveryService? {
        ...
        for (m: DeliveryServiceMatcher? in deliveryServiceMatchers) {
    ...
    fun filterAvailableCacheLocations(deliveryServiceId: String?): MutableList<CacheLocation?>? {
        val deliveryService: DeliveryService? = dsMap.get(deliveryServiceId)
