...
class CacheRegister constructor() {
    ...
    var trafficRouters: JsonNode? = null
    var cacheMap: Map<String?, Cache?>? = null
    private var deliveryServiceMatchers: TreeSet<DeliveryServiceMatcher>? = null
    var deliveryServices: Map<String?, DeliveryService>? = null
        private set
    var config: JsonNode? = null
    var stats: JsonNode? = null
    var edgeTrafficRouterCount: Int = 0
        private set
    ...
    val cacheLocations: Set<CacheLocation?>
        get() {
    ...
    fun getEdgeTrafficRouterLocations(): List<TrafficRouterLocation> {
        return ArrayList(edgeTrafficRouterLocations.values)
    }

    val allEdgeTrafficRouters: List<Node?>
        get() {
     ...
     */
    fun setConfiguredLocations(locations: Set<CacheLocation>) {
    ...
    fun setEdgeTrafficRouterLocations(locations: Collection<TrafficRouterLocation>) {
        var count: Int = 0
        ...
        for (newLoc: TrafficRouterLocation in locations) {
            ...
            val trafficRouters: List<Node?>? = newLoc.getTrafficRouters()
    ...
    fun getDeliveryServiceMatchers(deliveryService: DeliveryService): Set<DeliveryServiceMatcher?> {
    ...
    fun getDeliveryService(deliveryServiceId: String?): DeliveryService? {
        return deliveryServices!!.get(deliveryServiceId)
    }

    fun filterAvailableCacheLocations(deliveryServiceId: String?): List<CacheLocation?>? {
        val deliveryService: DeliveryService? = deliveryServices!!.get(deliveryServiceId)
        if (deliveryService == null) {
            return null
        }
        return deliveryService.filterAvailableLocations(cacheLocations)
    }

    fun setDeliveryServiceMap(dsMap: Map<String?, DeliveryService>?) {
