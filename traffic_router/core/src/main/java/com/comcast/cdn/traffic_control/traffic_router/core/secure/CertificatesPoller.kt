...
class CertificatesPoller constructor() : ConfigurationListener {
    private val executor: ScheduledExecutorService
    private var future: ScheduledFuture<*>? = null
    private var certificatesClient: CertificatesClient? = null
    var pollingInterval: Long = defaultFixedRate
        private set
    var certificatesQueue: BlockingQueue<List<CertificateData?>>? = null
    private var lastFetchedData: List<CertificateData?> = ArrayList()
    var trafficRouterManager: TrafficRouterManager? = null

    @Autowired
    private val environment: Environment? = null
    val environmentPollingInterval: Long?
        get() {
            if (environment == null) {
                LOGGER.warn("Could not find Environment object!")
            }
            try {
                val value: Long? = environment!!.getProperty(intervalProperty, Long::class.java)
                if (value == null) {
    ...
    fun start() {
        val runnable: Runnable = label@ Runnable({
            try {
                trafficRouterManager!!.trackEvent("lastHttpsCertificatesCheck")
                val certificateDataList: List<CertificateData?>? = certificatesClient!!.refreshData()
                ...
                LOGGER.warn(
                    "Failed to refresh certificate data: " + t.javaClass.getCanonicalName() + " " + t.message,
                    t
                )
            }
        })
        val customFixedRate: Long? = environmentPollingInterval
    ...
    public override fun configurationChanged() {
        restart()
    }

    companion object {
        private val LOGGER: Logger = Logger.getLogger(CertificatesPoller::class.java)
        private val defaultFixedRate: Long = 3600 * 1000L
