...
class RegionalGeoDsvc constructor(val id: String?) {
    private val urlRules: MutableList<RegionalGeoRule> = ArrayList()
    fun addRule(urlRule: RegionalGeoRule) {
        urlRules.add(urlRule)
    }

    fun matchRule(url: String?): RegionalGeoRule? {
        for (rule: RegionalGeoRule in urlRules) {
