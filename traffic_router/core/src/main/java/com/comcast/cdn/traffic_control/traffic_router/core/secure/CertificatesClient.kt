...
class CertificatesClient constructor() {
    ...
    fun fetchRawData(stringBuilder: StringBuilder?): Int {
        ...
        val certificatesUrl: String? = trafficOpsUtils!!.getUrl(
            "certificate.api.url",
            "https://\${toHostname}/api/2.0/cdns/name/\${cdnName}/sslkeys"
        )
        try {
            val fetcher: ProtectedFetcher =
            ...
            LOGGER.warn(
                "Failed to fetch data for certificates from " + certificatesUrl + "(" + e.javaClass.getSimpleName() + ") : " + e.message,
                e
            )
        }
        return -1
    ...
    fun getCertificateData(jsonData: String?): List<CertificateData?>? {
        try {
            LOGGER.debug("Certificates successfully updated @ " + lastValidfetchTimestamp)
            return (ObjectMapper().readValue(
                jsonData,
                object : TypeReference<CertificatesResponse?>() {}) as CertificatesResponse).getResponse()
        } catch (e: Exception) {
            LOGGER.warn("Failed parsing json data: " + e.message)
        }
        return ArrayList()
    }

    fun doubleDecode(encoded: String): Array<String> {
        val decodedBytes: ByteArray = Base64.getMimeDecoder().decode(encoded.toByteArray())
    ...
    companion object {
        private val LOGGER: Logger = Logger.getLogger(CertificatesClient::class.java)
