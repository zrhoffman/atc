...
org.apache.log4j.Loggerimport java.net.URLimport java.util.*import javax.servlet.FilterChainimport

javax.servlet.ServletExceptionimport javax.servlet.http.HttpServletRequestimport javax.servlet.http.HttpServletResponse
class RouterFilter constructor() : OncePerRequestFilter() {
    @Autowired
    ...
    private var doNotLog: Boolean = false
    ...
    public override fun doFilterInternal(
        ...
        if (staticContentWhiteList.contains(request.getRequestURI())) {
            ...
            val access: HTTPAccessRecord? = HTTPAccessRecord.Builder(requestDate, request).build()
            ACCESS.info(HTTPAccessEventBuilder.create(access))
            return
        }
        val httpAccessRecord: HTTPAccessRecord? = HTTPAccessRecord.Builder(requestDate, request).build()
    ...
    private fun writeHttpResponse(
        ...
        val httpAccessRecordBuilder: HTTPAccessRecord.Builder? = HTTPAccessRecord.Builder(httpAccessRecord)
        ...
        try {
            ...
            val requestHeaders: MutableSet<String?>? = trafficRouterManager.getTrafficRouter().getRequestHeaders()
            if (routeResult != null && routeResult.getRequestHeaders() != null) {
                requestHeaders.addAll(routeResult.getRequestHeaders())
            }
            val accessRequestHeaders: MutableMap<String?, String?>? =
                HttpAccessRequestHeaders().makeMap(httpServletRequest, requestHeaders)
            val access: HTTPAccessRecord? = httpAccessRecordBuilder.resultType(track.getResult())
    ...
    private fun setMultiResponse(
        ...
        if (routeResult.getDeliveryService() != null) {
            val responseHeaders: MutableMap<String?, String?>? = routeResult.getDeliveryService().getResponseHeaders()
            for (key: String? in responseHeaders.keys) {
        ...
        val redirect: String? = httpServletRequest.getParameter(REDIRECT_QUERY_PARAM)
        if (!(HEAD == httpServletRequest.getMethod())) {
    ...
    private fun setSingleResponse(
        ...
        val redirect: String? = httpServletRequest.getParameter(REDIRECT_QUERY_PARAM)
        val format: String? = httpServletRequest.getParameter("format")
        val location: URL? = routeResult.getUrl()
        if (routeResult.getDeliveryService() != null) {
            val deliveryService: DeliveryService? = routeResult.getDeliveryService()
            val responseHeaders: MutableMap<String?, String?>? = deliveryService.getResponseHeaders()
            for (key: String? in responseHeaders.keys) {
                response.addHeader(key, responseHeaders.get(key))
            }
        }
    ...
    fun setDoNotLog(logAccessString: String?) {
        doNotLog = java.lang.Boolean.valueOf(logAccessString)
    }
    ...
    companion object {
        private val ACCESS: Logger? = Logger.getLogger("com.comcast.cdn.traffic_control.traffic_router.core.access")
