...
class RegionalGeoUpdater constructor() : AbstractServiceUpdater() {
    @Throws(IOException::class)
    public override fun loadDatabase(): Boolean {
        val existingDB: File = databasesDirectory!!.resolve(databaseName).toFile()
        return RegionalGeo.Companion.parseConfigFile(existingDB, false)
    }

    @Throws(IOException::class)
    public override fun verifyDatabase(dbFile: File): Boolean {
        return RegionalGeo.Companion.parseConfigFile(dbFile, true)
    }

    companion object {
        private val LOGGER: Logger = Logger.getLogger(RegionalGeoUpdater::class.java)
    }
