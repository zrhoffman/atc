...
com.fasterxml.jackson.databind.JsonNodeimport com.fasterxml.jackson.databind.ObjectMapper
...
class FederationMappingBuilder constructor() {
    ...
    fun fromJSON(json: String?): FederationMapping? {
        ...
        val cname: String? = JsonUtils.getString(jsonNode, "cname")
        val ttl: Int = JsonUtils.getInt(jsonNode, "ttl")
        val network: ComparableTreeSet<CidrAddress?>? = ComparableTreeSet()
        if (jsonNode.has("resolve4")) {
            val networkList: JsonNode? = JsonUtils.getJsonNode(jsonNode, "resolve4")
            network.addAll(buildAddresses(networkList))
        }
        val network6: ComparableTreeSet<CidrAddress?>? = ComparableTreeSet()
        if (jsonNode.has("resolve6")) {
            val network6List: JsonNode? = JsonUtils.getJsonNode(jsonNode, "resolve6")
    ...
    private fun buildAddresses(networkArray: JsonNode?): ComparableTreeSet<CidrAddress?>? {
        val network: ComparableTreeSet<CidrAddress?>? = ComparableTreeSet()
        for (currNetwork: JsonNode? in networkArray) {
            val addressString: String? = currNetwork.asText()
            try {
    ...
    companion object {
        private val LOGGER: Logger? = Logger.getLogger(FederationMapping::class.java)
    }
}