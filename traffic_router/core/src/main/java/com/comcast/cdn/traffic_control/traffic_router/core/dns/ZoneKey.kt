...
open class ZoneKey(name: Name, records: MutableList<Record?>) : Comparable<ZoneKey> {
    ...
    override fun hashCode(): Int {
        return name.hashCode() + initialHashCode
    }

    override fun equals(obj: Any?): Boolean {
        val ozk = obj as ZoneKey?
        return name == ozk!!.name && initialHashCode == ozk.initialHashCode && obj!!.javaClass == this.javaClass
    }

    // this correctly sorts the names such that the superDomains are last
    override fun compareTo(zk: ZoneKey): Int {
        val i = name!!.compareTo(zk.name)
        return if (i < 0) {
            1
        } else if (i > 0) {
            -1
    ...
    init {
        ...
        setName(name)
        setRecords(records)
        initialHashCode = records.hashCode() // if the records are signed, the hashCode will change
        timestamp = System.currentTimeMillis()
    }
}