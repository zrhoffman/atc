...
class DeliveryServiceMatcher(var deliveryService: DeliveryService?) : Comparable<DeliveryServiceMatcher> {
    enum class Type {
        HOST, HEADER, PATH
    }

    private val requestMatchers: MutableList<RequestMatcher?>? = ArrayList()
    fun addMatch(type: Type, string: String?, target: String?) {
        requestMatchers!!.add(RequestMatcher(type, string, target))
    }

    fun getRequestMatchers(): List<RequestMatcher?> {
    ...
    override fun toString(): String {
        return if (requestMatchers!!.size > 1) {
            "DeliveryServiceMatcher{" +
                ...
                "deliveryService=" + deliveryService +
                ", requestMatcher=" + requestMatchers[0] +
                '}'
    }
}