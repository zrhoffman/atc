...
class CacheLocation @JvmOverloads constructor(
    ...
    backupCacheGroups: List<String>? = null,
     ...
     */
    val backupCacheGroups: List<String>? = null
     ...
     */
    var isUseClosestGeoLoc: Boolean = true
    ...
    fun loadDeepCaches(deepCacheNames: Set<String?>?, cacheRegister: CacheRegister?) {
        synchronized(caches, {
            if (caches.isEmpty() && deepCacheNames != null) {
                for (deepCacheName: String? in deepCacheNames) {
                    val deepCache: Cache? = cacheRegister.getCacheMap().get(deepCacheName)
                    if (deepCache != null) {
    ...
    companion object {
        val LOGGER: Logger = Logger.getLogger(CacheLocation::class.java)
    }
    /**
    ...
    init {
        ...
        isUseClosestGeoLoc = useClosestGeoOnBackupFailure
