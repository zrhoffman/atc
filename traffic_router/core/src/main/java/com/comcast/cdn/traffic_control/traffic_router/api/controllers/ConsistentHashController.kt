...
class ConsistentHashController {
    ...
    fun testPatternBasedRegex(
        ...
        val pathToHash = trafficRouterManager.getTrafficRouter().buildPatternBasedHashString(regex, requestPath)
            ?: return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(null)
        val map: MutableMap<String, String?> = HashMap()
        map[REQUEST_PATH] = requestPath
        map[CONSISTENT_HASH_REGEX] = regex
        map[RESULTING_PATH_TO_HASH] = pathToHash
    ...
    fun testPatternBasedDeliveryService(
        ...
        val map: MutableMap<String, String?> = HashMap()
        map[REQUEST_PATH] = requestPath
        map[DELIVERY_SERVICE_ID] = deliveryServiceId
        map[RESULTING_PATH_TO_HASH] = pathToHash
        return ResponseEntity.ok(map)
    }
    ...
    companion object {
        ...
        const val RESULTING_PATH_TO_HASH = "resultingPathToConsistentHash"
        const val REQUEST_PATH = "requestPath"
        const val CONSISTENT_HASH_REGEX = "consistentHashRegex"
        const val DELIVERY_SERVICE_ID = "deliveryServiceId"
    }
}