...
open class DefaultHashable constructor() : Hashable<DefaultHashable?>, Comparable<DefaultHashable?> {
    private var hashes: Array<Double?>?
    private var order: Int = 0
    public override fun setOrder(order: Int) {
        this.order = order
    }
    ...
    public override fun hasHashes(): Boolean {
        return if (hashes.size > 0) true else false
    }

    public override fun getClosestHash(hash: Double): Double {
        return hashes.get(NumberSearcher.findClosest(hashes, hash))
    }

    public override fun generateHashes(hashId: String?, hashCount: Int): DefaultHashable? {
        val hashSet: TreeSet<Double?>? = TreeSet()
        val hashFunction: MD5HashFunction? = MD5HashFunction()
    ...
    public override fun getHashValues(): MutableList<Double?>? {
        return Arrays.asList(*hashes)
    }

    public override fun compareTo(o: DefaultHashable?): Int {
        if (getOrder() < 0 && o.getOrder() < 0) {
            return if (getOrder() < o.getOrder()) 1 else if (getOrder() > o.getOrder()) -1 else 0
        } else {
            return if (getOrder() < o.getOrder()) -1 else if (getOrder() > o.getOrder()) 1 else 0
        }
    }
