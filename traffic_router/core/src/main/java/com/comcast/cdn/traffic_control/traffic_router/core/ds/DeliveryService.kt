...
class DeliveryService(private val id: String?, @field:JsonIgnore private val props: JsonNode?) {
    ...
    fun createURIString(request: HTTPRequest?, cache: Cache?): String? {
        ...
        if (fqdn == null) {
            val cacheName: Array<String?> = cache.getFqdn().split(REGEX_PERIOD.toRegex(), 2).toTypedArray()
            fqdn =
                cacheName[0].toString() + "." + request.getHostname().split(REGEX_PERIOD.toRegex(), 2).toTypedArray()[1]
        }
    ...
    fun extractSignificantQueryParams(r: HTTPRequest?): String? {
        ...
        try {
            for (qparam in r.getQueryString().split("&".toRegex()).toTypedArray()) {
                if (qparam.isEmpty()) {
                    continue
                }
                val parts: Array<String?> = qparam.split("=".toRegex()).toTypedArray()
