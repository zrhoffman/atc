...
class ConsistentHasher constructor() {
    ...
    fun <T : Hashable<*>?> selectHashable(hashables: MutableList<T?>?, dispersion: Dispersion?, s: String?): T? {
        val selectedHashables: MutableList<T?>? = selectHashables(hashables, dispersion, s)
    ...
    fun <T : Hashable<*>?> selectHashables(
        ...
        val sortedHashables: SortedMap<Double?, T?>? = sortHashables(hashables, s)
        val selectedHashables: MutableList<T?>? = ArrayList()
        for (hashable: T? in sortedHashables.values) {
    ...
    private fun <T : Hashable<*>?> sortHashables(hashables: MutableList<T?>?, s: String?): SortedMap<Double?, T?>? {
        val hash: Double = hashFunction.hash(s)
        val hashableMap: SortedMap<Double?, T?>? = TreeMap()
        val zeroHashes: MutableList<T?>? = ArrayList()
        for (hashable: T? in hashables) {
            if (!hashable.hasHashes()) {
                zeroHashes.add(hashable)
                continue
            }
            val closestHash: Double = hashable.getClosestHash(hash)
            val hashDelta: Double = getSafePositiveHash(hashableMap, Math.abs(hash - closestHash))
    ...
    private fun <T : Hashable<*>?> synthesizeZeroHashes(
        ...
        var minHash: Double = 0.0
        var maxHash: Double = 0.0
        try {
        ...
        for (hashable: T? in zeroHashes) {
            if (hashable.getOrder() >= 0) { // append
                val syntheticHash: Double = getSafePositiveHash(hashableMap, maxHash)
                hashableMap.put(syntheticHash, hashable)
                maxHash = syntheticHash
            } else { // negative order specified, prepend
                val syntheticHash: Double = getSafeNegativeHash(hashableMap, minHash)
                hashableMap.put(syntheticHash, hashable)
    ...
    private fun <T : Hashable<*>?> getSafeHash(
        ...
        var syntheticHash: Double = hash
        var bits: Long = java.lang.Double.doubleToLongBits(syntheticHash)
        do {
