...
class ConsistentHasher constructor() {
    ...
    fun <T : Hashable<*>?> selectHashable(hashables: List<T>?, dispersion: Dispersion?, s: String?): T? {
        val selectedHashables: List<T> = selectHashables(hashables, dispersion, s)
        return if (!selectedHashables.isEmpty()) selectedHashables.get(0) else null
    }

    fun <T : Hashable<*>?> selectHashables(hashables: List<T>?, s: String?): List<T> {
        return selectHashables(hashables, null, s)
    }

    fun <T : Hashable<*>?> selectHashables(hashables: List<T>?, dispersion: Dispersion?, s: String?): MutableList<T?> {
    ...
    private fun <T : Hashable<*>?> sortHashables(hashables: List<T>?, s: String?): SortedMap<Double, T> {
