...
class FederationExporter constructor() {
    private var federationRegistry: FederationRegistry? = null
    fun getMatchingFederations(cidrAddress: CidrAddress): List<Any> {
        val federationsList: MutableList<Any> = ArrayList()
        for (federation: Federation? in federationRegistry!!.findFederations(cidrAddress)) {
            val filteredFederationMappings: MutableList<Map<String, Any?>> = ArrayList()
            for (federationMapping: FederationMapping? in federation.getFederationMappings()) {
    ...
    private fun getMappingProperties(
        ...
        val filteredMapping: FederationMapping? = federationMapping!!.createFilteredMapping(cidrAddress)
    ...
    private fun addAddressProperties(
        ...
        for (cidrAddress: CidrAddress? in cidrAddresses) {
