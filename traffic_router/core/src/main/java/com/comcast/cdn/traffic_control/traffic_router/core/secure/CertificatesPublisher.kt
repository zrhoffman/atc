 ...
 * under the License.
 */   class CertificatesPublisher constructor(
    certificatesQueue: BlockingQueue<List<CertificateData>?>, publishStatusQueue: BlockingQueue<Boolean?>,
    certificateChecker: CertificateChecker, trafficRouterManager: TrafficRouterManager
) {
    var deliveryServicesJson: JsonNode? = null
    var deliveryServices: List<DeliveryService> = ArrayList()
    private var running: Boolean = true
    val worker: Thread
    private fun publishCertificateList(certificateDataList: List<CertificateData>) {
        try {
            val objectName: ObjectName = ObjectName(DeliveryServiceCertificatesMBean.OBJECT_NAME)
            ...
            LOGGER.error(
                "Failed to add certificate data list as management MBean! " + e.javaClass.getSimpleName() + ": " + e.message,
                e
            )
        }
    }
    ...
    companion object {
        private val LOGGER: Logger = Logger.getLogger(CertificatesPublisher::class.java)
    }
    ...
    init {
        worker = Thread(label@ Runnable({
            while (running) {
                try {
                    val certificateDataList: List<CertificateData>? = certificatesQueue.take()
                    ...
                    if (certificateChecker.certificatesAreValid(certificateDataList, deliveryServicesJson)) {
                        deliveryServices.forEach(Consumer({ ds: DeliveryService ->
                            val hasX509Cert: Boolean =
                                certificateChecker.hasCertificate(certificateDataList, ds.getId())
