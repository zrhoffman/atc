...
class TrafficOpsUtils constructor() {
    ...
    fun replaceTokens(input: String?): String {
        return input!!.replace("\${tmHostname}", (hostname)!!).replace("\${toHostname}", (hostname)!!)
            .replace("\${cdnName}", (cdnName)!!)
    }
    ...
    fun getUrl(parameter: String?, defaultValue: String?): String {
        return if (config != null) replaceTokens(
            JsonUtils.optString(
                config,
                parameter,
                defaultValue
    ...
    val authJSON: JsonNode
        get() {
            val authMap: MutableMap<String, String?> = HashMap()
            authMap.put("u", username)
            authMap.put("p", password)
            val mapper: ObjectMapper = ObjectMapper()
            val data: JsonNode = mapper.valueToTree(authMap)
            return data
        }
    val authUrl: String
        get() {
