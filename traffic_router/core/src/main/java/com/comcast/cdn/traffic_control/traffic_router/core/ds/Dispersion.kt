...
class Dispersion(dsJo: JsonNode?) {
    var limit = DEFAULT_LIMIT
        private set
    var isShuffled = DEFAULT_SHUFFLED
        private set
    ...
    fun getCacheList(cacheMap: SortedMap<Double?, Cache?>?): List<Cache?>? {
        if (cacheMap == null) {
            return null
        }
    ...
    init {
        val jo = dsJo!!["dispersion"]
        if (jo != null) {
            val limit = JsonUtils.optInt(jo, "limit", DEFAULT_LIMIT)
            if (limit != 0) {
                this.limit = limit
            }
            isShuffled = JsonUtils.optBoolean(jo, "shuffled", DEFAULT_SHUFFLED)
        } else if (dsJo.has("maxDnsIpsForLocation")) {
            // if no specific dispersion, use maxDnsIpsForLocation (should be DNS DSs only)
            limit = dsJo["maxDnsIpsForLocation"].asInt(DEFAULT_LIMIT)
        }
    }
}