...
class Dispersion(dsJo: JsonNode?) {
    ...
    private var shuffled = DEFAULT_SHUFFLED
    fun getLimit(): Int {
        return limit
    }
    ...
    init {
        ...
        if (jo != null) {
            ...
            setShuffled(JsonUtils.optBoolean(jo, "shuffled", DEFAULT_SHUFFLED))
        } else if (dsJo.has("maxDnsIpsForLocation")) {
            // if no specific dispersion, use maxDnsIpsForLocation (should be DNS DSs only)
            setLimit(dsJo.get("maxDnsIpsForLocation").asInt(DEFAULT_LIMIT))
        }
    }
}