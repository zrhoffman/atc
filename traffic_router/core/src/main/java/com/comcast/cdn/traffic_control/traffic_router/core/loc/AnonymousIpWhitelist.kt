...
class AnonymousIpWhitelist constructor() {
    ...
    fun add(network: String?) {
        val node: NetworkNode? = NetworkNode(network, AnonymousIp.Companion.WHITE_LIST_LOC)
    ...
    operator fun contains(address: String?): Boolean {
        ...
        try {
            val nn: NetworkNode? = whitelist.getNetwork(address)
            if (nn.getLoc() === AnonymousIp.Companion.WHITE_LIST_LOC) {
                return true
            }
    ...
    companion object {
        private val LOGGER: Logger? = Logger.getLogger(AnonymousIpWhitelist::class.java)
    }
