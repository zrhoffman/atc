...
object DNSAccessEventBuilder {
    ...
    private fun createDnsMessageHeader(dnsMessage: Message?): String {
        val queryHeader: String = " xn=" + dnsMessage!!.getHeader().getID()
        val query: String = " " + createQuery(dnsMessage.getQuestion())
        val rcode: String = " rcode=" + Rcode.string(dnsMessage.getHeader().getRcode())
    ...
    private fun createTTLandAnswer(dnsMessage: Message?): String {
        ...
        val answerStringBuilder: StringBuilder = StringBuilder()
        val ttlStringBuilder: StringBuilder = StringBuilder()
        for (record: Record in dnsMessage.getSectionArray(Section.ANSWER)) {
            val s: String = record.rdataToString() + " "
            val ttl: String = record.getTTL().toString() + " "
    ...
    fun create(dnsAccessRecord: DNSAccessRecord?, wireParseException: WireParseException): String {
        val event: String = createEvent(dnsAccessRecord)
        val rerr: String =
            "Bad Request:" + wireParseException.javaClass.getSimpleName() + ":" + wireParseException.message
    ...
    private fun createQuery(query: Record?): String {
        if (query != null && query.getName() != null) {
            val qname: String = query.getName().toString()
            val qtype: String = Type.string(query.getType())
            val qclass: String = DClass.string(query.getDClass())
