...
object HTTPAccessEventBuilder {
    private fun formatRequest(request: HttpServletRequest?): String {
        var url: String = formatObject(request!!.getRequestURL())
        if (("-" == url)) {
            return url
        }
    ...
    private fun formatRequestHeaders(requestHeaders: Map<String?, String?>?): String {
        if (requestHeaders == null || requestHeaders.isEmpty()) {
            return "rh=\"-\""
        }
        val stringBuilder: StringBuilder = StringBuilder()
        var first: Boolean = true
        for (entry: Map.Entry<String?, String?> in requestHeaders.entries) {
            ...
            stringBuilder.append(entry.value!!.replace("\"".toRegex(), "'"))
            stringBuilder.append('"')
        }
    ...
    fun create(httpAccessRecord: HTTPAccessRecord?): String {
        val start: Long = httpAccessRecord.getRequestDate().getTime()
        val timeString: String = String.format("%d.%03d", start / 1000, start % 1000)
        val httpServletRequest: HttpServletRequest? = httpAccessRecord.getRequest()
        var chi: String = formatObject(httpServletRequest!!.getRemoteAddr())
        val url: String = formatRequest(httpServletRequest)
        val cqhm: String = formatObject(httpServletRequest.getMethod())
        val cqhv: String = formatObject(httpServletRequest.getProtocol())
        val resultType: String = formatObject(httpAccessRecord.getResultType())
        val rerr: String = formatObject(httpAccessRecord.getRerr())
        var resultDetails: String? = "-"
        if (!("-" == resultType)) {
            resultDetails = formatObject(httpAccessRecord.getResultDetails())
        }
        var rloc: String? = "-"
        val resultLocation: Geolocation? = httpAccessRecord.getResultLocation()
        ...
        val xMmClientIpHeader: String? = httpServletRequest.getHeader(HTTPRequest.Companion.X_MM_CLIENT_IP)
        val fakeIpParameter: String? = httpServletRequest.getParameter(HTTPRequest.Companion.FAKE_IP)
        val remoteIp: String = chi
        if (xMmClientIpHeader != null) {
            chi = xMmClientIpHeader
        } else if (fakeIpParameter != null) {
            chi = fakeIpParameter
        }
        val rgb: String = formatObject(httpAccessRecord.getRegionalGeoResult())
        val stringBuilder: StringBuilder = StringBuilder(timeString)
        ...
        if (httpAccessRecord.getResponseCode() != -1) {
            val pssc: String = formatObject(httpAccessRecord.getResponseCode())
            val ttms: Double = (System.nanoTime() - httpAccessRecord.getRequestNanoTime()) / 1000000.0
            stringBuilder.append(" pssc=").append(pssc).append(" ttms=").append(String.format("%.03f", ttms))
        }
        val respurl: String = " rurl=\"" + formatObject(httpAccessRecord.getResponseURL()) + "\""
        stringBuilder.append(respurl)
        val respurls: String = " rurls=\"" + formatObject(httpAccessRecord.getResponseURLs()) + "\""
        stringBuilder.append(respurls)
        val userAgent: String = httpServletRequest.getHeader("User-Agent") + "\" "
