...
class InetRecord {
    ...
    public override fun equals(o: Any?): Boolean {
        ...
        val that: InetRecord? = o as InetRecord?
        if (ttl != that.ttl || type != that.type) return false
        if (if (ad != null) !(ad == that.ad) else that.ad != null) return false
        return !(if (target != null) !(target == that.target) else that.target != null)
    }

    public override fun hashCode(): Int {
        var result: Int = if (ad != null) ad.hashCode() else 0
        result = 31 * result + (ttl xor (ttl ushr 32)) as Int
        result = 31 * result + (type xor (type ushr 32)) as Int
        result = 31 * result + (if (target != null) target.hashCode() else 0)
        return result
    }
}