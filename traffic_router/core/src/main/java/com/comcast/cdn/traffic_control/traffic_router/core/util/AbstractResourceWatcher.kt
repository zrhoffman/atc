...
org.apache.log4j.Loggerimport java.io.Fileimport java.lang.Exceptionimport java.net.URL
abstract class AbstractResourceWatcher constructor() : AbstractServiceUpdater() {
    ...
    fun configure(config: JsonNode?) {
        ...
        if (authUrl == null || credentials == null) {
            LOGGER.warn("[ " + javaClass.getSimpleName() + " ] Invalid Traffic Ops authorization URL or credentials data, not updating configuration!")
            return
        }
        val watcherConfig: WatcherConfig? = WatcherConfig(getWatcherConfigPrefix(), config, trafficOpsUtils)
        val resourceUrl: String? = if ((watcherConfig.getUrl() != null && !watcherConfig.getUrl()
                .isEmpty())
        ) watcherConfig.getUrl() else defaultDatabaseURL
        val pollingInterval: Long =
            if ((watcherConfig.getInterval() > 0)) watcherConfig.getInterval() else getPollingInterval()
        val configTimeout: Int = if ((watcherConfig.getTimeout() > 0)) watcherConfig.getTimeout() else timeout
        if (((authUrl == authorizationUrl) && (credentials == postData) && (resourceUrl == dataBaseURL) && (
                    pollingInterval == getPollingInterval()) && (
                    configTimeout == timeout))
        ) {
            LOGGER.info("[ " + javaClass.getName() + " ] Nothing changed in configuration")
            return
        }
    ...
    public override fun loadDatabase(): Boolean {
        val existingDB: File? = databasesDirectory.resolve(databaseName).toFile()
        if (!existingDB.exists() || !existingDB.canRead()) {
            return false
        }
        val jsonData: CharArray? = CharArray(existingDB.length() as Int)
        val reader: FileReader? = FileReader(existingDB)
        try {
    ...
    @Throws(IOException::class)
    public override fun verifyDatabase(dbFile: File?): Boolean {
        if (!dbFile.exists() || !dbFile.canRead()) {
            return false
        }
        val jsonData: CharArray? = CharArray(dbFile.length() as Int)
        val reader: FileReader? = FileReader(dbFile)
        try {
            reader.read(jsonData)
        } finally {
            reader.close()
    ...
    override fun downloadDatabase(url: String?, existingDb: File?): File? {
        ...
        val interpolatedUrl: String? = trafficOpsUtils.replaceTokens(url)
        if (fetcher == null) {
            LOGGER.warn("[" + javaClass.getSimpleName() + "] Waiting for configuration to be processed, unable to download from '" + interpolatedUrl + "'")
            return null
        }
        ...
        try {
            ...
            LOGGER.warn("[ " + javaClass.getSimpleName() + " ] Failed to fetch data from '" + interpolatedUrl + "': " + e.message)
        }
        if (jsonData == null) {
            return existingDb
        }
    ...
    companion object {
        private val LOGGER: Logger? = Logger.getLogger(AbstractResourceWatcher::class.java)
    }
}