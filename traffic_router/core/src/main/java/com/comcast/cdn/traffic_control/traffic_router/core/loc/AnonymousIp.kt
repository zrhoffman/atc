...
class AnonymousIp private constructor() {
    ...
    private var blockAnonymousIp: Boolean = true
    private var blockHostingProvider: Boolean = true
    private var blockPublicProxy: Boolean = true
    private var blockTorExitNode: Boolean = true
    private var ipv4Whitelist: AnonymousIpWhitelist? = null
    private var ipv6Whitelist: AnonymousIpWhitelist? = null
    ...
    companion object {
        private val LOGGER: Logger? = Logger.getLogger(AnonymousIp::class.java)
        private var currentConfig: AnonymousIp? = AnonymousIp()
        val BLOCK_CODE: Int = 403
	 ...
	 */
        ...
        private fun parseConfigJson(config: JsonNode?): AnonymousIp? {
            val anonymousIp: AnonymousIp? = AnonymousIp()
            try {
                val blockingTypes: JsonNode? = JsonUtils.getJsonNode(config, "anonymousIp")
                ...
                anonymousIp.enabled = currentConfig.enabled
        ...
        fun parseConfigFile(f: File?, verifyOnly: Boolean): Boolean {
            ...
            val anonymousIp: AnonymousIp? = parseConfigJson(json)
            if (anonymousIp == null) {
                return false
            }
            if (!verifyOnly) {
        ...
        fun enforce(trafficRouter: TrafficRouter?, dsvcId: String?, url: String?, ip: String?): Boolean {
            val address: InetAddress? = InetAddresses.forString(ip)
            if (inWhitelist(ip)) {
                return false
            }
            val response: AnonymousIpResponse? = trafficRouter.anonymousIpDatabaseService.lookupIp(address)
            if (response == null) {
                return false
            }
            ...
            // specific policy
            if (getCurrentConfig().blockAnonymousIp && response.isAnonymousVpn()) {
                return true
            }
            if (getCurrentConfig().blockHostingProvider && response.isHostingProvider()) {
                return true
            }
            if (getCurrentConfig().blockPublicProxy && response.isPublicProxy()) {
                return true
            }
            if (getCurrentConfig().blockTorExitNode && response.isTorExitNode()) {
                return true
            }
            return false
        }
	 ...
	 */   fun enforce(
            ...
            if (!trafficRouter.anonymousIpDatabaseService.isInitialized()) {
                return
            }

            // Check if the ip is allowed
            val block: Boolean = enforce(
                trafficRouter,
                deliveryService.getId(),
                httpRequest.getRequestedUrl(),
                httpRequest.getClientIP()
            )

            // Block the ip if it is not allowed
            if (block) {
                routeResult.setResponseCode(BLOCK_CODE)
                track.setResult(ResultType.ANON_BLOCK)
                if (getCurrentConfig().redirectUrl != null) {
                    routeResult.addUrl(URL(getCurrentConfig().redirectUrl))
                }
            }
        }
    }
