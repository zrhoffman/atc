...
class HTTPRequest : Request {
    ...
    fun applyRequest(request: HttpServletRequest?) {
        ...
        val xmm: String? = request.getHeader(X_MM_CLIENT_IP)
        val fip: String? = request.getParameter(FAKE_IP)
        if (xmm != null) {
            setClientIP(xmm)
        } else if (fip != null) {
            setClientIP(fip)
        ...
        while (headerNames.hasMoreElements()) {
            val name: String? = headerNames.nextElement() as String?
    ...
    public override fun equals(obj: Any?): Boolean {
        if (this === obj) {
            ...
            val rhs: HTTPRequest? = obj as HTTPRequest?
