...
class FederationRegistry constructor() {
    private var federations: List<Federation?>? = ArrayList()
    fun setFederations(federations: List<Federation?>?) {
    ...
    fun findInetRecords(deliveryServiceId: String?, cidrAddress: CidrAddress): MutableList<InetRecord>? {
        ...
        synchronized((federations)!!, {
            for (federation: Federation? in federations!!) {
        ...
        for (federationMapping: FederationMapping? in targetFederation.getFederationMappings()) {
            val cidrAddresses: ComparableTreeSet<CidrAddress?>? = federationMapping!!.getResolveAddresses(cidrAddress)
            if (cidrAddresses == null) {
                continue
            }
            for (resolverAddress: CidrAddress? in cidrAddresses) {
    ...
    protected fun createInetRecords(federationMapping: FederationMapping?): MutableList<InetRecord> {
        val inetRecord: InetRecord = InetRecord(federationMapping.getCname(), federationMapping.getTtl())
        val inetRecords: MutableList<InetRecord> = ArrayList()
        inetRecords.add(inetRecord)
        return inetRecords
    }

    fun findFederations(cidrAddress: CidrAddress?): List<Federation?> {
