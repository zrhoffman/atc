...
class DnsSecKeyPairImpl constructor(keyPair: JsonNode?, defaultTTL: Long) : DnsSecKeyPair {
    ...
    public override fun getTTL(): Long {
        return ttl
    }

    public override fun setTTL(ttl: Long) {
        this.ttl = ttl
    }
    ...
    public override fun getInception(): Date? {
        return inception
    }

    public override fun setInception(inception: Date?) {
        this.inception = inception
    }

    public override fun getEffective(): Date? {
        return effective
    }

    public override fun setEffective(effective: Date?) {
        this.effective = effective
    }

    public override fun getExpiration(): Date? {
        return expiration
    }

    public override fun setExpiration(expiration: Date?) {
        this.expiration = expiration
    }

    public override fun isKeySigningKey(): Boolean {
        return ((getDNSKEYRecord().getFlags() and DNSKEYRecord.Flags.SEP_KEY) != 0)
    }

    public override fun isExpired(): Boolean {
        return getExpiration().before(Calendar.getInstance().getTime())
    }

    public override fun isUsable(): Boolean {
        val now: Date? = Calendar.getInstance().getTime()
        return getEffective().before(now)
    }

    public override fun isKeyCached(maxTTL: Long): Boolean {
    ...
    public override fun getDNSKEYRecord(): DNSKEYRecord? {
        return dnskeyRecord
    }

    public override fun getPrivate(): PrivateKey? {
        return privateKey
    }

    public override fun getPublic(): PublicKey? {
        try {
            return dnskeyRecord.getPublicKey()
        } catch (e: DNSSECException) {
            LOGGER.error("Failed to extract public key from DNSKEY record for " + name + " : " + e.message, e)
        }
        return null
    }
    ...
    public override fun equals(obj: Any?): Boolean {
        ...
        if (!(getDNSKEYRecord() == okp.getDNSKEYRecord())) {
            return false
        } else if (!(getPrivate() == okp.getPrivate())) {
            return false
        } else if (!(getPublic() == okp.getPublic())) {
            return false
    ...
    companion object {
        private val LOGGER: Logger? = Logger.getLogger(DnsSecKeyPairImpl::class.java)
    }
    ...
    init {
        ...
        try {
            ...
            LOGGER.error("Failed to decode PKCS1 key from json data!: " + e.message, e)
        }
