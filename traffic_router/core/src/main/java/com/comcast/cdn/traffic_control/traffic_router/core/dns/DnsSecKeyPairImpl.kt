...
class DnsSecKeyPairImpl constructor(keyPair: JsonNode?, defaultTTL: Long) : DnsSecKeyPair {
    ...
    override val isKeySigningKey: Boolean
        get() {
            return ((dNSKEYRecord!!.getFlags() and DNSKEYRecord.Flags.SEP_KEY) != 0)
        }
    ...
    override val isUsable: Boolean
        get() {
            ...
            return effective.before(now)
        }

    public override fun isKeyCached(maxTTL: Long): Boolean {
        return expiration.after(Date(System.currentTimeMillis() - (maxTTL * 1000)))
    }
    ...
    override fun equals(obj: Any?): Boolean {
        ...
        if (!(dNSKEYRecord == okp!!.dNSKEYRecord)) {
            return false
        } else if (!(private == okp.private)) {
            return false
        } else if (!(public == okp.public)) {
            return false
        } else if (!(effective == okp.effective)) {
            return false
        } else if (!(expiration == okp.expiration)) {
            return false
        } else if (!(inception == okp.inception)) {
            return false
