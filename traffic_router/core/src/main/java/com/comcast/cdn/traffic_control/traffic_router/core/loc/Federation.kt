...
class Federation constructor(val deliveryService: String?, val federationMappings: List<FederationMapping?>?) :
    Comparable<Federation> {
    public override fun equals(o: Any?): Boolean {
        if (this === o) return true
        if (o == null || javaClass != o.javaClass) return false
        val that: Federation = o as Federation
        if (if (deliveryService != null) !(deliveryService == that.deliveryService) else that.deliveryService != null) return false
        return !(if (federationMappings != null) !(federationMappings == that.federationMappings) else that.federationMappings != null)
    }

    public override fun hashCode(): Int {
        var result: Int = if (deliveryService != null) deliveryService.hashCode() else 0
        result = 31 * result + (if (federationMappings != null) federationMappings.hashCode() else 0)
        return result
    }

    public override fun compareTo(other: Federation): Int {
        return deliveryService!!.compareTo((other.deliveryService)!!)
    }

    fun containsCidrAddress(cidrAddress: CidrAddress?): Boolean {
        for (federationMapping: FederationMapping? in federationMappings!!) {
