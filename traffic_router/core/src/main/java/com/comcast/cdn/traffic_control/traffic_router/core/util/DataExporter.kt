...
class DataExporter constructor() {
    ...
    fun getCachesByIp(ip: String?, geolocationProvider: String?): MutableMap<String?, Any?>? {
        ...
        try {
            ...
            val federationsList: MutableList<Any?>? = federationExporter.getMatchingFederations(cidrAddress)
    ...
    private fun getLocationFromCzm(ip: String?): Location? {
        ...
        if (locId != null) {
            ...
            val caches: MutableCollection<CacheLocation?>? = trafficRouter.cacheRegister.getCacheLocations()
    ...
    fun getLocations(): MutableList<String?>? {
        ...
        val trafficRouter: TrafficRouter? = trafficRouterManager.getTrafficRouter()
        for (location: CacheLocation? in trafficRouter.cacheRegister.getCacheLocations()) {
    ...
    fun getCaches(locationId: String?): MutableList<CacheModel?>? {
        val trafficRouter: TrafficRouter? = trafficRouterManager.getTrafficRouter()
        val location: CacheLocation? = trafficRouter.cacheRegister.getCacheLocation(locationId)
    ...
    private fun getCaches(location: CacheLocation?): MutableList<CacheModel?>? {
        ...
        for (cache: Cache? in location.getCaches()) {
            ...
            val ips: MutableList<InetRecord?>? = cache.getIpAddresses(null)
            if (ips != null) {
    ...
    fun getCacheControlMaxAge(): Int {
        ...
        if (trafficRouterManager != null) {
            ...
            if (trafficRouter != null) {
                val cacheRegister: CacheRegister? = trafficRouter.cacheRegister
    ...
    fun getStaticZoneCacheStats(): MutableMap<String?, Any?>? {
        return createCacheStatsMap(trafficRouterManager.getTrafficRouter().zoneManager.getStaticCacheStats())
    }

    fun getDynamicZoneCacheStats(): MutableMap<String?, Any?>? {
        return createCacheStatsMap(trafficRouterManager.getTrafficRouter().zoneManager.getDynamicCacheStats())
    }
