...
class LanguidState constructor() {
    var isReady: Boolean = false
    var trafficRouterManager: TrafficRouterManager? = null
    var port: Int = 0
    var apiPort: Int = 0
    var securePort: Int = 0
    var secureApiPort: Int = 0
    fun init() {
        if (trafficRouterManager == null || trafficRouterManager.getTrafficRouter() == null) {
            return
        }
        val tr: TrafficRouter? = trafficRouterManager.getTrafficRouter()
        if (tr.getCacheRegister() == null) {
            return
        }
        val hostname: String
        try {
            hostname = InetAddress.getLocalHost().getHostName().replace("\\..*".toRegex(), "")
        } catch (e: UnknownHostException) {
            LOGGER.error("Cannot lookup hostname of this traffic router!: " + e.message)
            return
        }
        val routers: JsonNode? = tr.getCacheRegister().getTrafficRouters()
        val keyIter: Iterator<String> = routers!!.fieldNames()
        while (keyIter.hasNext()) {
            val key: String = keyIter.next()
            val routerJson: JsonNode = routers.get(key)
    ...
    private fun initPorts(routerJson: JsonNode) {
        if (routerJson.has("port")) {
            port = routerJson.get("port").asInt()
        }
        if (routerJson.has("api.port")) {
            apiPort = routerJson.get("api.port").asInt()
            trafficRouterManager.setApiPort(apiPort)
        }
        if (routerJson.has("secure.port")) {
            securePort = routerJson.get("secure.port").asInt()
        }
        if (routerJson.has("secure.api.port")) {
            secureApiPort = routerJson.get("secure.api.port").asInt()
            trafficRouterManager.setSecureApiPort(secureApiPort)
        }
    }

    companion object {
        private val LOGGER: Logger = Logger.getLogger(LanguidState::class.java)
    }
}