...
class RegionalGeoRule constructor(
    private val regionalGeoDsvc: RegionalGeoDsvc,
    val urlRegex: String?, val pattern: Pattern, val postalsType: PostalsType,
    private val postals: Set<String?>, private val whiteListRoot: NetworkNode?,
    // if disallowed, client will be redirected to this url
    val alternateUrl: String?, private val coordinateRanges: List<RegionalGeoCoordinateRange>?
) {
    ...
    fun isAllowedCoordinates(lat: Double, lon: Double): Boolean {
        ...
        for (i in coordinateRanges.indices) {
            val coordinateRange: RegionalGeoCoordinateRange = coordinateRanges.get(i)
    ...
    fun isIpInWhiteList(ip: String?): Boolean {
        ...
        try {
            val nn: NetworkNode? = whiteListRoot.getNetwork(ip)
    ...
    companion object {
        private val LOGGER: Logger = Logger.getLogger(RegionalGeoRule::class.java)
