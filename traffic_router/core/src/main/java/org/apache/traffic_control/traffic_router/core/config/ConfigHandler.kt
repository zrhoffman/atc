...
org.apache.traffic_control.traffic_router.core.request.HTTPRequestimport org.apache.traffic_control.traffic_router.core.router.StatTrackerimport org.apache.traffic_control.traffic_router.core.router.TrafficRouterManagerimport org.apache.traffic_control.traffic_router.core.secure.CertificatesPollerimport org.apache.traffic_control.traffic_router.core.secure.CertificatesPublisherimport org.apache.traffic_control.traffic_router.geolocation.Geolocationimport java.io.IOExceptionimport java.util.*import java.util.concurrent.BlockingQueueimport
...
java.util.function.Consumerimport java.util.stream.Stream org.springframework.web.bind.annotation .RequestMapping
...
class ConfigHandler {
    ...
    fun processConfig(jsonStr: String?): Boolean {
        ...
        synchronized(ConfigHandler.Companion.configSync) {
            ...
            try {
                ...
                val deliveryServiceMap = parseDeliveryServiceConfig(JsonUtils.getJsonNode(jo, ConfigHandler.Companion.deliveryServicesKey))
                ...
                val httpsDeliveryServices = deliveryServices.stream().filter { ds: DeliveryService? -> !ds.isDns() && ds.isSslEnabled() }.collect(Collectors.toList())
                ...
                if (!httpsDeliveryServices.isEmpty()) {
                    try {
                        ...
                        ConfigHandler.Companion.LOGGER.warn("Failed to notify certificates publisher we're waiting for certificates", e)
                    }
                    while (!cancelled.get() && !publishStatusQueue.isEmpty()) {
                        try {
                            ConfigHandler.Companion.LOGGER.info("Waiting for https certificates to support new config " + String.format("%x", publishStatusQueue.hashCode()))
                            Thread.sleep(1000L)
                        } catch (t: Throwable) {
                            ConfigHandler.Companion.LOGGER.warn("Interrupted while waiting for status on publishing ssl certs", t)
                        }
                    }
                }
                if (cancelled.get()) {
                ...
                if (jo.has(ConfigHandler.Companion.topologiesKey)) {
                    parseTopologyConfig(JsonUtils.getJsonNode(jo, ConfigHandler.Companion.topologiesKey), deliveryServiceMap, cacheRegister)
                }
                ...
                trafficRouterManager.getNameServer().ecsEnabledDses = deliveryServices.stream().filter { obj: DeliveryService? -> obj.isEcsEnabled() }.collect(Collectors.toSet())
                ...
                cancelled.set(false)
                publishStatusQueue.clear()
                ConfigHandler.Companion.LOGGER.error("Exiting processConfig: Failed to process config for snapshot from $date", e)
                return false
            }
        }
    ...
    private fun parseTopologyConfig(allTopologies: JsonNode?, deliveryServiceMap: MutableMap<String?, DeliveryService?>?, cacheRegister: CacheRegister?) {
        ...
        deliveryServiceMap.forEach(BiConsumer { xmlId: String?, ds: DeliveryService? ->
            ...
            Stream.of(ds.getTopology())
                    ...
                    .filter { node: String? -> cacheRegister.getCacheLocation(node) != null }
                    ...
                    .forEach { cache: Cache? ->
                        cacheRegister.getDeliveryServiceMatchers(ds).stream()
                                .flatMap { deliveryServiceMatcher: DeliveryServiceMatcher? -> deliveryServiceMatcher.getRequestMatchers().stream() }
                                ...
                                .forEach { pattern: String? ->
                                    ...
                                    try {
                                        ...
                                        ConfigHandler.Companion.LOGGER.error("Unable to create a DeliveryServiceReference from DeliveryService '" + ds.getId() + "'", e)
                                    }
                                }
    ...
    @Throws(JsonUtilsException::class)
    private fun parseDeliveryServiceMatchSets(allDeliveryServices: JsonNode?, deliveryServiceMap: MutableMap<String?, DeliveryService?>?, cacheRegister: CacheRegister?) {
    ...
    private fun parseGeolocationConfig(config: JsonNode?) {
        ...
        if (config.has(ConfigHandler.Companion.NEUSTAR_POLLING_URL)) {
            System.setProperty(ConfigHandler.Companion.NEUSTAR_POLLING_URL, JsonUtils.getString(config, ConfigHandler.Companion.NEUSTAR_POLLING_URL))
        }
        if (config.has(ConfigHandler.Companion.NEUSTAR_POLLING_INTERVAL)) {
            System.setProperty(ConfigHandler.Companion.NEUSTAR_POLLING_INTERVAL, JsonUtils.getString(config, ConfigHandler.Companion.NEUSTAR_POLLING_INTERVAL))
        }
    }
    ...
    private fun parseLocalizationMethods(loc: String?, jo: JsonNode?): MutableSet<LocalizationMethod?>? {
        ...
        if (jo != null && jo.hasNonNull(ConfigHandler.Companion.LOCALIZATION_METHODS) && JsonUtils.getJsonNode(jo, ConfigHandler.Companion.LOCALIZATION_METHODS).isArray) {
    ...
    private fun getEdgeTrafficRouterLocationMap(jo: JsonNode?): MutableMap<String?, Location?>? {
        ...
        while (locs.hasNext()) {
            ...
            try {
                ...
                locations[loc] = Location(loc, Geolocation(JsonUtils.getDouble(locJo, "latitude"), JsonUtils.getDouble(locJo, "longitude")))
            } catch (e: JsonUtilsException) {
                ConfigHandler.Companion.LOGGER.warn(e, e)
            }
        }
            ...
            try {
                ...
                if (cl != null) {
                    ...
                    if (status == null || "ONLINE" != status.asText() && "REPORTED" != status.asText()) {
                        ConfigHandler.Companion.LOGGER.warn(String.format("Skipping Edge Traffic Router %s due to %s status", trafficRouterName, status))
                        continue
                    } else {
                        ConfigHandler.Companion.LOGGER.info(String.format("Edge Traffic Router %s %s @ %s; %s", status, trafficRouterName, trLoc, cl.geolocation.toString()))
                    }
