...
com.fasterxml.jackson.databind.ObjectMapperimport org.apache.traffic_control.traffic_router.core.util.*import java.io.IOException
...
class TrafficOpsUtils {
    ...
    fun replaceTokens(input: String?): String? {
        return input.replace("\${tmHostname}", getHostname()).replace("\${toHostname}", getHostname()).replace("\${cdnName}", getCdnName())
    }
    ...
    fun getAuthJSON(): JsonNode? {
        val authMap: MutableMap<String?, String?> = HashMap()
    ...
    fun getAuthUrl(): String? {
        return getUrl("api.auth.url", "https://\${toHostname}/api/" + TrafficOpsUtils.Companion.TO_API_VERSION + "/user/login")
    }
