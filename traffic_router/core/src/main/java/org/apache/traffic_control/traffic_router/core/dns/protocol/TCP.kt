...
java.io.DataOutputStreamimport java.io.IOExceptionimport java.nio.channels.Channels org.apache.logging.log4j.LogManagerimport org.apache.traffic_control.traffic_router.core.dns.protocol.*import org.apache.traffic_control.traffic_router.core.dns.protocol.TCP
...
class TCP : AbstractProtocol() {
    ...
    override fun run() {
        while (!isShutdownRequested) {
            val handler: TCPSocketHandler
            try {
                handler = TCPSocketHandler(getServerSocket().accept())
                submit(handler)
            } catch (e: IOException) {
     ...
     */(private val socket: Socket?) : SocketHandler {
        ...
        override fun run() {
            ...
            try {
                Channels.newInputStream(Channels.newChannel(socket.getInputStream())).use { iis ->
                    DataInputStream(iis).use { `is` ->
    ...
    companion object {
        private val LOGGER = LogManager.getLogger(TCP::class.java)
    }
}