...
class RegionalGeo private constructor() {
    ...
    private fun addRule(dsvcId: String?, urlRegex: String?,
                        postalsType: PostalsType?, postals: MutableSet<String?>?,
                        networkRoot: NetworkNode?, alternateUrl: String?, isSteeringDS: Boolean, coordinateRanges: MutableList<RegionalGeoCoordinateRange?>?): Boolean {
        ...
        if ((alternateUrl.lowercase(Locale.getDefault()).startsWith(HTTP_SCHEME) || alternateUrl.lowercase(Locale.getDefault()).startsWith(HTTPS_SCHEME))
        ...
        if (isSteeringDS && !(alternateUrl.lowercase(Locale.getDefault()).startsWith(HTTP_SCHEME) || alternateUrl.lowercase(Locale.getDefault()).startsWith(HTTPS_SCHEME))) {
        ...
        val urlRule = RegionalGeoRule(regionalGeoDsvc,
    ...
    companion object {
        private val LOGGER = LogManager.getLogger(RegionalGeo::class.java)
        ...
        private fun checkCoordinateRangeValidity(cr: RegionalGeoCoordinateRange?): Boolean {
            if (cr.getMinLat() < -90.0 || cr.getMinLat() > 90.0 || cr.getMaxLat() < -90.0 || cr.getMaxLat() > 90.0 || cr.getMinLon() < -180.0 || cr.getMinLon() > 180.0 || cr.getMaxLon() < -180.0 || cr.getMaxLon() > 180.0) {
        ...
        private fun parseLocationJson(locationJson: JsonNode?,
                                      postals: MutableSet<String?>?): PostalsType? {
        ...
        fun enforce(dsvcId: String?, url: String?,
                    ip: String?, postalCode: String?, lat: Double, lon: Double): RegionalGeoResult? {
        ...
        fun enforce(trafficRouter: TrafficRouter?, request: Request?,
            ...
            val result = enforce(deliveryService.getId(), httpRequest.requestedUrl,
        ...
        private fun createRedirectURIString(request: HTTPRequest?, deliveryService: DeliveryService?,
                                            cache: Cache?, regionalGeoResult: RegionalGeoResult?): String? {
