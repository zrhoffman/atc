...
import org.apache.traffic_control.traffic_router.core.edge.Node.IPVersionsimport
...
class CoverageZoneController {
    ...
    @ResponseBody
    fun getCacheLocationForIp(@RequestParam(name = "ip") ip: String?,
                              @RequestParam(name = "deliveryServiceId") deliveryServiceId: String?): ResponseEntity<CacheLocation?>? {
        val requestVersion = if (ip.contains(":")) IPVersions.IPV6ONLY else IPVersions.IPV4ONLY
        val cacheLocation = trafficRouterManager.getTrafficRouter().getCoverageZoneCacheLocation(ip, deliveryServiceId, requestVersion)
    ...
    @ResponseBody
    fun getCachesForDeliveryService(@RequestParam(name = "deliveryServiceId") deliveryServiceId: String?,
                                    @RequestParam(name = "cacheLocationId") cacheLocationId: String?): ResponseEntity<MutableList<Cache?>?>? {
        val caches = trafficRouterManager.getTrafficRouter().selectCachesByCZ(deliveryServiceId, cacheLocationId, null, IPVersions.ANY)
