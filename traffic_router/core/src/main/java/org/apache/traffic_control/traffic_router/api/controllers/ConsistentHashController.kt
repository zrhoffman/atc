...
import org.apache.traffic_control.traffic_router.api.controllers.ConsistentHashControllerimport

import org.apache.traffic_control.traffic_router.core.router.TrafficRouterManagerimport org.springframework.beans.factory.annotation .Autowiredimport org.springframework.http.HttpStatusimport org.springframework.http.ResponseEntityimport org.springframework.stereotype.Controllerimport org.springframework.web.bind.annotation .RequestParamimport org.springframework.web.bind.annotation .ResponseBody org.springframework.web.bind.annotation .RequestMapping
...
class ConsistentHashController {
    ...
    @ResponseBody
    fun hashCoverageZoneCache(@RequestParam(name = "ip") ip: String?,
                              @RequestParam(name = ConsistentHashController.Companion.DELIVERY_SERVICE_ID) deliveryServiceId: String?,
                              @RequestParam(name = ConsistentHashController.Companion.REQUEST_PATH) requestPath: String?): ResponseEntity<*>? {
        val cache = trafficRouterManager.getTrafficRouter().consistentHashForCoverageZone(ip, deliveryServiceId, requestPath)
    ...
    @ResponseBody
    fun hashCoverageZoneDeepCache(@RequestParam(name = "ip") ip: String?,
                                  @RequestParam(name = ConsistentHashController.Companion.DELIVERY_SERVICE_ID) deliveryServiceId: String?,
                                  @RequestParam(name = ConsistentHashController.Companion.REQUEST_PATH) requestPath: String?): ResponseEntity<*>? {
        val cache = trafficRouterManager.getTrafficRouter().consistentHashForCoverageZone(ip, deliveryServiceId, requestPath, true)
                ?: return ResponseEntity.status(HttpStatus.NOT_FOUND).body("{}")
        return ResponseEntity.ok(cache)
    }
    ...
    @ResponseBody
    fun hashGeolocatedCache(@RequestParam(name = "ip") ip: String?,
                            @RequestParam(name = ConsistentHashController.Companion.DELIVERY_SERVICE_ID) deliveryServiceId: String?,
                            @RequestParam(name = ConsistentHashController.Companion.REQUEST_PATH) requestPath: String?): ResponseEntity<*>? {
        val cache = trafficRouterManager.getTrafficRouter().consistentHashForGeolocation(ip, deliveryServiceId, requestPath)
                ?: return ResponseEntity.status(HttpStatus.NOT_FOUND).body("{}")
        return ResponseEntity.ok(cache)
    }
    ...
    @ResponseBody
    fun hashDeliveryService(@RequestParam(name = ConsistentHashController.Companion.DELIVERY_SERVICE_ID) deliveryServiceId: String?,
                            @RequestParam(name = ConsistentHashController.Companion.REQUEST_PATH) requestPath: String?): ResponseEntity<*>? {
        val deliveryService = trafficRouterManager.getTrafficRouter().consistentHashDeliveryService(deliveryServiceId, requestPath)
                ?: return ResponseEntity.status(HttpStatus.NOT_FOUND).body("{}")
    ...
    @ResponseBody
    fun testPatternBasedRegex(@RequestParam(name = "regex") regex: String?,
                              @RequestParam(name = ConsistentHashController.Companion.REQUEST_PATH) requestPath: String?): ResponseEntity<MutableMap<String?, String?>?>? {
        ...
        if (requestPath != null && requestPath.length > ConsistentHashController.Companion.MAX_REQUEST_PATH_LENGTH) {
            ...
            map["Bad Input"] = "Request Path length is restricted by API to " + ConsistentHashController.Companion.MAX_REQUEST_PATH_LENGTH + " characters"
    ...
    @ResponseBody
    fun testPatternBasedDeliveryService(@RequestParam(name = ConsistentHashController.Companion.DELIVERY_SERVICE_ID) deliveryServiceId: String?,
                                        @RequestParam(name = ConsistentHashController.Companion.REQUEST_PATH) requestPath: String?): ResponseEntity<MutableMap<String?, String?>?>? {
        val pathToHash = trafficRouterManager.getTrafficRouter().buildPatternBasedHashStringDeliveryService(deliveryServiceId, requestPath)
                ?: return ResponseEntity.status(HttpStatus.NOT_FOUND).body(null)
        val map: MutableMap<String?, String?> = HashMap()
        map[ConsistentHashController.Companion.REQUEST_PATH] = requestPath
    ...
    @ResponseBody
    fun hashSteeringCoverageZoneCache(@RequestParam(name = "ip") ip: String?,
                                      @RequestParam(name = ConsistentHashController.Companion.DELIVERY_SERVICE_ID) deliveryServiceId: String?,
                                      @RequestParam(name = ConsistentHashController.Companion.REQUEST_PATH) requestPath: String?): ResponseEntity<*>? {
        val cache = trafficRouterManager.getTrafficRouter().consistentHashSteeringForCoverageZone(ip, deliveryServiceId, requestPath)
                ?: return ResponseEntity.status(HttpStatus.NOT_FOUND).body("{}")
        return ResponseEntity.ok(cache)
    }
