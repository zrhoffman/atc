...
class ZoneSignerImpl : ZoneSigner {
    ...
    private fun sign(rrset: RRset?, dnskeyRecord: DNSKEYRecord?, privateKey: PrivateKey?, inception: Date?, expiration: Date?): RRSIGRecord? {
        return try {
            DNSSEC.sign(rrset, dnskeyRecord, privateKey, inception, expiration)
        } catch (e: DNSSECException) {
            val message = String.format("Failed to sign Resource Record Set for %s %d %d %d : %s",
                    dnskeyRecord.getName(), dnskeyRecord.getDClass(), dnskeyRecord.getType(), dnskeyRecord.getTTL(), e.message)
    ...
    private fun signRRset(rrSet: RRset?, kskPairs: MutableList<DnsSecKeyPair?>?, zskPairs: MutableList<DnsSecKeyPair?>?,
                          inception: Date?, expiration: Date?,
                          RRSIGCache: ConcurrentMap<RRSIGCacheKey?, ConcurrentMap<RRsetKey?, RRSIGRecord?>?>?): RRset? {
        ...
        pairs.forEach(Consumer { pair: DnsSecKeyPair? ->
            ...
            try {
                if (RRSIGCache == null) {
                    ...
                    val sigMap = RRSIGCache.computeIfAbsent(RRSIGCacheKey(privateKey.encoded, dnskeyRecord.algorithm), Function<RRSIGCacheKey?, ConcurrentMap<RRsetKey?, RRSIGRecord?>?> { rrsigCacheKey: RRSIGCacheKey? -> ConcurrentHashMap() })
                    signature = sigMap.computeIfAbsent(RRsetKey(rrSet), Function { k: RRsetKey? -> sign(rrSet, dnskeyRecord, privateKey, inception, expiration) })
                    if (signature != null && isSignatureAlmostExpired(signature.timeSigned, signature.expire, now)) {
                        signature = sigMap.compute(RRsetKey(rrSet), BiFunction { k: RRsetKey?, v: RRSIGRecord? -> sign(rrSet, dnskeyRecord, privateKey, inception, expiration) })
                    }
                }
            } catch (e: Exception) {
                val message = String.format("Failed to sign Resource Record Set for %s %d %d %d : %s",
    ...
    private fun createNsecRecords(records: MutableList<Record?>?): MutableList<NSECRecord?>? {
        val recordMap = records.stream().collect(Collectors.groupingBy(Function { obj: Record? -> obj.getName() }))
        ...
        names.forEach(Consumer { name: Name? ->
            val mostTypes = recordMap[name].stream().mapToInt { obj: Record? -> obj.getType() }.toArray()
    ...
    override fun signZone(records: MutableList<Record?>?, kskPairs: MutableList<DnsSecKeyPair?>?,
                          zskPairs: MutableList<DnsSecKeyPair?>?, inception: Date?, expiration: Date?,
                          RRSIGCache: ConcurrentMap<RRSIGCacheKey?, ConcurrentMap<RRsetKey?, RRSIGRecord?>?>?): MutableList<Record?>? {
    ...
    companion object {
        private val LOGGER = LogManager.getLogger(ZoneSignerImpl::class.java)
    }
}