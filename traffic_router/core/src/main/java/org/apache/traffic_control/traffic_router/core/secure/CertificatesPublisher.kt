...
javax.management.* org.springframework.web.bind.annotation .RequestMapping
 ...
 * under the License.
 */   class CertificatesPublisher(certificatesQueue: BlockingQueue<MutableList<CertificateData?>?>?, publishStatusQueue: BlockingQueue<Boolean?>?,
        ...
        worker = Thread(Runnable {
            while (running) {
                try {
                    ...
                    CertificatesPublisher.Companion.LOGGER.warn("Interrupted while waiting for new certificate data list, trying again...", e)
                }
            }
        })
        worker.start()
    ...
    private fun publishCertificateList(certificateDataList: MutableList<CertificateData?>?) {
        try {
            ...
            CertificatesPublisher.Companion.LOGGER.error("Failed to add certificate data list as management MBean! " + e.javaClass.simpleName + ": " + e.message, e)
        }
    }
    ...
    companion object {
        private val LOGGER = LogManager.getLogger(CertificatesPublisher::class.java)
    }
}