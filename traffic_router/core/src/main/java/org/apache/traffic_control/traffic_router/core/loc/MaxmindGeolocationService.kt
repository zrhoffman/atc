...
class MaxmindGeolocationService : GeolocationService {
    ...
    override fun location(ip: String?): Geolocation? {
        ...
        val response = getCityResponse(databaseReader, ip.split("/".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()[0])
    ...
    fun destroy() {
        ...
        try {
            ...
            MaxmindGeolocationService.Companion.LOGGER.warn("Caught exception while trying to close geolocation database reader: " + ex.message, ex)
        }
    }
    ...
    companion object {
        private val LOGGER = LogManager.getLogger(MaxmindGeolocationService::class.java)
    }
}