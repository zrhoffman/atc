...
abstract class AbstractProtocol : Protocol {
    ...
    fun submit(job: SocketHandler?) {
        ...
        handler = if (queueDepth > 0 && queueLength >= queueDepth || queueDepth == 0 && queueLength > 0) {
            LOGGER.warn(String.format("%s request thread pool full and queue depth limit reached (%d >= %d); discarding request",
    ...
    companion object {
        ...
        private val LOGGER = LogManager.getLogger(AbstractProtocol::class.java)
