...
import org.apache.traffic_control.traffic_router.core.dns.DNSAccessRecordimport org.xbill.DNS.*import java.math.RoundingModeimport

java.text.DecimalFormat org.springframework.web.bind.annotation .RequestMapping
...
object DNSAccessEventBuilder {
    fun create(dnsAccessRecord: DNSAccessRecord?): String? {
        ...
        val dsString = if (dnsAccessRecord.getDeliveryServiceXmlIds() == null) "-" else dnsAccessRecord.getDeliveryServiceXmlIds()
        val dsInfo = "svc=\"$dsString\""
    ...
    private fun createEvent(dnsAccessRecord: DNSAccessRecord?): String? {
        val timeString = String.format("%d.%03d", dnsAccessRecord.getQueryInstant() / 1000, dnsAccessRecord.getQueryInstant() % 1000)
        ...
        val stringBuilder = StringBuilder(timeString).append(" qtype=DNS chi=").append(clientAddressString).append(" rhi=")
    ...
    private fun createTTLandAnswer(dnsMessage: Message?): String? {
        ...
        return "ttl=\"" + ttlStringBuilder.toString().trim { it <= ' ' } + "\" ans=\"" + answerStringBuilder.toString().trim { it <= ' ' } + "\""
    }
    ...
    fun create(dnsAccessRecord: DNSAccessRecord?, wireParseException: WireParseException?): String? {
        ...
        val dsID = if (dnsAccessRecord.getDeliveryServiceXmlIds() == null) "-" else dnsAccessRecord.getDeliveryServiceXmlIds()
