 ...
 */   class LetsEncryptDnsChallengeWatcher : AbstractResourceWatcher() {
    ...
    public override fun useData(data: String?): Boolean {
        try {
            ...
            val dataMap = mapper.readValue(data, object : TypeReference<HashMap<String?, MutableList<LetsEncryptDnsChallenge?>?>?>() {})
            val challengeList = dataMap["response"]
            val mostRecentConfig = mapper.readTree(readConfigFile())
            val deliveryServicesNode = JsonUtils.getJsonNode(mostRecentConfig, ConfigHandler.Companion.deliveryServicesKey) as ObjectNode
            challengeList.forEach(Consumer { challenge: LetsEncryptDnsChallenge? ->
                val deliveryServiceConfig = deliveryServicesNode[challenge.getXmlId()] as ObjectNode
                if (deliveryServiceConfig == null) {
                    LetsEncryptDnsChallengeWatcher.Companion.LOGGER.error("finding deliveryservice in cr-config for " + challenge.getXmlId())
                    return@forEach
                }
                var staticEntryString = challenge.getFqdn()
                val domains = deliveryServiceConfig["domains"] as ArrayNode
                if (domains == null || domains.size() == 0) {
                    LetsEncryptDnsChallengeWatcher.Companion.LOGGER.error("no domains found in cr-config for deliveryservice " + challenge.getXmlId())
                    return@forEach
                }
                val domainIter = domains.iterator()
                while (domainIter.hasNext()) {
                    val domainNode = domainIter.next()
                ...
                val staticDnsEntriesNode = updateStaticEntries(challenge, staticEntryString, mapper, deliveryServiceConfig)
                deliveryServiceConfig.set<JsonNode?>("staticDnsEntries", staticDnsEntriesNode)
                deliveryServicesNode.set<JsonNode?>(challenge.getXmlId(), deliveryServiceConfig)
            })
            ...
            LetsEncryptDnsChallengeWatcher.Companion.LOGGER.warn("Failed updating dns challenge txt record with data from $dataBaseURL:", e)
        }
        return false
    }
    ...
    override fun verifyData(data: String?): Boolean {
        try {
            val mapper = ObjectMapper(JsonFactory())
            mapper.readValue(data, object : TypeReference<HashMap<String?, MutableList<LetsEncryptDnsChallenge?>?>?>() {})
            return true
        } catch (e: Exception) {
            LetsEncryptDnsChallengeWatcher.Companion.LOGGER.warn("Failed to build dns challenge data while verifying:", e)
        }
        return false
    }
    ...
    private fun readConfigFile(): String? {
        try {
            FileInputStream(databasesDirectory.resolve(configFile).toString()).use { `is` ->
                BufferedReader(InputStreamReader(`is`)).use { buf ->
    ...
    private fun updateStaticEntries(challenge: LetsEncryptDnsChallenge?, name: String?, mapper: ObjectMapper?, deliveryServiceConfig: ObjectNode?): ArrayNode? {
    ...
    companion object {
        private val LOGGER = LogManager.getLogger(LetsEncryptDnsChallengeWatcher::class.java)
        val DEFAULT_LE_DNS_CHALLENGE_URL: String? = "https://\${toHostname}/api/" + TrafficOpsUtils.Companion.TO_API_VERSION + "/letsencrypt/dnsrecords/"
    }
}