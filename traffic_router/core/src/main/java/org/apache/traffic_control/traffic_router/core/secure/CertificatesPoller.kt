...
class CertificatesPoller : ConfigurationListener {
    ...
    private var pollingInterval: Long = CertificatesPoller.Companion.defaultFixedRate
    ...
    fun getEnvironmentPollingInterval(): Long? {
        if (environment == null) {
            CertificatesPoller.Companion.LOGGER.warn("Could not find Environment object!")
        }
        return try {
            val value = environment.getProperty(CertificatesPoller.Companion.intervalProperty, Long::class.java)
            if (value == null) {
                CertificatesPoller.Companion.LOGGER.info("No custom value for " + CertificatesPoller.Companion.intervalProperty)
            }
            value
        } catch (e: Exception) {
            CertificatesPoller.Companion.LOGGER.warn("Failed to get value of " + CertificatesPoller.Companion.intervalProperty + ": " + e.message)
            null
        }
    }
    ...
    fun start() {
        val runnable = Runnable {
            try {
                ...
                val certificateDataList = certificatesClient.refreshData() ?: return@Runnable
                ...
                CertificatesPoller.Companion.LOGGER.warn("Failed to refresh certificate data: " + t.javaClass.canonicalName + " " + t.message, t)
            }
        }
        val customFixedRate = getEnvironmentPollingInterval()
        if (customFixedRate == null) {
            CertificatesPoller.Companion.LOGGER.info("Using default fixed rate polling interval $pollingInterval msec")
        } else {
            CertificatesPoller.Companion.LOGGER.info("Using custom fixed rate polling interval $customFixedRate msec")
            pollingInterval = customFixedRate
        }
        future = executor.scheduleWithFixedDelay(runnable, 0, pollingInterval, TimeUnit.MILLISECONDS)
        CertificatesPoller.Companion.LOGGER.info("Polling for certificates every $pollingInterval msec")
    }
    ...
    fun restart() {
        ...
        while (!futureIsDone()) {
            try {
                ...
                CertificatesPoller.Companion.LOGGER.info("Interrupted sleep while waiting for certificate poller future to finish")
            }
        }
        start()
    }
    ...
    companion object {
        private val LOGGER = LogManager.getLogger(CertificatesPoller::class.java)
