...
import org.xbill.DNS.*import java.util.function.Consumerimport

java.util.function.Functionimport java.util.stream.Collectors org.springframework.web.bind.annotation .RequestMapping
...
class RRSetsBuilder {
    ...
    fun build(records: MutableList<Record?>?): MutableList<RRset?>? {
        val map = records.stream().sorted().collect(
                Collectors.groupingBy<Record?, String?, Any?, MutableList<Record?>?>(Function<Record?, String?> { record: Record? -> RRSetsBuilder.Companion.qualifer(record) }, Collectors.toList())
        )
