...
import org.apache.traffic_control.traffic_router.core.loc.NetworkNodeExceptionimport

java.net.Inet4Addressimport java.net.Inet6Addressimport java.net.InetAddressimport java.net.UnknownHostExceptionimport java.util.*
class CidrAddress @JvmOverloads constructor(private val address: InetAddress?, private val netmaskLength: Int = address.getAddress().size * 8) : Comparable<CidrAddress?> {
    ...
    init {
        ...
        for (i in 0 until netmaskLength) {
            maskBytes[i / 8] = maskBytes[i / 8].toInt() or (1 shl 7 - i) % 8
        }
    }
    ...
    override fun compareTo(other: CidrAddress?): Int {
        ...
        val numNetmaskBytes = Math.ceil(len.toDouble() / 8).toInt()
        for (i in 0 until numNetmaskBytes) {
            val diff = (hostBytes.get(i).toInt() and mask.get(i).toInt()) - (other.hostBytes.get(i).toInt() and mask.get(i).toInt())
            if (diff != 0) {
                return diff
            }
        }
    ...
    companion object {
        ...
        fun fromString(cidrString: String?): CidrAddress? {
            val hostNetworkArray: Array<String?> = cidrString.split("/".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
