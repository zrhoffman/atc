...
com.maxmind.geoip2.DatabaseReaderimport com.maxmind.geoip2.exception.GeoIp2Exceptionimport com.maxmind.geoip2.model.AnonymousIpResponseimport org.apache.traffic_control.traffic_router.core.loc.AnonymousIpDatabaseServiceimport java.io.*import java.net.InetAddress
...
class AnonymousIpDatabaseService {
    ...
    fun reloadDatabase() {
        ...
        if (databaseFile != null) {
            ...
            initialized = if (databaseReader != null) {
                true
            } else {
    ...
    private fun createDatabaseReader(databaseFile: File?): DatabaseReader? {
        ...
        return try {
            ...
            AnonymousIpDatabaseService.Companion.LOGGER.error(databaseFile.getAbsolutePath() + " is not a valid Anonymous IP data file", e)
            null
        }
    }
    ...
    fun lookupIp(ipAddress: InetAddress?): AnonymousIpResponse? {
        return if (initialized) {
            ...
            try {
                ...
                AnonymousIpDatabaseService.Companion.LOGGER.debug(String.format("AnonymousIP: IP %s not found in anonymous ip database", ipAddress.getHostAddress()))
                null
            } catch (e: IOException) {
                AnonymousIpDatabaseService.Companion.LOGGER.error("AnonymousIp ERR: IO Error during lookup of ip in anonymous ip database", e)
                null
            }
        } else {
            null
    ...
    fun finalize() {
        if (databaseReader != null) {
            try {
                ...
                AnonymousIpDatabaseService.Companion.LOGGER.warn("Caught exception while trying to close anonymous ip database reader: ", e)
            }
        }
        super.finalize()
    }
    ...
    companion object {
        private val LOGGER = LogManager.getLogger(AnonymousIpDatabaseService::class.java)
    }
}