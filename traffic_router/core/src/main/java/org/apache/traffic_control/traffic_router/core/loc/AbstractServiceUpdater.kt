...
org.apache.commons.codec.digest.DigestUtilsimport org.apache.commons.io.IOUtilsimport org.apache.logging.log4j.LogManagerimport java.io.*import java.lang.Exceptionimport
...
abstract class AbstractServiceUpdater {
    ...
    private val updater: Runnable? = object : Runnable {
        override fun run() {
            try {
                ...
                AbstractServiceUpdater.Companion.LOGGER.error("[" + javaClass.simpleName + "] Failed updating database!", t)
            }
        }
    }
    ...
    fun updateDatabase(): Boolean {
        ...
        try {
            try {
                ...
                AbstractServiceUpdater.Companion.LOGGER.fatal("[" + javaClass.simpleName + "] Caught exception while attempting to download: " + getDataBaseURL(), e)
                return false
            }
    ...
    protected fun copyDatabaseIfDifferent(existingDB: File?, newDB: File?): Boolean {
        ...
        if (!renamed) {
            AbstractServiceUpdater.Companion.LOGGER.fatal("[" + javaClass.simpleName + "] Unable to rename " + newDB + " to " + existingDB.getAbsolutePath() + "; current working directory is " + System.getProperty("user.dir"))
            return false
        }
        AbstractServiceUpdater.Companion.LOGGER.info("[" + javaClass.simpleName + "] Successfully updated database " + existingDB)
        return true
    ...
    protected open fun downloadDatabase(url: String?, existingDb: File?): File? {
        ...
        val dbURL = URL(url)
        val conn = dbURL.openConnection()
        val existingLastModified = existingDb.lastModified()
        if (conn is HttpURLConnection && useModifiedTimestamp(existingDb)) {
        ...
        conn.getInputStream().use { `in` ->
            FileOutputStream(outputFile).use { out ->
                if (conn is HttpURLConnection) {
                    ...
                    if ((conn as HttpURLConnection).responseCode == HttpURLConnection.HTTP_NOT_MODIFIED) {
                        AbstractServiceUpdater.Companion.LOGGER.info("[" + javaClass.simpleName + "] " + url + " not modified since our existing database's last update time of " + Date(existingLastModified))
                        return existingDb
                    }
                } else if (dbURL.protocol == "file" && conn.lastModified > 0 && conn.lastModified <= existingLastModified) {
                    AbstractServiceUpdater.Companion.LOGGER.info("[" + javaClass.simpleName + "] " + url + " not modified since our existing database's last update time of " + Date(existingLastModified))
                    return existingDb
                }
                IOUtils.copy(if (sourceCompressed) GZIPInputStream(`in`) else `in`, out)
            }
        }
        return outputFile
    }
    ...
    companion object {
        private val LOGGER = LogManager.getLogger(AbstractServiceUpdater::class.java)
    }
}