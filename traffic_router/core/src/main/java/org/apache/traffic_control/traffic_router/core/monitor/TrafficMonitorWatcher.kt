...
class TrafficMonitorWatcher : ApplicationListener<ApplicationContextEvent?> {
    ...
    fun init() {
        ...
        processConfig()
        crUpdater = PeriodicResourceUpdater(crHandler, TrafficMonitorResourceUrl(this, configUrl), databasesDirectory.resolve(configFile).toString(), configRefreshPeriod, true)
        crUpdater.init()
        stateUpdater = PeriodicResourceUpdater(stateHandler, TrafficMonitorResourceUrl(this, stateUrl), databasesDirectory.resolve(statusFile).toString(), statusRefreshPeriod, true)
        stateUpdater.init()
    }
    ...
    private fun processConfig() {
        ...
        try {
            ...
            var localConfig = java.lang.Boolean.parseBoolean(props.getProperty("traffic_monitor.bootstrap.local", "false"))
            var localEnvString = System.getenv("TRAFFIC_MONITOR_BOOTSTRAP_LOCAL")
            if (localEnvString != null) {
                localEnvString = localEnvString.lowercase(Locale.getDefault())
            }
            ...
            if (localConfig || !isBootstrapped()) {
                ...
                if (hostList == null || hostList.isEmpty()) {
                    ...
                    setHosts(if (hostList.contains(";")) hostList.split(";".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray() else arrayOf(hostList))
                }
            ...
            if (reloadPeriodStr != null) {
                val newReloadPeriod: Long = reloadPeriodStr.toInt()
        ...
        if (hosts == null) {
            hosts = monitorHosts.split(";".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
        }
    }
    ...
    companion object {
        private val LOGGER = LogManager.getLogger(TrafficMonitorWatcher::class.java)
