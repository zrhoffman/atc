...
org.apache.traffic_control.traffic_router.core.util.AbstractUpdatableimport org.apache.traffic_control.traffic_router.core.util.PeriodicResourceUpdaterimport org.apache.traffic_control.traffic_router.core.util.ResourceUrlimport java.io.*import java.util.concurrent.*
import java.util.zip.GZIPInputStream

org.apache.commons.codec.digest.DigestUtilsimport org.apache.commons.io.IOUtilsimport org.apache.logging.log4j.LogManagerimport org.asynchttpclient.*import java.io.*
 ...
 * @author jlaue
 */
class PeriodicResourceUpdater(private val listener: AbstractUpdatable?, protected val urls: ResourceUrl?, protected var databaseLocation: String?, interval: Int, pauseTilLoaded: Boolean) {
            ...
            if (!hasBeenLoaded || needsUpdating(existingDB)) {
                ...
                if (request != null) {
                    ...
                    if (asyncHttpClient != null && !asyncHttpClient.isClosed()) {
                        asyncHttpClient.executeRequest<Any?>(request, PeriodicResourceUpdater.UpdateHandler(request)) // AsyncHandlers are NOT thread safe; one instance per request
                    }
                    return true
                }
            } else {
    ...
    private inner class UpdateHandler(val request: Request?) : AsyncCompletionHandler<Any?>() {
        ...
        override fun onCompleted(response: Response?): Int? {
            ...
            if (PeriodicResourceUpdater.Companion.GZIP_ENCODING_STRING == response.getHeader("Content-Encoding")) {
                ...
                GZIPInputStream(response.getResponseBodyAsStream()).use { zippedInputStream ->
    ...
    companion object {
        private val LOGGER = LogManager.getLogger(PeriodicResourceUpdater::class.java)
