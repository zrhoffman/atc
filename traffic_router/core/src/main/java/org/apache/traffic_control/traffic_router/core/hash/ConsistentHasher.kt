...
import org.apache.traffic_control.traffic_router.core.ds.Dispersionimport

import org.apache.traffic_control.traffic_router.core.hash.Hashableimport org.apache.traffic_control.traffic_router.core.hash.MD5HashFunctionimport java.util.* org.springframework.web.bind.annotation .RequestMapping
...
class ConsistentHasher {
    ...
    fun <T : Hashable<*>?> selectHashable(hashables: MutableList<T?>?, dispersion: Dispersion?, s: String?): T? {
        val selectedHashables = selectHashables<T?>(hashables, dispersion, s)
    ...
    fun <T : Hashable<*>?> selectHashables(hashables: MutableList<T?>?, s: String?): MutableList<T?>? {
        return selectHashables<T?>(hashables, null, s)
    }

    fun <T : Hashable<*>?> selectHashables(hashables: MutableList<T?>?, dispersion: Dispersion?, s: String?): MutableList<T?>? {
        val sortedHashables = sortHashables<T?>(hashables, s)
    ...
    private fun <T : Hashable<*>?> sortHashables(hashables: MutableList<T?>?, s: String?): SortedMap<Double?, T?>? {
        ...
        for (hashable in hashables) {
            ...
            val hashDelta = getSafePositiveHash<T?>(hashableMap, Math.abs(hash - closestHash))
            hashableMap[hashDelta] = hashable
        }
        return synthesizeZeroHashes<T?>(hashableMap, zeroHashes)
    }
	 ...
	 */
    private fun <T : Hashable<*>?> synthesizeZeroHashes(hashableMap: SortedMap<Double?, T?>?, zeroHashes: MutableList<T?>?): SortedMap<Double?, T?>? {
        ...
        for (hashable in zeroHashes) {
            if (hashable.getOrder() >= 0) { // append
                val syntheticHash = getSafePositiveHash<T?>(hashableMap, maxHash)
                hashableMap[syntheticHash] = hashable
                maxHash = syntheticHash
            } else { // negative order specified, prepend
                val syntheticHash = getSafeNegativeHash<T?>(hashableMap, minHash)
                hashableMap[syntheticHash] = hashable
    ...
    private fun <T : Hashable<*>?> getSafePositiveHash(hashableMap: SortedMap<Double?, T?>?, hash: Double): Double {
        return getSafeHash<T?>(hashableMap, hash, true)
    }

    private fun <T : Hashable<*>?> getSafeNegativeHash(hashableMap: SortedMap<Double?, T?>?, hash: Double): Double {
        return getSafeHash<T?>(hashableMap, hash, false)
    }

    private fun <T : Hashable<*>?> getSafeHash(hashableMap: SortedMap<Double?, T?>?, hash: Double, add: Boolean): Double {
