 ...
 */
package org.apache.traffic_control.traffic_router.core.utilimportimportimport

com.fasterxml.jackson.databind.JsonNodeimport org.apache.traffic_control.traffic_router.core.config.WatcherConfigimport org.apache.traffic_control.traffic_router.core.loc.AbstractServiceUpdaterimport org.apache.traffic_control.traffic_router.core.util.AbstractResourceWatcherimport org.apache.traffic_control.traffic_router.core.util.ProtectedFetcherimport org.apache.traffic_control.traffic_router.core.util.TrafficOpsUtilsimport java.net.* org.apache.logging.log4j.LogManagerimport java.io.*import java.lang.Exception
import java.net.*
...
abstract class AbstractResourceWatcher : AbstractServiceUpdater() {
    ...
    fun configure(config: JsonNode?) {
        ...
        try {
            ...
            AbstractResourceWatcher.Companion.LOGGER.warn("Failed to update URL for TrafficOps authorization, " +
        ...
        if (authUrl == null || credentials == null) {
            AbstractResourceWatcher.Companion.LOGGER.warn("[ " + javaClass.simpleName + " ] Invalid Traffic Ops authorization URL or credentials data, not updating configuration!")
            return
        }
        val watcherConfig = WatcherConfig(getWatcherConfigPrefix(), config, trafficOpsUtils)
        val resourceUrl = if (watcherConfig.url != null && !watcherConfig.url.isEmpty()) watcherConfig.url else defaultDatabaseURL
        ...
        if (authUrl == authorizationUrl && credentials == postData && resourceUrl == dataBaseURL && pollingInterval == getPollingInterval() && configTimeout == timeout) {
            AbstractResourceWatcher.Companion.LOGGER.info("[ " + javaClass.name + " ] Nothing changed in configuration")
            return
        }
    ...
    override fun loadDatabase(): Boolean {
        ...
        val jsonData = CharArray(existingDB.length().toInt())
    ...
    override fun verifyDatabase(dbFile: File?): Boolean {
        ...
        val jsonData = CharArray(dbFile.length().toInt())
    ...
    override fun downloadDatabase(url: String?, existingDb: File?): File? {
        ...
        if (fetcher == null) {
            AbstractResourceWatcher.Companion.LOGGER.warn("[" + javaClass.simpleName + "] Waiting for configuration to be processed, unable to download from '" + interpolatedUrl + "'")
            return null
        }
        ...
        try {
            ...
            AbstractResourceWatcher.Companion.LOGGER.warn("[ " + javaClass.simpleName + " ] Failed to fetch data from '" + interpolatedUrl + "': " + e.message)
        }
        if (jsonData == null) {
            return existingDb
        }
        ...
        try {
            ...
            FileWriter(databaseFile).use { fw ->
                fw.write(jsonData)
                fw.flush()
            }
        } catch (e: IOException) {
            AbstractResourceWatcher.Companion.LOGGER.warn("Failed to create file from data received from '$interpolatedUrl'")
        }
        return databaseFile
    }
    ...
    companion object {
        private val LOGGER = LogManager.getLogger(AbstractResourceWatcher::class.java)
    }
}