...
com.fasterxml.jackson.databind.ObjectMapperimport org.apache.traffic_control.traffic_router.core.router.TrafficRouterManagerimport org.apache.traffic_control.traffic_router.core.secure.CertificatesClientimport org.apache.traffic_control.traffic_router.core.secure.CertificatesResponseimport org.apache.traffic_control.traffic_router.core.util.ProtectedFetcherimport org.apache.traffic_control.traffic_router.core.util.TrafficOpsUtilsimport org.apache.traffic_control.traffic_router.shared.CertificateDataimport java.util.* com.fasterxml.jackson.core.type.TypeReferenceimport org.apache.logging.log4j.LogManager
...
class CertificatesClient {
    ...
    fun refreshData(): MutableList<CertificateData?>? {
        ...
        while (status != HttpURLConnection.HTTP_NOT_MODIFIED && status != HttpURLConnection.HTTP_OK) {
            ...
            try {
                ...
                if (!shutdown) {
                    CertificatesClient.Companion.LOGGER.warn("Interrupted while pausing to fetch certificates from traffic ops", e)
                } else {
                    return null
                }
            }
    ...
    fun fetchRawData(stringBuilder: StringBuilder?): Int {
        ...
        val certificatesUrl = trafficOpsUtils.getUrl("certificate.api.url", "https://\${toHostname}/api/" + TrafficOpsUtils.Companion.TO_API_VERSION + "/cdns/name/\${cdnName}/sslkeys")
        try {
            val fetcher = ProtectedFetcher(trafficOpsUtils.getAuthUrl(), trafficOpsUtils.getAuthJSON().toString(), 15000)
            return fetcher.getIfModifiedSince(certificatesUrl, 0L, stringBuilder)
        } catch (e: Exception) {
            CertificatesClient.Companion.LOGGER.warn("Failed to fetch data for certificates from " + certificatesUrl + "(" + e.javaClass.simpleName + ") : " + e.message, e)
        }
        return -1
    }
    ...
    fun getCertificateData(jsonData: String?): MutableList<CertificateData?>? {
        try {
            ...
            return (ObjectMapper().readValue(jsonData, object : TypeReference<CertificatesResponse?>() {}) as CertificatesResponse).response
        } catch (e: Exception) {
    ...
    companion object {
        private val LOGGER = LogManager.getLogger(CertificatesClient::class.java)
