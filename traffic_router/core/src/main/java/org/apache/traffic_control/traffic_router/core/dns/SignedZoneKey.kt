...
class SignedZoneKey(name: Name?, records: MutableList<Record?>?) : ZoneKey(name, records) {
    ...
    fun getRefreshHorizon(): Long {
        return timestamp + Math.round(getSignatureDuration().toDouble() / 2.0) // force a refresh when we're halfway through our validity period
    }
    ...
    companion object {
        private val LOGGER = LogManager.getLogger(SignedZoneKey::class.java)
    }
}