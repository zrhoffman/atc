...
class CoverageZoneTest constructor() {
    ...
    fun before() {
        val deliveryService: DeliveryService? = Mockito.mock(DeliveryService::class.java)
        Mockito.`when`(deliveryService.getId()).thenReturn("delivery-service-1")
        val deliveryServiceReference: DeliveryServiceReference? =
            DeliveryServiceReference("delivery-service-1", "some.example.com")
        val deliveryServices: MutableList<DeliveryServiceReference?>? = ArrayList()
        deliveryServices.add(deliveryServiceReference)
        val testLocation: Geolocation? = Geolocation(40.0, -101)
        val farEastLocation: Geolocation? = Geolocation(40.0, -101.5)
        val eastLocation: Geolocation? = Geolocation(40.0, -100)
        val westLocation: Geolocation? = Geolocation(40.0, -105)
        val farEastCache1: Cache? = Cache("far-east-cache-1", "hashid", 1)
        farEastCache1.setIsAvailable(true)
        val lms: MutableSet<LocalizationMethod?>? = HashSet()
        lms.add(LocalizationMethod.GEO)
        val farEastCacheGroup: CacheLocation? = CacheLocation("far-east-cache-group", farEastLocation, lms)
        farEastCacheGroup.addCache(farEastCache1)
        farEastCache1.setDeliveryServices(deliveryServices)
        val eastCache1: Cache? = Cache("east-cache-1", "hashid", 1)
        eastCache1.setIsAvailable(true)
        val eastCacheGroup: CacheLocation? = CacheLocation("east-cache-group", eastLocation)
        eastCacheGroup.addCache(eastCache1)
        val westCache1: Cache? = Cache("west-cache-1", "hashid", 1)
        westCache1.setIsAvailable(true)
        westCache1.setDeliveryServices(deliveryServices)
        val westCacheGroup: CacheLocation? = CacheLocation("west-cache-group", westLocation)
        ...
        val eastNetworkNode: NetworkNode? = NetworkNode("12.23.34.0/24", "east-cache-group", testLocation)
    ...
    fun trafficRouterReturnsNearestCacheGroupForDeliveryService() {
        ...
        MatcherAssert.assertThat(cacheLocation.getId(), org.hamcrest.Matchers.equalTo("west-cache-group"))
