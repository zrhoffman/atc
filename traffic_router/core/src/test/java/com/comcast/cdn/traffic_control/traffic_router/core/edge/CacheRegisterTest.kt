...
class CacheRegisterTest constructor() {
    ...
    fun before() {
        ...
        val deliveryServiceMatcher3: DeliveryServiceMatcher? = DeliveryServiceMatcher(deliveryService2)
        ...
        val dnsMatcher1: DeliveryServiceMatcher? = DeliveryServiceMatcher(deliveryService1)
        dnsMatcher1.addMatch(DeliveryServiceMatcher.Type.HOST, ".*\\.service01-kabletown\\..*", "")
        val dnsMatchers: TreeSet<DeliveryServiceMatcher?>? = TreeSet()
    ...
    fun itPicksTheMostSpecificDeliveryService() {
        ...
        MatcherAssert.assertThat(
            cacheRegister.getDeliveryService(httpRequest).getId(),
            Matchers.equalTo("delivery service 2")
        )
        val request: Request? = Request()
        request.setHostname("foo.service01-kabletown.com")
        MatcherAssert.assertThat(
            cacheRegister.getDeliveryService(request).getId(),
            Matchers.equalTo("delivery service 1")
        )
    }
