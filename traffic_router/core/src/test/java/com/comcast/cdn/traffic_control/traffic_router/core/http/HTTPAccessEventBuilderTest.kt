...
class HTTPAccessEventBuilderTest constructor() {
    private var request: HttpServletRequest? = null
    @Before
    @Throws(Exception::class)
    fun before() {
        PowerMockito.mockStatic(Date::class.java)
        val startDate: Date? = Mockito.mock(Date::class.java)
        Mockito.`when`(startDate.getTime()).thenReturn(144140678000L)
        PowerMockito.whenNew(Date::class.java).withArguments(Matchers.anyLong()).thenReturn(startDate)
        val finishDate: Date? = Mockito.mock(Date::class.java)
        Mockito.`when`(finishDate.getTime()).thenReturn(144140678125L)
    ...
    fun itGeneratesAccessEvents() {
        val builder: HTTPAccessRecord.Builder? = HTTPAccessRecord.Builder(Date(144140678000L), request)
        val httpAccessRecord: HTTPAccessRecord? = builder.build()
        val httpAccessEvent: String? = HTTPAccessEventBuilder.create(httpAccessRecord)
    ...
    fun itAddsResponseData() {
        ...
        val builder: HTTPAccessRecord.Builder? = HTTPAccessRecord.Builder(Date(144140633999L), request)
        ...
        val httpAccessRecord: HTTPAccessRecord? = builder.resultType(ResultType.CZ).build()
        val httpAccessEvent: String? = HTTPAccessEventBuilder.create(httpAccessRecord)
        MatcherAssert.assertThat(
            httpAccessEvent,
    ...
    fun itRoundsUpToNearestMicroSecond() {
        ...
        val fastFinishDate: Date? = Mockito.mock(Date::class.java)
        Mockito.`when`(fastFinishDate.getTime()).thenReturn(144140678000L)
    ...
    fun itUsesXMmClientIpHeaderForChi() {
        ...
        val httpAccessRecord: HTTPAccessRecord? = HTTPAccessRecord.Builder(Date(144140678000L), request).build()
        val httpAccessEvent: String? = HTTPAccessEventBuilder.create(httpAccessRecord)
        MatcherAssert.assertThat(
            httpAccessEvent,
