...
class ZoneManagerTest constructor() {
    ...
    fun setUp() {
        ...
        val file: File? = File("src/test/db/czmap.json")
        val mapper: ObjectMapper? = ObjectMapper()
        val jsonNode: JsonNode? = mapper.readTree(file)
        val coverageZones: JsonNode? = jsonNode.get("coverageZones")
        val czIter: MutableIterator<String?>? = coverageZones.fieldNames()
        while (czIter.hasNext()) {
            val loc: String? = czIter.next()
            val locData: JsonNode? = coverageZones.get(loc)
            val networks: JsonNode? = locData.get("network")
            val network: String? = networks.get(0).asText().split("/").toTypedArray().get(0)
            var ip: InetAddress? = InetAddresses.forString(network)
    ...
    fun testDynamicZoneCache() {
        ...
        val zoneManager: ZoneManager? = trafficRouter.getZoneManager()
        for (ds: DeliveryService? in cacheRegister.getDeliveryServices().values) {
            if (!ds.isDns()) {
                continue
            }
            val domain: String? = ds.getDomain()
            val edgeName: Name? = Name(ds.getRoutingName() + "." + domain + ".")
            for (source: InetAddress? in netMap.values) {
                val location: CacheLocation? =
                    trafficRouter.getCoverageZoneCacheLocation(source.getHostAddress(), ds, IPVersions.IPV4ONLY)
                val caches: MutableList<Cache?>? = trafficRouter.selectCachesByCZ(ds, location, IPVersions.IPV4ONLY)
                if (caches == null) {
                    continue
                }
                val builder: DNSAccessRecord.Builder? = DNSAccessRecord.Builder(1, source)
                ...
                if ((maxDnsIps > 0) && !trafficRouter.isConsistentDNSRouting() && (caches.size > maxDnsIps)) {
                    val top: BigInteger? = fact(caches.size)
                    val f: BigInteger? = fact(caches.size - maxDnsIps)
                    val s: BigInteger? = fact(maxDnsIps)
                    combinations = top.divide(f.multiply(s)).toLong()
                    var c: Int = 0
                    while (c < (combinations * 100)) {
                        val zone: Zone? = trafficRouter.getZone(
                            edgeName,
                            Type.A,
                            source,
                            true,
                ...
                val cacheStats: CacheStats? = zoneManager.getDynamicCacheStats()
                for (j in 0..(combinations * 100)) {
                    val missCount: Long = cacheStats.missCount()
                    val zone: Zone? = trafficRouter.getZone(edgeName, Type.A, source, true, builder)
                    Assert.assertNotNull(zone)
                    Assert.assertEquals(
                        missCount,
