...
class DNSExceptionTest constructor() {
    @Test
    fun testDNSExceptionInt() {
        val rcode: Int = Rcode.NXDOMAIN
        val ex: DNSException? = DNSException(rcode)
        Assert.assertEquals(rcode.toLong(), ex.getRcode().toLong())
    }
    ...
    fun testDNSExceptionIntString() {
        ...
        val ex: DNSException? = DNSException(rcode, msg)
        Assert.assertEquals(rcode.toLong(), ex.getRcode().toLong())
    ...
    fun testDNSExceptionIntStringThrowable() {
        val rcode: Int = Rcode.NXDOMAIN
        val msg: String? = "message"
        val cause: Exception? = Exception()
        val ex: DNSException? = DNSException(rcode, msg, cause)
        Assert.assertEquals(rcode.toLong(), ex.getRcode().toLong())
        Assert.assertEquals(msg, ex.message)
    ...
    fun testDNSExceptionIntThrowable() {
        val rcode: Int = Rcode.NXDOMAIN
        val cause: Exception? = Exception()
        val ex: DNSException? = DNSException(rcode, cause)
        Assert.assertEquals(rcode.toLong(), ex.getRcode().toLong())
        Assert.assertEquals(cause, ex.cause)
    }
}