...
class DnsNameGenerator constructor() {
    @Throws(Exception::class)
    fun getNames(deliveryServicesConfig: JsonNode?, cdnConfig: JsonNode?): MutableList<String?>? {
        val names: MutableList<String?>? = ArrayList()
        val domainName: String? = cdnConfig.get("domain_name").asText()
        for (matchsets: JsonNode? in deliveryServicesConfig.get("matchsets")) {
            for (matchset: JsonNode? in matchsets) {
                if (!("DNS" == matchset.get("protocol").asText())) {
                    continue
                }
                for (matchlist: JsonNode? in matchset.get("matchlist")) {
                    val name: String? = matchlist.get("regex").asText()
                        .replace("\\.".toRegex(), "")
                        .replace("\\*".toRegex(), "")
                        .replace("\\\\".toRegex(), "")
