...
class CacheLocationComparatorTest constructor() {
    @Test
    fun testCompareBothLocEqual() {
        val comparator: LocationComparator? = LocationComparator(Geolocation(1f, 1f))
        val loc1: CacheLocation? = CacheLocation("loc1", Geolocation(0f, 0f))
        val loc2: CacheLocation? = CacheLocation("loc2", Geolocation(0f, 0f))
    ...
    fun testCompareBothLocNull() {
        val comparator: LocationComparator? = LocationComparator(Geolocation(1f, 1f))
        val loc1: CacheLocation? = CacheLocation("loc1", null)
        val loc2: CacheLocation? = CacheLocation("loc2", null)
        Assert.assertEquals(0, comparator.compare(loc1, loc2).toLong())
        Assert.assertEquals(0, comparator.compare(loc2, loc1).toLong())
    }
    ...
    fun testCompareLocsDifferent() {
        val comparator: LocationComparator? = LocationComparator(Geolocation(1f, 1f))
        val loc1: CacheLocation? = CacheLocation("loc1", Geolocation(1f, 1f))
        val loc2: CacheLocation? = CacheLocation("loc2", Geolocation(0f, 0f))
