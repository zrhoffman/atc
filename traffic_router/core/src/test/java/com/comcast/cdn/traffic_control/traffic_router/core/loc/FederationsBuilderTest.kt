...
class FederationsBuilderTest constructor() {
    @Test
    @Throws(Exception::class)
    fun itConsumesValidJSON() {
        val federationsBuilder: FederationsBuilder? = FederationsBuilder()
        val json: String? = ("{ \"response\": [ " +
                ...
                "} " +
                "] }")
        val federations: MutableList<Federation?>? = federationsBuilder.fromJSON(json)
        MatcherAssert.assertThat(federations.size, Matchers.equalTo(2))
        MatcherAssert.assertThat(federations.get(0).getDeliveryService(), Matchers.equalTo("kable-town-01"))
        MatcherAssert.assertThat(federations.get(0).getFederationMappings(), CoreMatchers.not(IsNull.nullValue()))
    }
}