...
class AbstractResourceWatcherTest constructor() {
    ...
    fun setUp() {
        ...
        var config: JsonNode? = cacheRegister.getConfig()
    ...
    fun tearDown() {
        ...
        MatcherAssert.assertThat(
            ...
            Matchers.endsWith(
                FederationsWatcher.Companion.DEFAULT_FEDERATION_DATA_URL.split("api").toTypedArray().get(1)
            )
        )
    }
    ...
    fun testWatchers() {
        ...
        val cacheRegister: CacheRegister? = trafficRouter.getCacheRegister()
        var config: JsonNode? = cacheRegister.getConfig()
        Assert.assertNull(config.get(federationsWatcher.getWatcherConfigPrefix() + ".polling.url"))
        MatcherAssert.assertThat(
            federationsWatcher.getDataBaseURL(),
            Matchers.endsWith(
                FederationsWatcher.Companion.DEFAULT_FEDERATION_DATA_URL.split("api").toTypedArray().get(1)
        ...
        MatcherAssert.assertThat(
            ...
            Matchers.endsWith(SteeringWatcher.Companion.DEFAULT_STEERING_DATA_URL.split("api").toTypedArray().get(1))
        )
        val newFedsUrl: String? = "https://\${toHostname}/api/3.0/notAFederationsEndpoint"
        ...
        MatcherAssert.assertThat(
            config.get(federationsWatcher.getWatcherConfigPrefix() + ".polling.url").asText(),
    ...
    companion object {
        private val LOGGER: Logger? = Logger.getLogger(AbstractResourceWatcherTest::class.java)
