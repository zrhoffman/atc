...
class DNSRoutingMissesTest constructor() {
    ...
    fun before() {
        val name: Name? = Name.fromString("edge.foo-img.kabletown.com")
        ...
        val federationRegistry: FederationRegistry? = Mockito.mock(
            FederationRegistry::class.java
        )
        Mockito.`when`(
    ...
    fun itSetsDetailsWhenBypassDestination() {
        ...
        Mockito.`when`(deliveryService.isDns()).thenReturn(true)
        Mockito.doReturn(deliveryService).`when`(trafficRouter).selectDeliveryService(request)
        val bypassDestination: JsonNode? = Mockito.mock(
            JsonNode::class.java
        )
        Mockito.`when`(bypassDestination.get("DNS")).thenReturn(null)
    ...
    fun itSetsDetailsWhenCacheNotFoundByGeolocation() {
        ...
        val cacheLocation: CacheLocation? = Mockito.mock(CacheLocation::class.java)
