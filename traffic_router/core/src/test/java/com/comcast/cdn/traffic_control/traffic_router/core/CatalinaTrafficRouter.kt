...
class CatalinaTrafficRouter constructor(serverXmlPath: String?, appBase: String?) {
    ...
    init {
        ...
        val trafficRouterService: StandardService? =
            catalina.getServer().findService("traffic_router_core") as StandardService?
        val secureConnectorList: MutableList<Connector?>? = Arrays.stream(trafficRouterService.findConnectors()).filter(
            Predicate({ k: Connector? -> (k.getAttribute("portAttribute") == "SecureApiPort") })
        ).collect(Collectors.toList())
        val hasHttpsPort: Boolean = secureConnectorList.size > 0
        val securePort: Int = if (hasHttpsPort) secureConnectorList.get(0).getPort() else 0
        val apiPort: Int = Arrays.stream(trafficRouterService.findConnectors())
            .filter(Predicate({ k: Connector? -> (k.getAttribute("portAttribute") == "ApiPort") }))
            .collect(Collectors.toList()).get(0).getPort()
        val connectors: Array<Connector?>? = trafficRouterService.findConnectors()
        for (connector: Connector? in connectors) {
            if (connector.getPort() == 80) {
                connector.setPort(System.getProperty("routerHttpPort", "8888").toInt())
            }
            SocketUtils.findAvailableTcpPort()
            if (connector.getPort() == 443) {
                connector.setPort(System.getProperty("routerSecurePort", "8443").toInt())
            }
            if (connector.getPort() == 3443) {
                connector.setPort(System.getProperty("secureApiPort", "3443").toInt())
            }
            println("[" + System.currentTimeMillis() + "] >>>>>>>>>>>>>>>> Traffic Router listening on port " + connector.getPort() + " " + connector.getScheme())
        }
        println(
        ...
        val standardHost: StandardHost? = trafficRouterService.getContainer().findChild("localhost") as StandardHost?
        standardHost.setAppBase(appBase)

        // We have to manually set up the default servlet, the Catalina class doesn't do this for us
        val rootContext: StandardContext? = standardHost.findChild("") as StandardContext?
        val defaultServlet: Wrapper? = rootContext.createWrapper()
