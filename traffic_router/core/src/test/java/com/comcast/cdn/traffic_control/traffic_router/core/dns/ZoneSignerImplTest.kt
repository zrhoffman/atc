...
class ZoneSignerImplTest constructor() {
    internal class IsRRsetTypeA constructor() : ArgumentMatcher<RRset?>() {
        public override fun matches(rrset: Any?): Boolean {
            return (rrset as RRset?).getType() == Type.A
        }
    }

    internal class IsRRsetTypeNSEC constructor() : ArgumentMatcher<RRset?>() {
        public override fun matches(rrset: Any?): Boolean {
            return (rrset as RRset?).getType() == Type.NSEC
        }
    }
    ...
    fun signZoneWithRRSIGCacheTest() {
        val zoneSigner: ZoneSignerImpl? = PowerMockito.spy(ZoneSignerImpl())
        ...
        val zskDnskey: DNSKEYRecord? = Mockito.mock(DNSKEYRecord::class.java)
        Mockito.`when`(zskPair.getDNSKEYRecord()).thenReturn(zskDnskey)
        val zskKey: PrivateKey? = Mockito.mock(PrivateKey::class.java)
        Mockito.`when`(zskPair.getPrivate()).thenReturn(zskKey)
        Mockito.`when`(zskKey.getEncoded()).thenReturn(byteArrayOf(1))
        Mockito.`when`(zskDnskey.getAlgorithm()).thenReturn(1)
        val kskPairs: MutableList<DnsSecKeyPair?>? = ArrayList()
        val zskPairs: MutableList<DnsSecKeyPair?>? = listOf(zskPair)
        val inception: Date? = Date()
        val expire: Date? = Date.from(inception.toInstant().plusSeconds(100000))
        val aRRSigRecord: RRSIGRecord? = RRSIGRecord(
        ...
        val nsecRRSigRecord: RRSIGRecord? = RRSIGRecord(
            Name("foo.example.com."),
            DClass.IN,
            60,
            Type.NSEC,
        ...
        val newInception: Date? = Date.from(inception.toInstant().plusSeconds(100))
        val newExpire: Date? = Date.from(newInception.toInstant().plusSeconds(100000))
        val newARRSigRecord: RRSIGRecord? = RRSIGRecord(
            Name("foo.example.com."),
            DClass.IN,
            60,
            Type.A,
        ...
        val newNSECRRSigRecord: RRSIGRecord? = RRSIGRecord(
            Name("foo.example.com."),
            DClass.IN,
            60,
            Type.NSEC,
        ...
        val expiresSoonInception: Date? = Date.from(inception.toInstant().minusSeconds(100))
        val expiresSoonExpire: Date? = Date.from(inception.toInstant().plusSeconds(50))
        val expiresSoonARRSigRecord: RRSIGRecord? = RRSIGRecord(
            Name("foo.example.com."),
            DClass.IN,
            60,
            Type.A,
        ...
        val expiresSoonNSECRRSigRecord: RRSIGRecord? = RRSIGRecord(
            Name("foo.example.com."),
            DClass.IN,
            60,
            Type.NSEC,
        ...
        var signedRecords: MutableList<Record?>? =
            zoneSigner.signZone(records, kskPairs, zskPairs, inception, expire, RRSIGCache)
        var ret: RRSIGRecord? = signedRecords.stream()
            .filter(Predicate({ r: Record? -> r is RRSIGRecord && (r as RRSIGRecord?).getTypeCovered() == Type.A }))
