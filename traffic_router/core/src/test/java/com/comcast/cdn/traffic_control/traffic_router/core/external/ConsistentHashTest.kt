...
class ConsistentHashTest constructor() {
    ...
    fun before() {
        ...
        val steeringNode: JsonNode? = objectMapper.readTree(inputStream).get("response").get(0)
        steeringDeliveryServiceId = steeringNode.get("deliveryService").asText()
        val iterator: MutableIterator<JsonNode?>? = steeringNode.get("targets").iterator()
        while (iterator.hasNext()) {
            val target: JsonNode? = iterator.next()
        ...
        var jsonNode: JsonNode? = objectMapper.readTree(inputStream)
        ...
        while (deliveryServices.hasNext() && deliveryServiceId == null) {
            ...
            val dispersionNode: JsonNode? = deliveryServiceNode.get("dispersion")
            if (dispersionNode == null || dispersionNode.get("limit").asInt() != 1 && (dispersionNode.get("shuffled")
                    .asText() == "true")
            ) {
                continue
            }
        ...
        val network: JsonNode? =
            jsonNode.get("coverageZones").get(jsonNode.get("coverageZones").fieldNames().next()).get("network")
        for (i in 0 until network.size()) {
            val cidrString: String? = network.get(i).asText()
            val cidrAddress: CidrAddress? = CidrAddress.Companion.fromString(cidrString)
            if (cidrAddress.getNetmaskLength() == 24) {
                val hostBytes: ByteArray? = cidrAddress.getHostBytes()
                ipAddressInCoverageZone =
                    String.format("%d.%d.%d.123", hostBytes.get(0), hostBytes.get(1), hostBytes.get(2))
                break
            }
        }
    ...
    fun itAppliesConsistentHashingToRequestsForCoverageZone() {
        ...
        try {
            var requestPath: String? = URLEncoder.encode("/some/path/thing", "UTF-8")
            var httpGet: HttpGet? =
            ...
            val objectMapper: ObjectMapper? = ObjectMapper(JsonFactory())
            var cacheNode: JsonNode? = objectMapper.readTree(EntityUtils.toString(response.getEntity()))
            val cacheId: String? = cacheNode.get("id").asText()
            ...
            Assert.assertThat(cacheNode.get("id").asText(), IsEqual.equalTo(cacheId))
            response.close()
            ...
            response = closeableHttpClient.execute(httpGet)
            cacheNode = objectMapper.readTree(EntityUtils.toString(response.getEntity()))
            Assert.assertThat(cacheNode.get("id").asText(), IsNot.not(IsEqual.equalTo(cacheId)))
            Assert.assertThat(cacheNode.get("id").asText(), IsNot.not(IsEqual.equalTo("")))
        } finally {
    ...
    fun itAppliesConsistentHashingToSteeringDeliveryService() {
        var response: CloseableHttpResponse? = null
        try {
            val requestPath: String? = URLEncoder.encode("/some/path/thing", "UTF-8")
            val httpGet: HttpGet? =
            ...
            response = closeableHttpClient.execute(httpGet)
            val objectMapper: ObjectMapper? = ObjectMapper(JsonFactory())
            val deliveryServiceNode: JsonNode? = objectMapper.readTree(EntityUtils.toString(response.getEntity()))
            Assert.assertThat(deliveryServiceNode.get("id").asText(), Matchers.isIn(steeredDeliveryServices))
        } finally {
            if (response != null) response.close()
        }
    }
    ...
    fun itUsesBypassFiltersWithDeliveryServiceSteering() {
        var response: CloseableHttpResponse? = null
        try {
            var requestPath: String? = URLEncoder.encode("/some/path/force-to-target-2/more/asdfasdf", "UTF-8")
            var httpGet: HttpGet? =
                HttpGet("http://localhost:3333/crs/consistenthash/deliveryservice?ip=98.76.54.123&deliveryServiceId=" + steeringDeliveryServiceId + "&requestPath=" + requestPath)
            response = closeableHttpClient.execute(httpGet)
            val objectMapper: ObjectMapper? = ObjectMapper(JsonFactory())
            var deliveryServiceNode: JsonNode? = objectMapper.readTree(EntityUtils.toString(response.getEntity()))
            Assert.assertThat(deliveryServiceNode.get("id").asText(), IsEqual.equalTo("steering-target-2"))
            ...
            Assert.assertThat(deliveryServiceNode.get("id").asText(), IsEqual.equalTo("steering-target-1"))
        } finally {
            if (response != null) response.close()
        }
    }
    ...
    fun itUsesRegexToStandardizeRequestPath() {
        var response: CloseableHttpResponse? = null
        try {
            var requestPath: String? = URLEncoder.encode("/some/path/thing.m3u8", "UTF-8")
            val encodedConsistentHashRegex: String? = URLEncoder.encode(consistentHashRegex, "UTF-8")
            var httpGet: HttpGet? =
            ...
            val objectMapper: ObjectMapper? = ObjectMapper(JsonFactory())
            var resp: JsonNode? = objectMapper.readTree(EntityUtils.toString(response.getEntity()))
            val resultingPathToConsistentHash: String? = resp.get("resultingPathToConsistentHash").asText()
    ...
    fun itAppliesPatternBasedConsistentHashingToSteeringDeliveryService() {
        ...
        try {
            ...
            val objectMapper: ObjectMapper? = ObjectMapper(JsonFactory())
            var deliveryServiceNode: JsonNode? = objectMapper.readTree(EntityUtils.toString(response.getEntity()))
            val deliveryServiceId: String? = deliveryServiceNode.get("id").asText()
            ...
            response = closeableHttpClient.execute(httpGet)
            deliveryServiceNode = objectMapper.readTree(EntityUtils.toString(response.getEntity()))
            Assert.assertThat(deliveryServiceNode.get("id").asText(), IsEqual.equalTo(deliveryServiceId))
        } finally {
            if (response != null) response.close()
        }
    }
