...
class CoverageZoneTest constructor() {
    ...
    fun itGetsCacheLocation() {
        ...
        try {
            ...
            val jsonString: String? = EntityUtils.toString(response.getEntity())
            val objectMapper: ObjectMapper? = ObjectMapper(JsonFactory())
            val jsonNode: JsonNode? = objectMapper.readTree(jsonString)
            Assert.assertThat(jsonNode.get("id").asText(), IsEqual.equalTo("location-3"))
            Assert.assertThat(jsonNode.get("geolocation"), IsNot.not(Matchers.nullValue()))
            Assert.assertThat(jsonNode.get("caches").get(0).get("id").asText(), Matchers.startsWith("edge-cache-03"))
            Assert.assertThat(jsonNode.get("caches").get(0).get("fqdn").asText(), Matchers.startsWith("edge-cache-03"))
            Assert.assertThat(
                jsonNode.get("caches").get(0).get("fqdn").asText(),
                Matchers.endsWith("thecdn.example.com")
            )
            Assert.assertThat(jsonNode.get("caches").get(0).get("port").asInt(), Matchers.greaterThan(1024))
            Assert.assertThat(
                jsonNode.get("caches").get(0).get("hashValues").get(0).asDouble(),
                Matchers.greaterThan(1.0)
            )
            Assert.assertThat(
                isValidIpV4String(jsonNode.get("caches").get(0).get("ip4").asText()),
                IsEqual.equalTo(true)
            )
            Assert.assertThat(jsonNode.get("caches").get(0).get("ip6").asText(), IsNot.not(IsEqual.equalTo("")))
            Assert.assertThat(jsonNode.get("caches").get(0).has("available"), IsEqual.equalTo(true))
    ...
    fun itGetsCaches() {
        ...
        try {
            ...
            val objectMapper: ObjectMapper? = ObjectMapper(JsonFactory())
            val jsonNode: JsonNode? = objectMapper.readTree(EntityUtils.toString(response.getEntity()))
            Assert.assertThat(jsonNode.isArray(), IsEqual.equalTo(true))
            val cacheNode: JsonNode? = jsonNode.get(0)
            Assert.assertThat(cacheNode.get("id").asText(), IsNot.not(Matchers.nullValue()))
            Assert.assertThat(cacheNode.get("fqdn").asText(), IsNot.not(Matchers.nullValue()))
            Assert.assertThat(cacheNode.get("ip4").asText(), IsNot.not(Matchers.nullValue()))
            Assert.assertThat(cacheNode.get("ip6").asText(), IsNot.not(Matchers.nullValue()))
            // If the value is null or otherwise not an int we'll get back -123456, so any other value returned means success
            Assert.assertThat(cacheNode.get("port").asInt(-123456), IsNot.not(IsEqual.equalTo(-123456)))
            Assert.assertThat(cacheNode.get("deliveryServices").isArray(), IsEqual.equalTo(true))
            Assert.assertThat(
                cacheNode.get("hashValues").get(0).asDouble(-1024.1024),
                IsNot.not(IsEqual.equalTo(-1024.1024))
            )
            Assert.assertThat(
                cacheNode.get("available").asText(),
    ...
    fun itReturns404ForMissingDeliveryService() {
        ...
        closeableHttpClient.execute(httpGet)
            .use({ response -> Assert.assertThat(response.getStatusLine().getStatusCode(), IsEqual.equalTo(404)) })
    }

    fun isValidIpV4String(ip: String?): Boolean {
        val octets: Array<String?>? = ip.split("\\.").toTypedArray()
        if (octets.size != 4) {
            return false
        }
        for (octet: String? in octets) {
            val b: Int = octet.toInt()
            if (b < 0 || 255 < b) {
                return false
            }
        }
