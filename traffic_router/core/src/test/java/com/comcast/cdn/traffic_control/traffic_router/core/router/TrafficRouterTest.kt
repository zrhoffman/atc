...
class TrafficRouterTest constructor() {
    ...
    fun before() {
        ...
        val inetRecord: InetRecord? = InetRecord("cname1", 12345)
    ...
    fun itCreatesDnsResultsFromFederationMappingHit() {
        val name: Name? = Name.fromString("edge.example.com")
        val request: DNSRequest? = DNSRequest("example.com", name, Type.A)
        request.setClientIP("192.168.10.11")
        request.setHostname(name.relativize(Name.root).toString())
        val track: StatTracker.Track? = Mockito.spy(StatTracker.Companion.getTrack())
        Mockito.`when`(deliveryService.getRoutingName()).thenReturn("edge")
        Mockito.`when`(deliveryService.isDns()).thenReturn(true)
        val result: DNSRouteResult? = trafficRouter.route(request, track)
    ...
    fun itCreatesHttpResults() {
        ...
        val track: StatTracker.Track? = Mockito.spy(StatTracker.Companion.getTrack())
        val cache: Cache? = Mockito.mock(
            Cache::class.java
        )
        Mockito.`when`(cache.hasDeliveryService(Matchers.anyString())).thenReturn(true)
        val cacheLocation: CacheLocation? = CacheLocation("", Geolocation(50, 50))
        ...
        val cacheRegister: CacheRegister? = Mockito.mock(CacheRegister::class.java)
        Mockito.`when`(cacheRegister.getCacheLocations()).thenReturn(cacheLocationCollection)
        ...
        val httpRouteResult: HTTPRouteResult? = trafficRouter.route(httpRequest, track)
        MatcherAssert.assertThat(
            httpRouteResult.getUrl().toString(),
    ...
    fun itFiltersByIPAvailability() {
        ...
        val cacheIPv4: Cache? = Mockito.mock(
            Cache::class.java
        )
        ...
        Mockito.`when`(cacheIPv4.getId()).thenReturn("cache IPv4")
        val cacheIPv6: Cache? = Mockito.mock(
            Cache::class.java
        )
        ...
        Mockito.`when`(cacheIPv6.getId()).thenReturn("cache IPv6")
        val caches: MutableList<Cache?>? = ArrayList()
        ...
        val supportingIPv4Caches: MutableList<Cache?>? =
            trafficRouter.getSupportingCaches(caches, ds, IPVersions.IPV4ONLY)
        MatcherAssert.assertThat(supportingIPv4Caches.size, org.hamcrest.Matchers.equalTo(1))
        MatcherAssert.assertThat(supportingIPv4Caches.get(0).getId(), org.hamcrest.Matchers.equalTo("cache IPv4"))
        val supportingIPv6Caches: MutableList<Cache?>? =
            trafficRouter.getSupportingCaches(caches, ds, IPVersions.IPV6ONLY)
        MatcherAssert.assertThat(supportingIPv6Caches.size, org.hamcrest.Matchers.equalTo(1))
        MatcherAssert.assertThat(supportingIPv6Caches.get(0).getId(), org.hamcrest.Matchers.equalTo("cache IPv6"))
        val supportingEitherCaches: MutableList<Cache?>? = trafficRouter.getSupportingCaches(caches, ds, IPVersions.ANY)
        MatcherAssert.assertThat(supportingEitherCaches.size, org.hamcrest.Matchers.equalTo(2))
        cacheIPv6.setIsAvailable(false)
        val supportingAvailableCaches: MutableList<Cache?>? =
            trafficRouter.getSupportingCaches(caches, ds, IPVersions.ANY)
        MatcherAssert.assertThat(supportingAvailableCaches.size, org.hamcrest.Matchers.equalTo(1))
        MatcherAssert.assertThat(supportingAvailableCaches.get(0).getId(), org.hamcrest.Matchers.equalTo("cache IPv4"))
    }

    @Test
    @Throws(Exception::class)
    fun itChecksDefaultLocation() {
        val ip: String? = "1.2.3.4"
        val track: StatTracker.Track? = StatTracker.Track()
        val geolocation: Geolocation? = Mockito.mock(Geolocation::class.java)
        ...
        Mockito.`when`(geolocation.getCountryCode()).thenReturn("US")
        val map: MutableMap<String?, Geolocation?>? = HashMap()
        val defaultUSLocation: Geolocation? = Geolocation(37.751, -97.822)
        ...
        MatcherAssert.assertThat(result.get(0), org.hamcrest.Matchers.equalTo(cache))
    ...
    fun itChecksMissLocation() {
        var defaultUSLocation: Geolocation? = Geolocation(37.751, -97.822)
        Mockito.`when`(deliveryService.getMissLocation()).thenReturn(defaultUSLocation)
        Mockito.`when`(trafficRouter.isValidMissLocation(deliveryService)).thenCallRealMethod()
        var result: Boolean = trafficRouter.isValidMissLocation(deliveryService)
    ...
    fun itSetsResultToGeo() {
        ...
        val clientLocation: Geolocation? = Geolocation(40, -100)
        Mockito.`when`(
            trafficRouter.getClientLocation(
                Matchers.anyString(), Matchers.any(
                    DeliveryService::class.java
        ...
        var track: StatTracker.Track? = Mockito.spy(StatTracker.Companion.getTrack())
        ...
        Mockito.`when`(deliveryService.isDns()).thenReturn(true)
        val name: Name? = Name.fromString("edge.example.com")
        val dnsRequest: DNSRequest? = DNSRequest("example.com", name, Type.A)
    ...
    fun itRetainsPathElementsInURI() {
        ...
        Mockito.`when`(cache.getFqdn()).thenReturn("atscache-01.kabletown.net")
        Mockito.`when`(cache.getPort()).thenReturn(80)
        Mockito.`when`(
            deliveryService.createURIString(
                Matchers.any(
                    HTTPRequest::class.java
        ...
        val dest: StringBuilder? = StringBuilder()
        dest.append("http://")
        dest.append(cache.getFqdn().split("\\.", 2.toBoolean()).toTypedArray().get(0))
        dest.append(".")
        dest.append(httpRequest.getHostname().split("\\.", 2.toBoolean()).toTypedArray().get(1))
