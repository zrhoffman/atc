...
class MaxmindGeolocationServiceTest constructor() {
    ...
    fun itReturnsALocationWhenTheDatabaseIsLoaded() {
        val databaseFile: File? = Mockito.mock(File::class.java)
        Mockito.`when`(databaseFile.exists()).thenReturn(true)
        val location: Location? = PowerMockito.mock(
            Location::class.java
        )
        Mockito.`when`(location.getLatitude()).thenReturn(40.0)
        Mockito.`when`(location.getLongitude()).thenReturn(-105.0)
        val cityResponse: CityResponse? = PowerMockito.mock(CityResponse::class.java)
        Mockito.`when`(cityResponse.getLocation()).thenReturn(location)
        val databaseReader: DatabaseReader? = Mockito.mock(DatabaseReader::class.java)
        Mockito.`when`(databaseReader.city(InetAddress.getByName("192.168.99.100"))).thenReturn(cityResponse)
        val builder: DatabaseReader.Builder? = Mockito.mock(DatabaseReader.Builder::class.java)
