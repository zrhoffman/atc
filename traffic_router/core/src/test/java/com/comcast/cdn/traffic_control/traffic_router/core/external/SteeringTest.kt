...
class SteeringTest constructor() {
    ...
    var routerHttpPort: String? = System.getProperty("routerHttpPort", "8888")
    var testHttpPort: String? = System.getProperty("testHttpServerPort", "8889")
    @Throws(IOException::class)
    fun getJsonForResourcePath(resourcePath: String?): JsonNode? {
        val objectMapper: ObjectMapper? = ObjectMapper(JsonFactory())
        val inputStream: InputStream? = javaClass.getClassLoader().getResourceAsStream(resourcePath)
        if (inputStream == null) {
    ...
    fun setupSteering(
        ...
        domains.clear()
        weights.clear()
        val steeringNode: JsonNode? = getJsonForResourcePath(resourcePath)
        val steeredDeliveryServices: MutableIterator<JsonNode?>? = steeringNode.get("targets").iterator()
        while (steeredDeliveryServices.hasNext()) {
            val steeredDeliveryService: JsonNode? = steeredDeliveryServices.next()
            val targetId: String? = steeredDeliveryService.get("deliveryService").asText()
            val targetWeight: Int? = steeredDeliveryService.get("weight").asInt()
    ...
    @Throws(IOException::class)
    fun setupCrConfig() {
        val resourcePath: String? = "publish/CrConfig.json"
        val inputStream: InputStream? = javaClass.getClassLoader().getResourceAsStream(resourcePath)
        if (inputStream == null) {
            Assert.fail("Could not find file '" + resourcePath + "' needed for test from the current classpath as a resource!")
        }
        val jsonNode: JsonNode? = ObjectMapper(JsonFactory()).readTree(inputStream)
        val deliveryServices: MutableIterator<String?>? = jsonNode.get("deliveryServices").fieldNames()
        while (deliveryServices.hasNext()) {
            val dsId: String? = deliveryServices.next()
        ...
        for (deliveryServiceId: String? in targetDomains.keys) {
            val cacheIds: MutableIterator<String?>? = jsonNode.get("contentServers").fieldNames()
            while (cacheIds.hasNext()) {
                val cacheId: String? = cacheIds.next()
                val cacheNode: JsonNode? = jsonNode.get("contentServers").get(cacheId)
                if (!cacheNode.has("deliveryServices")) {
                    continue
                }
                if (cacheNode.get("deliveryServices").has(deliveryServiceId)) {
                    val port: Int = cacheNode.get("port").asInt()
                    val portText: String? = if ((port == 80)) "" else ":" + port
    ...
    fun itUsesSteeredDeliveryServiceIdInRedirect() {
        val httpGet: HttpGet? = HttpGet("http://localhost:" + routerHttpPort + "/stuff?fakeClientIpAddress=12.34.56.78")
        httpGet.addHeader("Host", "foo." + steeringDeliveryServiceId + ".bar")
        var response: CloseableHttpResponse? = null
        try {
            response = httpClient.execute(httpGet)
            Assert.assertThat(
                "Failed getting 302 for request " + httpGet.getFirstHeader("Host").getValue(),
                response.getStatusLine().getStatusCode(),
                IsEqual.equalTo(302)
            )
            Assert.assertThat(response.getFirstHeader("Location").getValue(), Matchers.isIn(validLocations))
    ...
    fun itReturns503ForBadDeliveryServiceInXtcSteeringOption() {
        val httpGet: HttpGet? =
            HttpGet("http://localhost:" + routerHttpPort + "/qwerytuiop/asdfghjkl?fakeClientIpAddress=12.34.56.78")
        httpGet.addHeader("Host", "foo.steering-test-1.thecdn.example.com")
        httpGet.addHeader("X-TC-Steering-Option", "ds-02")
        var response: CloseableHttpResponse? = null
        try {
            response = httpClient.execute(httpGet)
            Assert.assertThat(response.getStatusLine().getStatusCode(), IsEqual.equalTo(503))
        } finally {
            if (response != null) {
                response.close()
            }
    ...
    fun itUsesWeightedDistributionForRequestPath() {
        var count: Int = 0
        for (weight: Int in targetWeights.values) {
            count += weight
        }
        count *= 1000
        if (count > 100000) {
        ...
        for (steeredId: String? in targetWeights.keys) {
        ...
        for (i in 0 until count) {
            val path: String? = generateRandomPath()
            val httpGet: HttpGet? =
                HttpGet("http://localhost:" + routerHttpPort + path + "?fakeClientIpAddress=12.34.56.78")
            httpGet.addHeader("Host", "foo." + steeringDeliveryServiceId + ".bar")
            var response: CloseableHttpResponse? = null
            try {
                response = httpClient.execute(httpGet)
                Assert.assertThat(
                    "Did not get 302 for request '" + httpGet.getURI() + "'",
                    response.getStatusLine().getStatusCode(),
                    IsEqual.equalTo(302)
                )
                val location: String? = response.getFirstHeader("Location").getValue()
                for (id: String? in results.keys) {
        ...
        for (id: String? in targetWeights.keys) {
            expectedHitRates.put(id, targetWeights.get(id) / totalWeight)
        }
        for (id: String? in results.keys) {
            val hits: Int = results.get(id)
            val hitRate: Double = hits as Double / count
            Assert.assertThat(hitRate, IsCloseTo.closeTo(expectedHitRates.get(id), 0.009))
        }
    }
    ...
    fun z_itemsMigrateFromSmallerToLargerBucket() {
        ...
        for (target: String? in weights.keys) {
        ...
        hashedPaths.put(smallerTarget, ArrayList())
        hashedPaths.put(largerTarget, ArrayList())
        for (path: String? in randomPaths) {
            val httpGet: HttpGet? =
                HttpGet("http://localhost:" + routerHttpPort + path + "?fakeClientIpAddress=12.34.56.78")
            httpGet.addHeader("Host", "foo." + steeringDeliveryServiceId + ".bar")
            var response: CloseableHttpResponse? = null
            try {
                ...
                val location: String? = response.getFirstHeader("Location").getValue()
                for (targetXmlId: String? in hashedPaths.keys) {
        ...
        val httpPost: HttpPost? = HttpPost("http://localhost:" + testHttpPort + "/steering")
        ...
        rehashedPaths.put(smallerTarget, ArrayList())
        rehashedPaths.put(largerTarget, ArrayList())
        for (path: String? in randomPaths) {
            val httpGet: HttpGet? =
                HttpGet("http://localhost:" + routerHttpPort + path + "?fakeClientIpAddress=12.34.56.78")
            httpGet.addHeader("Host", "foo." + steeringDeliveryServiceId + ".bar")
            ...
            try {
                ...
                val location: String? = response.getFirstHeader("Location").getValue()
                for (targetXmlId: String? in rehashedPaths.keys) {
                    if (location.contains(targetXmlId)) {
        ...
        Assert.assertThat(
            rehashedPaths.get(smallerTarget).size,
            Matchers.greaterThan(hashedPaths.get(smallerTarget).size)
        )
        Assert.assertThat(rehashedPaths.get(largerTarget).size, Matchers.lessThan(hashedPaths.get(largerTarget).size))
        for (path: String? in hashedPaths.get(smallerTarget)) {
            Assert.assertThat(rehashedPaths.get(smallerTarget).contains(path), IsEqual.equalTo(true))
            Assert.assertThat(rehashedPaths.get(largerTarget).contains(path), IsEqual.equalTo(false))
        }
    }
    ...
    fun itUsesMultiLocationFormatResponse() {
        ...
        for (path: String? in paths) {
            val httpGet: HttpGet? = HttpGet("http://localhost:" + routerHttpPort + path)
            httpGet.addHeader("Host", "tr.client-steering-test-1.thecdn.example.com")
            var response: CloseableHttpResponse? = null
            try {
                response = httpClient.execute(httpGet)
                val location1: String? = ".client-steering-target-2.thecdn.example.com:8090" + path
                val location2: String? = ".client-steering-target-1.thecdn.example.com:8090" + path
                Assert.assertThat(
                    "Failed getting 302 for request " + httpGet.getFirstHeader("Host").getValue(),
                    response.getStatusLine().getStatusCode(),
                    IsEqual.equalTo(302)
                )
                Assert.assertThat(response.getFirstHeader("Location").getValue(), Matchers.endsWith(location1))
                val entity: HttpEntity? = response.getEntity()
                val objectMapper: ObjectMapper? = ObjectMapper(JsonFactory())
                Assert.assertThat(entity.getContent(), IsNot.not(Matchers.nullValue()))
                val json: JsonNode? = objectMapper.readTree(entity.getContent())
                Assert.assertThat(json.has("locations"), IsEqual.equalTo(true))
                Assert.assertThat(json.get("locations").size(), IsEqual.equalTo(2))
                Assert.assertThat(
                    json.get("locations").get(0).asText(),
                    IsEqual.equalTo(response.getFirstHeader("Location").getValue())
                )
                Assert.assertThat(json.get("locations").get(1).asText(), Matchers.endsWith(location2))
            } finally {
                if (response != null) {
                    response.close()
                }
    ...
    fun itUsesMultiLocationFormatResponseWithout302() {
        ...
        for (path: String? in paths) {
            ...
            try {
                ...
                val location2: String? = ".client-steering-target-1.thecdn.example.com:8090" + path
                Assert.assertThat(
                    "Failed getting 200 for request " + httpGet.getFirstHeader("Host").getValue(),
                    response.getStatusLine().getStatusCode(),
                    IsEqual.equalTo(200)
                )
                val entity: HttpEntity? = response.getEntity()
                ...
                Assert.assertThat(json.has("locations"), IsEqual.equalTo(true))
                Assert.assertThat(json.get("locations").size(), IsEqual.equalTo(2))
                Assert.assertThat(json.get("locations").get(0).asText(), Matchers.endsWith(location1))
                Assert.assertThat(json.get("locations").get(1).asText(), Matchers.endsWith(location2))
            } finally {
                if (response != null) {
                    response.close()
    ...
    fun itUsesNoMultiLocationFormatResponseWithout302WithHead() {
        ...
        paths.add("/qwerytuiop/asdfghjkl?fakeClientIpAddress=12.34.56.78&" + RouterFilter.Companion.REDIRECT_QUERY_PARAM + "=FalsE")
        for (path: String? in paths) {
            val httpHead: HttpHead? = HttpHead("http://localhost:" + routerHttpPort + path)
            httpHead.addHeader("Host", "tr.client-steering-test-1.thecdn.example.com")
            var response: CloseableHttpResponse? = null
            try {
                response = httpClient.execute(httpHead)
                Assert.assertThat(
                    "Failed getting 200 for request " + httpHead.getFirstHeader("Host").getValue(),
                    response.getStatusLine().getStatusCode(),
                    IsEqual.equalTo(200)
                )
                Assert.assertThat(
                    "Failed getting null body for HEAD request",
                    response.getEntity(),
                    Matchers.nullValue()
                )
            } finally {
    ...
    fun itUsesNoMultiLocationFormatResponseWithHead() {
        ...
        for (path: String? in paths) {
            ...
            try {
                response = httpClient.execute(httpHead)
                val location: String? = ".client-steering-target-2.thecdn.example.com:8090" + path
                Assert.assertThat(
                    "Failed getting 302 for request " + httpHead.getFirstHeader("Host").getValue(),
                    response.getStatusLine().getStatusCode(),
                    IsEqual.equalTo(302)
                )
                Assert.assertThat(response.getFirstHeader("Location").getValue(), Matchers.endsWith(location))
                Assert.assertThat(
                    "Failed getting null body for HEAD request",
                    response.getEntity(),
                    Matchers.nullValue()
    ...
    fun itUsesMultiLocationFormatWithMoreThanTwoEntries() {
        val path: String? = "/qwerytuiop/asdfghjkl?fakeClientIpAddress=12.34.56.78"
        val httpGet: HttpGet? = HttpGet("http://localhost:" + routerHttpPort + path)
        httpGet.addHeader("Host", "tr.client-steering-test-2.thecdn.example.com")
        var response: CloseableHttpResponse? = null
        try {
            response = httpClient.execute(httpGet)
            val location1: String? = ".steering-target-2.thecdn.example.com:8090" + path
            val location2: String? = ".steering-target-1.thecdn.example.com:8090" + path
            val location3: String? = ".client-steering-target-2.thecdn.example.com:8090" + path
            val location4: String? = ".client-steering-target-4.thecdn.example.com:8090" + path
            val location5: String? = ".client-steering-target-3.thecdn.example.com:8090" + path
            val location6: String? = ".client-steering-target-1.thecdn.example.com:8090" + path
            val location7: String? = ".steering-target-4.thecdn.example.com:8090" + path
            val location8: String? = ".steering-target-3.thecdn.example.com:8090" + path
            val entity: HttpEntity? = response.getEntity()
            Assert.assertThat(
                "Failed getting 302 for request " + httpGet.getFirstHeader("Host").getValue(),
                response.getStatusLine().getStatusCode(),
            ...
            val json: JsonNode? = objectMapper.readTree(entity.getContent())
            Assert.assertThat(json.has("locations"), IsEqual.equalTo(true))
            Assert.assertThat(json.get("locations").size(), IsEqual.equalTo(8))
            Assert.assertThat(
                json.get("locations").get(0).asText(),
                IsEqual.equalTo(response.getFirstHeader("Location").getValue())
            )
            Assert.assertThat(json.get("locations").get(1).asText(), Matchers.endsWith(location2))
            Assert.assertThat(json.get("locations").get(2).asText(), Matchers.endsWith(location3))
            Assert.assertThat(json.get("locations").get(3).asText(), Matchers.endsWith(location4))
            Assert.assertThat(json.get("locations").get(4).asText(), Matchers.endsWith(location5))
            Assert.assertThat(json.get("locations").get(5).asText(), Matchers.endsWith(location6))
            Assert.assertThat(json.get("locations").get(6).asText(), Matchers.endsWith(location7))
            Assert.assertThat(json.get("locations").get(7).asText(), Matchers.endsWith(location8))
        } finally {
            if (response != null) {
                response.close()
            }
    ...
    fun itSupportsClientSteeringDiversity() {
        val path: String? =
            "/foo?fakeClientIpAddress=192.168.42.10" // this IP should get a DEEP_CZ hit (via dczmap.json)
        val httpGet: HttpGet? = HttpGet("http://localhost:" + routerHttpPort + path)
        ...
        try {
            ...
            val json: JsonNode? = objectMapper.readTree(entity.getContent())
            Assert.assertThat(json.has("locations"), IsEqual.equalTo(true))
            Assert.assertThat(json.get("locations").size(), IsEqual.equalTo(5))
            val actualEdgesList: MutableList<String?>? = ArrayList()
            val actualTargets: MutableSet<String?>? = HashSet()
            for (n: JsonNode? in json.get("locations")) {
                var l: String? = n.asText()
                l = l.replaceFirst("http://".toRegex(), "")
                val parts: Array<String?>? = l.split("\\.").toTypedArray()
                actualEdgesList.add(parts.get(0))
                actualTargets.add(parts.get(1))
            }
            ...
            Assert.assertThat(actualEdgesList.get(0), Matchers.isIn(deepEdges))
            Assert.assertThat(actualEdgesList.get(1), Matchers.isIn(deepEdges))
            Assert.assertThat(actualEdgesList.get(2), IsEqual.equalTo("edge-cache-csd-3"))
            Assert.assertThat(actualEdgesList.get(3), Matchers.isIn(allEdges))
            Assert.assertThat(actualEdgesList.get(4), Matchers.isIn(allEdges))

            // assert that all 5 steering targets are included in the response
            val expectedTargetsArray: Array<String?>? =
