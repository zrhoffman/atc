...
class TCPTest constructor() {
    ...
    fun setUp() {
        val serverSocket: ServerSocket? = Mockito.mock(ServerSocket::class.java)
    ...
    @Test
    fun testSubmit() {
        val r: SocketHandler? = Mockito.mock(
    ...
    fun testTCPSocketHandler() {
        client = InetAddress.getLocalHost()
        val handler: TCPSocketHandler? = tcp.TCPSocketHandler(socket)
        val name: Name? = Name.fromString("www.foo.bar.")
        val question: Record? = Record.newRecord(name, Type.A, DClass.IN)
        val request: Message? = Message.newQuery(question)
        val wireRequest: ByteArray? = request.toWire()
        val requestOut: ByteArrayOutputStream? = ByteArrayOutputStream()
        val dos: DataOutputStream? = DataOutputStream(requestOut)
    ...
    fun testTCPSocketHandlerBadMessage() {
        val client: InetAddress? = InetAddress.getLocalHost()
        val handler: TCPSocketHandler? = tcp.TCPSocketHandler(socket)
        val wireRequest: ByteArray? = ByteArray(0)
        val baos: ByteArrayOutputStream? = ByteArrayOutputStream()
        val dos: DataOutputStream? = DataOutputStream(baos)
        dos.writeShort(wireRequest.size)
        dos.write(wireRequest)
    ...
    fun testTCPSocketHandlerQueryFail() {
        ...
        val serverFail: ByteArray? = response.toWire()
        val expectedResponseOut: ByteArrayOutputStream? = ByteArrayOutputStream()
        val dos2: DataOutputStream? = DataOutputStream(expectedResponseOut)
        dos2.writeShort(serverFail.size)
        dos2.write(serverFail)
        val responseOut: ByteArrayOutputStream? = ByteArrayOutputStream()
        ...
        val expected: ByteArray? = expectedResponseOut.toByteArray()
        val actual: ByteArray? = responseOut.toByteArray()
