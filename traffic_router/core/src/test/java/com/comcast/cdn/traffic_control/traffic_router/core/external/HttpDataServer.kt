...
class HttpDataServer constructor(private val testHttpServerPort: Int) : HttpHandler {
    private var httpServer: HttpServer? = null
    private var receivedSteeringPost: Boolean = false
    private var receivedCertificatesPost: Boolean = false
    private var receivedCrConfig2Post: Boolean = false
    private var receivedCrConfig3Post: Boolean = false
    private var receivedCrConfig4Post: Boolean = false
    ...
    @Throws(IOException::class)
    public override fun handle(httpExchange: HttpExchange?) {
        Thread(object : Runnable {
            public override fun run() {
                if (("POST" == httpExchange.getRequestMethod())) {
                    if (!receivedSteeringPost && ("/steering" == httpExchange.getRequestURI().getPath())) {
                        receivedSteeringPost = true
                    }
                    if (!receivedCertificatesPost && ("/certificates" == httpExchange.getRequestURI().getPath())) {
                        receivedCertificatesPost = true
                    }
                    if (!receivedCrConfig2Post && ("/crconfig-2" == httpExchange.getRequestURI().getPath())) {
                        receivedCrConfig2Post = true
                        receivedCrConfig3Post = false
                        receivedCrConfig4Post = false
                    }
                    if (!receivedCrConfig3Post && ("/crconfig-3" == httpExchange.getRequestURI().getPath())) {
                        receivedCrConfig2Post = false
                        receivedCrConfig3Post = true
                        receivedCrConfig4Post = false
                    }
                    if (!receivedCrConfig4Post && ("/crconfig-4" == httpExchange.getRequestURI().getPath())) {
                        receivedCrConfig2Post = false
                        receivedCrConfig3Post = false
                ...
                val uri: URI? = httpExchange.getRequestURI()
                ...
                if (("api/2.0/user/login" == path)) {
                    try {
                        ...
                        headers.set("Set-Cookie", HttpCookie("mojolicious", "fake-cookie").toString())
                        httpExchange.sendResponseHeaders(200, 0)
                    } catch (e: Exception) {
                ...
                val inputStream: InputStream? = javaClass.getClassLoader().getResourceAsStream(path)
                if (inputStream == null) {
                    println(">>> " + path + " not found")
                    val response: String? = "404 (Not Found)\n"
                ...
                if (!path.contains("CrConfig")) {
                    try {
                        httpExchange.getResponseBody().use({ os ->
                            httpExchange.sendResponseHeaders(200, 0)
                            val buffer: ByteArray? = ByteArray(0x10000)
                            var count: Int
                    ...
                    try {
                        ...
                        var body: String? = stringBuilder.toString()
