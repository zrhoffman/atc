...
class AbstractProtocolTest constructor() {
    ...
    fun itLogsARecordQueries() {
        ...
        val queryBytes: ByteArray? = query.toWire()
        ...
        val answers: Array<Record?>? = arrayOf(answer)
        val response: Message? = Mockito.mock(Message::class.java)
        PowerMockito.`when`(response.getHeader()).thenReturn(header)
        PowerMockito.`when`(response.getSectionArray(Section.ANSWER)).thenReturn(answers)
        PowerMockito.`when`(response.getQuestion()).thenReturn(question)
        val client: InetAddress? = Inet4Address.getByName("192.168.23.45")
        PowerMockito.`when`(
            nameServer.query(
                Matchers.any(
                    Message::class.java
        ...
        val abstractProtocol: FakeAbstractProtocol? = FakeAbstractProtocol(client, queryBytes)
    ...
    fun itLogsOtherQueries() {
        ...
        val name: Name? = Name.fromString("John Wayne.")
        ...
        val abstractProtocol: FakeAbstractProtocol? = FakeAbstractProtocol(client, query.toWire())
        abstractProtocol.setNameServer(nameServer)
        abstractProtocol.run()
        Mockito.verify(accessLogger)
    ...
    fun itLogsBadClientRequests() {
        val abstractProtocol: FakeAbstractProtocol? = FakeAbstractProtocol(client, byteArrayOf(1, 2, 3, 4, 5, 6, 7))
        abstractProtocol.setNameServer(nameServer)
        abstractProtocol.run()
        Mockito.verify(accessLogger)
    ...
    inner class FakeAbstractProtocol constructor(
    ...
    companion object {
        private val accessLogger: Logger? = Mockito.mock(Logger::class.java)
    }
}