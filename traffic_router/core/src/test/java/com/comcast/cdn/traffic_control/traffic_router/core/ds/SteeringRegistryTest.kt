...
class SteeringRegistryTest constructor() {
    @Test
    ...
    fun itConsumesValidJson() {
        val json: String? = ("{ \"response\": [ " +
                ...
                "}" +
                "] }")
        val steeringRegistry: SteeringRegistry? = SteeringRegistry()
        ...
        val steeringTarget1: SteeringTarget? = SteeringTarget()
        steeringTarget1.setDeliveryService("steering-target-01")
        steeringTarget1.setWeight(9876)
        val steeringTarget2: SteeringTarget? = SteeringTarget()
        steeringTarget2.setDeliveryService("steering-target-02")
        steeringTarget2.setWeight(12345)
        MatcherAssert.assertThat(
            steeringRegistry.get("steering-1").getTargets(),
            Matchers.containsInAnyOrder(steeringTarget1, steeringTarget2)
        )
        MatcherAssert.assertThat(
            steeringRegistry.get("steering-2").getTargets().get(1).getDeliveryService(),
            Matchers.equalTo("steering-target-02")
        )
        MatcherAssert.assertThat(
            steeringRegistry.get("steering-1").getFilters().get(0).getPattern(),
            Matchers.equalTo(".*/force-to-one/.*")
        )
        MatcherAssert.assertThat(
            steeringRegistry.get("steering-1").getFilters().get(0).getDeliveryService(),
            Matchers.equalTo("steering-target-01")
        )
        MatcherAssert.assertThat(
            steeringRegistry.get("steering-1").getFilters().get(1).getPattern(),
            Matchers.equalTo(".*/also-this/.*")
        )
        MatcherAssert.assertThat(
            steeringRegistry.get("steering-1").getFilters().get(1).getDeliveryService(),
            Matchers.equalTo("steering-target-01")
        )
        MatcherAssert.assertThat(
            steeringRegistry.get("steering-1").getBypassDestination("/stuff/force-to-one/more/stuff"),
            Matchers.equalTo("steering-target-01")
        )
        MatcherAssert.assertThat(
            steeringRegistry.get("steering-1").getBypassDestination("/should/not/match"),
            Matchers.nullValue()
        )
        MatcherAssert.assertThat(steeringRegistry.get("steering-2").getFilters().isEmpty(), Matchers.equalTo(true))
    }
}