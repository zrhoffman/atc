...
class RouterTest constructor() {
    ...
    private val routerHttpPort: String? = System.getProperty("routerHttpPort", "8888")
    private val routerSecurePort: String? = System.getProperty("routerSecurePort", "8443")
    private val testHttpPort: String? = System.getProperty("testHttpServerPort", "8889")
    private var trustStore: KeyStore? = null
    private val routerDnsPort: Int = Integer.valueOf(System.getProperty("dns.udp.port", "1053"))
    @Before
    @Throws(Exception::class)
    fun before() {
        val objectMapper: ObjectMapper? = ObjectMapper(JsonFactory())
        var resourcePath: String? = "api/2.0/steering"
        var inputStream: InputStream? = javaClass.getClassLoader().getResourceAsStream(resourcePath)
        if (inputStream == null) {
        ...
        val steeringData: JsonNode? = objectMapper.readTree(inputStream).get("response")
        val elements: MutableIterator<JsonNode?>? = steeringData.elements()
        while (elements.hasNext()) {
            val ds: JsonNode? = elements.next()
            val dsId: String? = ds.get("deliveryService").asText()
            steeringDeliveryServices.add(dsId)
        }
        resourcePath = "publish/CrConfig.json"
        inputStream = javaClass.getClassLoader().getResourceAsStream(resourcePath)
        if (inputStream == null) {
            Assert.fail("Could not find file '" + resourcePath + "' needed for test from the current classpath as a resource!")
        }
        ...
        while (deliveryServices.hasNext()) {
            ...
            val deliveryServiceNode: JsonNode? = jsonNode.get("deliveryServices").get(dsId)
            val matchsets: MutableIterator<JsonNode?>? = deliveryServiceNode.get("matchsets").iterator()
            while (matchsets.hasNext() && deliveryServiceId == null) {
                if (("HTTP" == matchsets.next().get("protocol").asText())) {
        ...
        while (cacheIds.hasNext()) {
            ...
            if (cacheNode.get("deliveryServices").has(httpsOnlyId)) {
                val port: Int = if (cacheNode.has("httpsPort")) cacheNode.get("httpsPort").asInt(443) else 443
                val portText: String? = if ((port == 443)) "" else ":" + port
            ...
            if (cacheNode.get("deliveryServices").has(httpAndHttpsId)) {
                var port: Int = if (cacheNode.has("httpsPort")) cacheNode.get("httpsPort").asInt(443) else 443
                var portText: String? = if ((port == 443)) "" else ":" + port
                httpAndHttpsLocations.add("https://" + cacheId + "." + httpAndHttpsDomain + portText + "/stuff?fakeClientIpAddress=12.34.56.78")
                port = if (cacheNode.has("port")) cacheNode.get("port").asInt(80) else 80
            ...
            if (cacheNode.get("deliveryServices").has(httpOnlyId)) {
                val port: Int = if (cacheNode.has("port")) cacheNode.get("port").asInt(80) else 80
                val portText: String? = if ((port == 80)) "" else ":" + port
        ...
        val keystoreStream: InputStream? = javaClass.getClassLoader().getResourceAsStream("keystore.jks")
    ...
    fun itAUsesEdgeTrafficRoutersForHttpRouting() {
        ...
        val resolver: SimpleResolver? = SimpleResolver("localhost")
        resolver.setPort(routerDnsPort)
        for (type: Int in Arrays.asList(Type.A, Type.AAAA)) {
            val lookup: Lookup? = Lookup(Name("tr.http-only-test.thecdn.example.com."), type)
            lookup.setResolver(resolver)
            lookup.run()
            Assert.assertThat(lookup.getResult(), IsEqual.equalTo(Lookup.SUCCESSFUL))
            Assert.assertThat(lookup.getAnswers().size, IsEqual.equalTo(4))
            for (record: Record? in lookup.getAnswers()) {
    ...
    fun itRedirectsValidHttpRequests() {
        ...
        try {
            ...
            Assert.assertThat(response.getStatusLine().getStatusCode(), IsEqual.equalTo(302))
            val header: Header? = response.getFirstHeader("Location")
            Assert.assertThat(header.getValue(), Matchers.isIn(validLocations))
            Assert.assertThat(header.getValue(), Matchers.startsWith("http://"))
        } finally {
    ...
    fun itDoesRoutingThroughPathsStartingWithCrs() {
        val httpGet: HttpGet? =
            HttpGet("http://localhost:" + routerHttpPort + "/crs/stats?fakeClientIpAddress=12.34.56.78")
    ...
    fun itConsistentlyRedirectsValidRequests() {
        ...
        try {
            ...
            for (i in 0..99) {
                response = httpClient.execute(httpGet)
                Assert.assertThat(response.getFirstHeader("Location").getValue(), IsEqual.equalTo(location))
                response.close()
            }
        } finally {
            if (response != null) response.close()
    ...
    fun itRedirectsHttpsRequests() {
        val httpGet: HttpGet? =
            HttpGet("https://localhost:" + routerSecurePort + "/stuff?fakeClientIpAddress=12.34.56.78")
        ...
        try {
            ...
            Assert.assertThat(response.getStatusLine().getStatusCode(), IsEqual.equalTo(302))
            val header: Header? = response.getFirstHeader("Location")
            Assert.assertThat(header.getValue(), Matchers.isIn(httpsOnlyLocations))
            Assert.assertThat(header.getValue(), Matchers.startsWith("https://"))
            Assert.assertThat(header.getValue(), Matchers.containsString(httpsOnlyId + ".thecdn.example.com/stuff"))
        } finally {
            if (response != null) response.close()
        }
    }
    ...
    fun itRedirectsFromHttpToHttps() {
        var httpGet: HttpGet? = HttpGet("http://localhost:" + routerHttpPort + "/stuff?fakeClientIpAddress=12.34.56.78")
        httpGet.addHeader("Host", "tr." + httpToHttpsId + ".bar")
        httpClient.execute(httpGet).use({ response ->
            Assert.assertThat(response.getStatusLine().getStatusCode(), IsEqual.equalTo(302))
            val header: Header? = response.getFirstHeader("Location")
            Assert.assertThat(header.getValue(), Matchers.isIn(httpToHttpsLocations))
            Assert.assertThat(header.getValue(), Matchers.startsWith("https://"))
            Assert.assertThat(header.getValue(), Matchers.containsString(httpToHttpsId + ".thecdn.example.com"))
            Assert.assertThat(header.getValue(), Matchers.containsString("/stuff"))
        })
        httpClient = HttpClientBuilder.create()
    ...
    fun itRejectsHttpsRequestsForHttpDeliveryService() {
        ...
        try {
            response = httpClient.execute(httpGet)
            Assert.assertThat(
                "Response 503 expected got" + response.getStatusLine().getStatusCode(),
    ...
    fun itPreservesProtocolForHttpAndHttps() {
        ...
        httpClient.execute(httpGet).use({ response ->
            Assert.assertThat(response.getStatusLine().getStatusCode(), IsEqual.equalTo(302))
            val header: Header? = response.getFirstHeader("Location")
            Assert.assertThat(header.getValue(), Matchers.isIn(httpAndHttpsLocations))
            Assert.assertThat(header.getValue(), Matchers.startsWith("http://"))
            Assert.assertThat(header.getValue(), Matchers.containsString(httpAndHttpsId + ".thecdn.example.com"))
            Assert.assertThat(header.getValue(), Matchers.containsString("/stuff"))
        })
        httpClient = HttpClientBuilder.create()
            .setSSLSocketFactory(ClientSslSocketFactory("tr.http-and-https-test.thecdn.example.com"))
    ...
    fun itRejectsCrConfigWithMissingCert() {
        ...
        httpClient.execute(httpGet).use({ response ->
            Assert.assertThat(response.getStatusLine().getStatusCode(), IsEqual.equalTo(302))
            Assert.assertThat(
                response.getFirstHeader("Location").getValue(), Matchers.isOneOf(
        ...
        var httpPost: HttpPost? = HttpPost("http://localhost:" + testHttpPort + "/crconfig-2")
        ...
        println(Arrays.toString(httpGet.getAllHeaders()))
        httpClient.execute(httpGet).use({ response ->
            Assert.assertThat(response.getStatusLine().getStatusCode(), IsEqual.equalTo(302))
            val location: String? = response.getFirstHeader("Location").getValue()
            Assert.assertThat(
    ...
    fun itDoesUseLocationFormatResponse() {
        val httpGet: HttpGet? =
            HttpGet("http://localhost:" + routerHttpPort + "/stuff?fakeClientIpAddress=12.34.56.78&format=json")
        httpGet.addHeader("Host", "tr." + deliveryServiceId + ".bar")
        var response: CloseableHttpResponse? = null
        try {
            response = httpClient.execute(httpGet)
            ...
            Assert.assertThat(json.get("location").asText(), Matchers.isIn(validLocations))
            Assert.assertThat(json.get("location").asText(), Matchers.startsWith("http://"))
        } finally {
            if (response != null) response.close()
        }
    }
    ...
    fun itDoesNotUseLocationFormatResponseForHead() {
        val httpHead: HttpHead? =
            HttpHead("http://localhost:" + routerHttpPort + "/stuff?fakeClientIpAddress=12.34.56.78&format=json")
        httpHead.addHeader("Host", "tr." + deliveryServiceId + ".bar")
        var response: CloseableHttpResponse? = null
        try {
            response = httpClient.execute(httpHead)
            Assert.assertThat(response.getStatusLine().getStatusCode(), IsEqual.equalTo(200))
            Assert.assertThat("Failed getting null body for HEAD request", response.getEntity(), Matchers.nullValue())
        } finally {
            if (response != null) response.close()
        }
    }
    ...
    internal inner class ClientSslSocketFactory constructor(private val host: String?) : SSLConnectionSocketFactory(
        ...
        override fun prepareSocket(sslSocket: SSLSocket?) {
            val serverName: SNIHostName? = SNIHostName(host)
            val serverNames: MutableList<SNIServerName?>? = ArrayList(1)
            serverNames.add(serverName)
            val params: SSLParameters? = sslSocket.getSSLParameters()
    ...
    internal inner class TestHostnameVerifier constructor() : HostnameVerifier {
        public override fun verify(s: String?, sslSession: SSLSession?): Boolean {
            Assert.assertThat(
