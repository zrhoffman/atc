...
class DeliveryServicesTest constructor() {
    ...
    fun itReturnsIdOfValidDeliveryService() {
        val encodedUrl: String? =
            URLEncoder.encode("http://trafficrouter01.steering-target-1.thecdn.example.com/stuff", "utf-8")
        val httpGet: HttpGet? = HttpGet("http://localhost:3333/crs/deliveryservices?url=" + encodedUrl)
    ...
    fun itReturnsNotFoundForNonexistentDeliveryService() {
        ...
        try {
            response = closeableHttpClient.execute(httpGet)
            MatcherAssert.assertThat(response.getStatusLine().getStatusCode(), IsEqual.equalTo(404))
        } finally {
            if (response != null) response.close()
        }
    }
    ...
    fun itReturnsBadRequestForBadUrlQueryParameter() {
        val encodedUrl: String? = "httptrafficrouter01somedeliveryservicesomecdndomainfoo/stuff"
        val httpGet: HttpGet? = HttpGet("http://localhost:3333/crs/deliveryservices?url=" + encodedUrl)
        var response: CloseableHttpResponse? = null
        try {
            response = closeableHttpClient.execute(httpGet)
