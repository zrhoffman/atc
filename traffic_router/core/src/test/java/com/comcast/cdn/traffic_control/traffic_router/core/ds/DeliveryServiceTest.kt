...
class DeliveryServiceTest constructor() {
    @Test
    @Throws(Exception::class)
    fun itHandlesLackOfRequestHeaderNamesInJSON() {
        val mapper: ObjectMapper? = ObjectMapper()
        val jsonStr: String? = "{\"routingName\":\"edge\",\"coverageZoneOnly\":false}"
        val jsonConfiguration: JsonNode? = mapper.readTree(jsonStr)
        val deliveryService: DeliveryService? = DeliveryService("a-delivery-service", jsonConfiguration)
        MatcherAssert.assertThat(deliveryService.getRequestHeaders().size, Matchers.equalTo(0))
    }
    ...
    fun itHandlesLackOfConsistentHashQueryParamsInJSON() {
        val mapper: ObjectMapper? = ObjectMapper()
        val json: JsonNode? = mapper.readTree("{\"routingName\":\"edge\",\"coverageZoneOnly\":false}")
        val d: DeliveryService? = DeliveryService("test", json)
    ...
    fun itConfiguresRequestHeadersFromJSON() {
        ...
        MatcherAssert.assertThat(
            deliveryService.getRequestHeaders(),
    ...
    fun itAddsRequiredCapabilities() {
        val mapper: ObjectMapper? = ObjectMapper()
        val jsonConfiguration: JsonNode? =
            mapper.readTree("{\"requiredCapabilities\":[\"all-read\",\"all-write\",\"cdn-read\"],\"routingName\":\"edge\",\"coverageZoneOnly\":false}")
        val deliveryService: DeliveryService? = DeliveryService("has-required-capabilities", jsonConfiguration)
        MatcherAssert.assertThat(
