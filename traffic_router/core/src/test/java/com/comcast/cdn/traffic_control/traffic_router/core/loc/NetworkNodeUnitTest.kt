...
class NetworkNodeUnitTest constructor() {
    @Test
    @Throws(Exception::class)
    fun itSupportsARootNode() {
        val root: NetworkNode? = NetworkNode("0.0.0.0/0")
        val network: NetworkNode? = NetworkNode("192.168.1.0/24")
    ...
    fun itDoesNotAddANodeOutsideOfNetwork() {
        val network: NetworkNode? = NetworkNode("192.168.0.0/16")
        val subnetwork: NetworkNode? = NetworkNode("10.10.0.0/16")
    ...
    fun itDoesNotAddDuplicates() {
        val supernet: NetworkNode? = NetworkNode("192.168.0.0/16")
        val network1: NetworkNode? = NetworkNode("192.168.1.0/24")
        val duplicate: NetworkNode? = NetworkNode("192.168.1.0/24")
    ...
    fun itPutsNetworksIntoOrderedHierarchy() {
        val root: NetworkNode? = NetworkNode("0.0.0.0/0")
        val subnet1: NetworkNode? = NetworkNode("192.168.6.0/24")
        val subnet2: NetworkNode? = NetworkNode("192.168.55.0/24")
        val net: NetworkNode? = NetworkNode("192.168.0.0/16")
        root.add(net)
    ...
    fun itSupportsDeepCaches() {
        ...
        val json: JsonNode? = mapper.readTree(czmapString)
        val networkNode: NetworkNode? = NetworkNode.Companion.generateTree(json, false, true)
        val foundNetworkNode: NetworkNode? = networkNode.getNetwork("192.168.55.100")
        val expected: MutableSet<String?>? = HashSet()
        expected.add("host1")
        expected.add("host2")
        MatcherAssert.assertThat(foundNetworkNode.getDeepCacheNames(), Matchers.equalTo(expected))
    }
    ...
    fun itDoesIpV6() {
        ...
        val foundNetworkNode: NetworkNode? = networkNode.getNetwork("1234:5678::1")
        MatcherAssert.assertThat(foundNetworkNode.getLoc(), Matchers.equalTo("us-co-denver"))
    }
    ...
    fun itPutsAllSubnetsUnderSuperNet() {
        ...
        val generation1Node: NetworkNode? = root.children.values.iterator().next()
        ...
        val generation2FirstNode: NetworkNode? = iterator.next().key
        val generation2SecondNode: NetworkNode? = iterator.next().key
    ...
    fun itMatchesIpsInOverlappingSubnets() {
        ...
        val json: JsonNode? = mapper.readTree(czmapString)
        val networkNode: NetworkNode? = generateTree(json, false)
        val foundNetworkNode: NetworkNode? = networkNode.getNetwork("192.168.55.2")
        MatcherAssert.assertThat(foundNetworkNode.getLoc(), Matchers.equalTo("us-co-denver"))
    }
