...
class ConsistentHasherTest {
    ...
    fun itHashes() {
        ...
        Assert.assertThat(
            hashable,
            Matchers.anyOf(IsEqual.equalTo(hashable1), IsEqual.equalTo(hashable2), IsEqual.equalTo(hashable3))
        )
        val nextHashable =
            consistentHasher!!.selectHashable(hashables, Dispersion(mapper.createObjectNode()), "some-string")
    ...
    fun itemsMigrateFromSmallerToLargerBucket() {
        ...
        Assert.assertThat(
            rehashedPaths[grownBucket]!!.size, Matchers.greaterThan(
                hashedPaths[smallerBucket]!!.size
            )
        )
        Assert.assertThat(
            rehashedPaths[shrunkBucket]!!.size, Matchers.lessThan(
