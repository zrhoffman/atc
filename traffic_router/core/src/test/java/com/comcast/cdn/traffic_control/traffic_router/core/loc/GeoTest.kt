...
class GeoTest constructor() {
    ...
    fun testIps() {
        try {
            val testips: Array<Array<String?>?>? = arrayOf(arrayOf("40.40.40.40", "cache-group-1"))
            for (i in testips.indices) {
                val location: Geolocation? = maxmindGeolocationService.location(testips.get(i).get(0))
    ...
    companion object {
        private val LOGGER: Logger? = Logger.getLogger(GeoTest::class.java)
