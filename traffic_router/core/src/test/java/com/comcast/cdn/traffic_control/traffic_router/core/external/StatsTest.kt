...
class StatsTest constructor() {
    ...
    fun itGetsApplicationStats() {
        val httpGet: HttpGet? = HttpGet("http://localhost:3333/crs/stats")
        var httpResponse: CloseableHttpResponse? = null
        try {
            httpResponse = httpClient.execute(httpGet)
            val responseContent: String? = EntityUtils.toString(httpResponse.getEntity())
            val objectMapper: ObjectMapper? = ObjectMapper()
            val data: MutableMap<String?, Any?>? =
                objectMapper.readValue(responseContent, object : TypeReference<HashMap<String?, Any?>?>() {})
            MatcherAssert.assertThat(data.keys, Matchers.containsInAnyOrder("app", "stats"))
            val appData: MutableMap<String?, Any?>? = data.get("app") as MutableMap<String?, Any?>?
            MatcherAssert.assertThat(
                appData.keys,
                Matchers.containsInAnyOrder("buildTimestamp", "name", "deploy-dir", "git-revision", "version")
            )
            val statsData: MutableMap<String?, Any?>? = data.get("stats") as MutableMap<String?, Any?>?
            MatcherAssert.assertThat(
                statsData.keys,
            ...
            val dnsStats: MutableMap<String?, Any?>? = statsData.get("dnsMap") as MutableMap<String?, Any?>?
            val cacheDnsStats: MutableMap<String?, Any?>? =
                dnsStats.values.iterator().next() as MutableMap<String?, Any?>?
            MatcherAssert.assertThat(
            ...
            val httpStats: MutableMap<String?, Any?>? = statsData.get("httpMap") as MutableMap<String?, Any?>?
            val cacheHttpStats: MutableMap<String?, Any?>? =
                httpStats.values.iterator().next() as MutableMap<String?, Any?>?
            MatcherAssert.assertThat(
            ...
            val updateTracker: MutableMap<String?, Any?>? = statsData.get("updateTracker") as MutableMap<String?, Any?>?
            val keys: MutableSet<String?>? = updateTracker.keys
            val expectedStats: MutableList<String?>? =
            ...
            if (!keys.containsAll(expectedStats)) {
                ...
                for (stat: String? in expectedStats) {
                    joiner.add(stat)
                }
                Assert.fail("Missing at least one of the following keys '" + joiner.toString() + "'")
            }
        } finally {
    ...
    fun itGetsLocationsByIp() {
        val httpGet: HttpGet? = HttpGet("http://localhost:3333/crs/stats/ip/8.8.8.8")
        var response: CloseableHttpResponse? = null
        try {
            response = httpClient.execute(httpGet)
            val actual: String? = EntityUtils.toString(response.getEntity())
            val data: MutableMap<String?, Any?>? =
            ...
            val locationByGeo: MutableMap<String?, Any?>? = data.get("locationByGeo") as MutableMap<String?, Any?>?
            MatcherAssert.assertThat(
                locationByGeo.keys,
