...
class ConfigHandlerTest constructor() {
    ...
    fun itTestRelativeUrl() {
        val redirectUrl: String? = "relative/url"
        val dsId: String? = "relative-url"
        val urlType: Array<String?>? = arrayOf("")
        val typeUrl: Array<String?>? = arrayOf("")
        val dsMap: MutableMap<String?, DeliveryService?>? = HashMap()
        val ds: DeliveryService? = Mockito.mock(DeliveryService::class.java)
        Mockito.`when`(ds.getId()).thenReturn(dsId)
        Mockito.`when`(ds.getGeoRedirectUrl()).thenReturn(redirectUrl)
        Mockito.doAnswer(object : Answer<Void?> {
            public override fun answer(invocation: InvocationOnMock?): Void? {
                val args: Array<Any?>? = invocation.getArguments()
                typeUrl.get(0) = (args.get(0)) as String?
                return null
            }
        }).`when`(ds).setGeoRedirectFile(Matchers.anyString())
        Mockito.doAnswer(object : Answer<Void?> {
            public override fun answer(invocation: InvocationOnMock?): Void? {
                val args: Array<Any?>? = invocation.getArguments()
                urlType.get(0) = args.get(0) as String?
                return null
            }
        }).`when`(ds).setGeoRedirectUrlType(Matchers.anyString())
        dsMap.put(dsId, ds)
        val register: CacheRegister? = PowerMockito.mock(CacheRegister::class.java)
        Whitebox.invokeMethod<Any?>(handler, "initGeoFailedRedirect", dsMap, register)
        MatcherAssert.assertThat(urlType.get(0), org.hamcrest.Matchers.equalTo("DS_URL"))
        MatcherAssert.assertThat(typeUrl.get(0), org.hamcrest.Matchers.equalTo(""))
    }

    @Test
    @Throws(Exception::class)
    fun itTestRelativeUrlNegative() {
        val redirectUrl: String? = "://invalid"
        val dsId: String? = "relative-url"
        val urlType: Array<String?>? = arrayOf("")
        val typeUrl: Array<String?>? = arrayOf("")
        val dsMap: MutableMap<String?, DeliveryService?>? = HashMap()
        ...
        val register: CacheRegister? = PowerMockito.mock(CacheRegister::class.java)
        Whitebox.invokeMethod<Any?>(handler, "initGeoFailedRedirect", dsMap, register)
        MatcherAssert.assertThat(urlType.get(0), org.hamcrest.Matchers.equalTo(""))
        MatcherAssert.assertThat(typeUrl.get(0), org.hamcrest.Matchers.equalTo(""))
    }
    ...
    fun itTestNoSuchDsUrl() {
        val path: String? = "/ds/url"
        val redirectUrl: String? = "http://test.com" + path
        val dsId: String? = "relative-url"
        val urlType: Array<String?>? = arrayOf("")
        val typeUrl: Array<String?>? = arrayOf("")
        val dsMap: MutableMap<String?, DeliveryService?>? = HashMap()
        ...
        Whitebox.invokeMethod<Any?>(handler, "initGeoFailedRedirect", dsMap, register)
        MatcherAssert.assertThat(urlType.get(0), org.hamcrest.Matchers.equalTo("NOT_DS_URL"))
        MatcherAssert.assertThat(typeUrl.get(0), org.hamcrest.Matchers.equalTo(path))
    }
    ...
    fun itTestNotThisDsUrl() {
        val path: String? = "/ds/url"
        val redirectUrl: String? = "http://test.com" + path
        val dsId: String? = "relative-ds"
        val anotherId: String? = "another-ds"
        val urlType: Array<String?>? = arrayOf("")
        val typeUrl: Array<String?>? = arrayOf("")
        val dsMap: MutableMap<String?, DeliveryService?>? = HashMap()
        val ds: DeliveryService? = Mockito.mock(DeliveryService::class.java)
        ...
        dsMap.put(dsId, ds)
        val anotherDs: DeliveryService? = Mockito.mock(DeliveryService::class.java)
        Mockito.`when`(ds.getId()).thenReturn(anotherId)
        val register: CacheRegister? = PowerMockito.mock(CacheRegister::class.java)
        Mockito.`when`(
            register.getDeliveryService(
                Matchers.any(
    ...
    fun itParsesTheTopologiesConfig() {
        ...
        val cacheId: String? = "edge"
        val cache: Cache? = Cache(cacheId, cacheId, 0)
        val location: String? = "CDN_in_a_Box_Edge"
        val cacheLocation: CacheLocation? = CacheLocation(location, Geolocation(38.897663, 38.897663))
        cacheLocation.addCache(cache)
        val locations: MutableSet<CacheLocation?>? = HashSet()
        locations.add(cacheLocation)
        val register: CacheRegister? = CacheRegister()
        register.setConfiguredLocations(locations)

        /* Add a capability to the Cache */
        val capability: String? = "a-capability"
        ...
        val dsId: String? = "top-ds"
        val routingName: String? = "cdn"
        val domain: String? = "ds.site.com"
        val topology: String? = "foo"
        val superHackedRegexp: String? = "(.*\\.|^)" + dsId + "\\..*"
        val ds: DeliveryService? = Mockito.mock(DeliveryService::class.java)
        Mockito.`when`(ds.getId()).thenReturn(dsId)
        Mockito.`when`(ds.getDomain()).thenReturn(domain)
        Mockito.`when`(ds.getRemap(superHackedRegexp)).thenReturn(domain)
        Mockito.`when`(ds.getRoutingName()).thenReturn(routingName)
        Mockito.`when`(ds.getTopology()).thenReturn(topology)
        Mockito.`when`(ds.hasRequiredCapabilities(capabilities)).thenReturn(true)
        Mockito.`when`(ds.isDns()).thenReturn(false)
        val dsMap: MutableMap<String?, DeliveryService?>? = HashMap()
        dsMap.put(dsId, ds)
        val dsMatcher: DeliveryServiceMatcher? = DeliveryServiceMatcher(ds)
        dsMatcher.addMatch(DeliveryServiceMatcher.Type.HOST, superHackedRegexp, "")
        val dsMatchers: TreeSet<DeliveryServiceMatcher?>? = TreeSet()
        ...
        val allTopologiesJson: JsonNode? = mapper.readTree("{\"" + topology + "\":{\"nodes\":[\"" + location + "\"]}}")
        ...
        val dsReferences: MutableCollection<DeliveryServiceReference?>? = cache.getDeliveryServices()
        MatcherAssert.assertThat(dsReferences.size, org.hamcrest.Matchers.equalTo(1))
        MatcherAssert.assertThat(
            dsReferences.iterator().next().getDeliveryServiceId(),
            org.hamcrest.Matchers.equalTo(dsId)
        )
    }
    ...
    fun testParseLocalizationMethods() {
        val allMethods: Array<LocalizationMethod?>? = arrayOf(
        ...
        val mapper: ObjectMapper? = ObjectMapper()
        val allMethodsString: String? = "{\"localizationMethods\": [\"CZ\",\"DEEP_CZ\",\"GEO\"]}"
        val allMethodsJson: JsonNode? = mapper.readTree(allMethodsString)
        var actual: MutableSet<LocalizationMethod?>? =
            Whitebox.invokeMethod(handler, "parseLocalizationMethods", "foo", allMethodsJson)
        MatcherAssert.assertThat(actual, org.hamcrest.Matchers.equalTo(expected))
        val noMethodsString: String? = "{}"
        val noMethodsJson: JsonNode? = mapper.readTree(noMethodsString)
        actual = Whitebox.invokeMethod(handler, "parseLocalizationMethods", "foo", noMethodsJson)
        MatcherAssert.assertThat(actual, org.hamcrest.Matchers.equalTo(expected))
        val nullMethodsString: String? = "{\"localizationMethods\": null}"
        val nullMethodsJson: JsonNode? = mapper.readTree(nullMethodsString)
        actual = Whitebox.invokeMethod(handler, "parseLocalizationMethods", "foo", nullMethodsJson)
        MatcherAssert.assertThat(actual, org.hamcrest.Matchers.equalTo(expected))
        val CZMethodsString: String? = "{\"localizationMethods\": [\"CZ\"]}"
        val CZMethodsJson: JsonNode? = mapper.readTree(CZMethodsString)
        expected.clear()
