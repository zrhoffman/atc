...
class LocationsTest constructor() {
    ...
    fun itGetsAListOfLocations() {
        val httpGet: HttpGet? = HttpGet("http://localhost:3333/crs/locations")
        ...
        try {
            ...
            MatcherAssert.assertThat(jsonNode.get("locations").get(0).asText(), IsNot.not(IsEqual.equalTo("")))
        } finally {
    ...
    fun itGetsAListOfCaches() {
        val httpGet: HttpGet? = HttpGet("http://localhost:3333/crs/locations/caches")
        var response: CloseableHttpResponse? = null
        try {
            response = closeableHttpClient.execute(httpGet)
            val objectMapper: ObjectMapper? = ObjectMapper(JsonFactory())
            val jsonNode: JsonNode? = objectMapper.readTree(EntityUtils.toString(response.getEntity()))
            val locationName: String? = jsonNode.get("locations").fieldNames().next()
            val cacheNode: JsonNode? = jsonNode.get("locations").get(locationName).get(0)
            MatcherAssert.assertThat(cacheNode.get("cacheId").asText(), IsNot.not(IsEqual.equalTo("")))
            MatcherAssert.assertThat(cacheNode.get("fqdn").asText(), IsNot.not(IsEqual.equalTo("")))
            MatcherAssert.assertThat(cacheNode.get("ipAddresses").isArray(), IsEqual.equalTo(true))
            MatcherAssert.assertThat(cacheNode.has("adminStatus"), IsEqual.equalTo(true))
            MatcherAssert.assertThat(cacheNode.get("port").asInt(-123456), IsNot.not(IsEqual.equalTo(-123456)))
            MatcherAssert.assertThat(
                cacheNode.get("lastUpdateTime").asInt(-123456),
                IsNot.not(IsEqual.equalTo(-123456))
            )
            MatcherAssert.assertThat(cacheNode.get("connections").asInt(-123456), IsNot.not(IsEqual.equalTo(-123456)))
            MatcherAssert.assertThat(cacheNode.get("currentBW").asInt(-123456), IsNot.not(IsEqual.equalTo(-123456)))
            MatcherAssert.assertThat(cacheNode.get("availBW").asInt(-123456), IsNot.not(IsEqual.equalTo(-123456)))
            MatcherAssert.assertThat(
                cacheNode.get("cacheOnline").asText(),
                AnyOf.anyOf(IsEqual.equalTo("true"), IsEqual.equalTo("false"))
            )
            MatcherAssert.assertThat(
                cacheNode.get("lastUpdateHealthy").asText(),
                AnyOf.anyOf(IsEqual.equalTo("true"), IsEqual.equalTo("false"))
            )
        } finally {
            if (response != null) response.close()
    ...
    fun itGetsCachesForALocation() {
        var httpGet: HttpGet? = HttpGet("http://localhost:3333/crs/locations")
        var response: CloseableHttpResponse? = null
        try {
            response = closeableHttpClient.execute(httpGet)
            val objectMapper: ObjectMapper? = ObjectMapper(JsonFactory())
            var jsonNode: JsonNode? = objectMapper.readTree(EntityUtils.toString(response.getEntity()))
            val location: String? = jsonNode.get("locations").get(0).asText()
            ...
            MatcherAssert.assertThat(jsonNode.get("caches").isArray(), IsEqual.equalTo(true))
            val cacheNode: JsonNode? = jsonNode.get("caches").get(0)
            MatcherAssert.assertThat(cacheNode.get("cacheId").asText(), IsNot.not(IsEqual.equalTo("")))
            MatcherAssert.assertThat(cacheNode.get("fqdn").asText(), IsNot.not(IsEqual.equalTo("")))
            MatcherAssert.assertThat(cacheNode.get("ipAddresses").isArray(), IsEqual.equalTo(true))
            MatcherAssert.assertThat(cacheNode.has("adminStatus"), IsEqual.equalTo(true))
