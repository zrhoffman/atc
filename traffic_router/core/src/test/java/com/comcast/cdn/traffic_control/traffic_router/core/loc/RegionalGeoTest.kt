...
class RegionalGeoTest constructor() {
    @Before
    @Throws(Exception::class)
    fun setUp() {
        val dbFile: File? = File("src/test/resources/regional_geoblock.json")
        RegionalGeo.Companion.parseConfigFile(dbFile, false)
    }

    @Test
    fun testEnforceAllowedCoordinateRange() {
        val dsvcId: String? = "ds-geoblock-exclude"
        val url: String? = "http://ds1.example.com/live1"
        val postal: String? = null
        val ip: String? = "10.0.0.1"
        val result: RegionalGeoResult? = RegionalGeo.Companion.enforce(dsvcId, url, ip, postal, 12.0, 55.0)
        MatcherAssert.assertThat(result.getType(), Matchers.equalTo(RegionalGeoResultType.ALLOWED))
        MatcherAssert.assertThat(result.getUrl(), Matchers.equalTo(url))
    }

    @Test
    fun testEnforceAlternateWithCacheNoCoordinateRangeNoPostalCode() {
        val dsvcId: String? = "ds-geoblock-include"
        val url: String? = "http://ds2.example.com/live2"
        val postal: String? = null
        val ip: String? = "10.0.0.1"
        val result: RegionalGeoResult? = RegionalGeo.Companion.enforce(dsvcId, url, ip, postal, 12.0, 55.0)
        MatcherAssert.assertThat(result.getType(), Matchers.equalTo(RegionalGeoResultType.ALTERNATE_WITH_CACHE))
    }
    ...
    fun testEnforceAlternateWithCache() {
        ...
        val result: RegionalGeoResult? = RegionalGeo.Companion.enforce(dsvcId, url, ip, postal, 0.0, 0.0)
        MatcherAssert.assertThat(result.getType(), Matchers.equalTo(RegionalGeoResultType.ALTERNATE_WITH_CACHE))
        MatcherAssert.assertThat(result.getUrl(), Matchers.equalTo("/path/redirect_T2"))
    }
    ...
    fun testEnforceAlternateWithoutCache() {
        val dsvcId: String? = "ds-geoblock-exclude"
        val url: String? = "http://ds1.example.com/live1"
        val postal: String? = "V5G"
        val ip: String? = "10.0.0.1"
        val result: RegionalGeoResult? = RegionalGeo.Companion.enforce(dsvcId, url, ip, postal, 0.0, 0.0)
        MatcherAssert.assertThat(result.getType(), Matchers.equalTo(RegionalGeoResultType.ALTERNATE_WITHOUT_CACHE))
        MatcherAssert.assertThat(result.getUrl(), Matchers.equalTo("http://example.com/redirect_T1"))
    }

    @Test
    fun testEnforceDeniedNoDsvc() {
        val dsvcId: String? = "ds-geoblock-no-exist"
        val url: String? = "http://ds1.example.com/live1"
        val postal: String? = "V5G"
        val ip: String? = "10.0.0.1"
        val result: RegionalGeoResult? = RegionalGeo.Companion.enforce(dsvcId, url, ip, postal, 0.0, 0.0)
        MatcherAssert.assertThat(result.getType(), Matchers.equalTo(RegionalGeoResultType.DENIED))
    }

    @Test
    fun testEnforceDeniedNoRegexMatch() {
        val dsvcId: String? = "ds-geoblock-include"
        val url: String? = "http://ds1.example.com/live-not-exist"
        val postal: String? = "V5G"
        val ip: String? = "10.0.0.1"
        val result: RegionalGeoResult? = RegionalGeo.Companion.enforce(dsvcId, url, ip, postal, 0.0, 0.0)
        MatcherAssert.assertThat(result.getType(), Matchers.equalTo(RegionalGeoResultType.DENIED))
    ...
    fun testEnforceAlternateToPathNoSlash() {
        val dsvcId: String? = "ds-geoblock-include"
        val url: String? = "http://ds1.example.com/live3"
        val postal: String? = "V5D"
        val ip: String? = "10.0.0.1"
        val result: RegionalGeoResult? = RegionalGeo.Companion.enforce(dsvcId, url, ip, postal, 0.0, 0.0)
        MatcherAssert.assertThat(result.getType(), Matchers.equalTo(RegionalGeoResultType.ALTERNATE_WITH_CACHE))
        MatcherAssert.assertThat(result.getUrl(), Matchers.equalTo("/redirect_T3"))
    }
    ...
    fun testEnforceWhiteListAllowed() {
        val dsvcId: String? = "ds-geoblock-include"
        val url: String? = "http://ds1.example.com/live4"
        val postal: String? = null
        val ip: String? = "129.100.254.2"
        val result: RegionalGeoResult? = RegionalGeo.Companion.enforce(dsvcId, url, ip, postal, 0.0, 0.0)
        MatcherAssert.assertThat(result.getType(), Matchers.equalTo(RegionalGeoResultType.ALLOWED))
        MatcherAssert.assertThat(result.getUrl(), Matchers.equalTo(url))
    }
    ...
    fun testEnforceAllowedHttpsRedirect() {
        val dsvcId: String? = "ds-geoblock-redirect-https"
        val url: String? = "http://ds1.example.com/httpsredirect"
        val postal: String? = null
        val ip: String? = "129.100.254.2"
        val result: RegionalGeoResult? = RegionalGeo.Companion.enforce(dsvcId, url, ip, postal, 0.0, 0.0)
        MatcherAssert.assertThat(result.getType(), Matchers.equalTo(RegionalGeoResultType.ALTERNATE_WITHOUT_CACHE))
        MatcherAssert.assertThat(result.getUrl(), Matchers.equalTo("https://example.com/redirect_https"))
    }

    @Test
    fun testEnforceSteeringReDirect() {
        val dsvcId: String? = "ds-steering-1"
        val url: String? = "http://ds1.example.com/steering"
        val postal: String? = null
        val ip: String? = "129.100.254.4"
        val result: RegionalGeoResult? = RegionalGeo.Companion.enforce(dsvcId, url, ip, postal, 0.0, 0.0)
        MatcherAssert.assertThat(result.getType(), Matchers.equalTo(RegionalGeoResultType.ALTERNATE_WITHOUT_CACHE))
        MatcherAssert.assertThat(result.getUrl(), Matchers.equalTo("https://example.com/steering-test"))
    }
    ...
    fun testEnforceNotInWhiteListAlternate() {
        ...
        val url: String? = "http://ds1.example.com/live4"
        val postal: String? = "N7G"
        val ip: String? = "129.202.254.2"
        val result: RegionalGeoResult? = RegionalGeo.Companion.enforce(dsvcId, url, ip, postal, 0.0, 0.0)
        MatcherAssert.assertThat(result.getType(), Matchers.equalTo(RegionalGeoResultType.ALTERNATE_WITH_CACHE))
        MatcherAssert.assertThat(result.getUrl(), Matchers.equalTo("/redirect_T4"))
    }
    ...
    fun testEnforceWhiteListAllowedRouteResultMultipleUrls() {
        val clientIp: String? = "129.100.254.2"
        val requestUrl: String? = "http://ds1.example.com/live4"
        ...
        val cache: Cache? = PowerMockito.mock(
            Cache::class.java
        )
        val ds: DeliveryService? = PowerMockito.mock(DeliveryService::class.java)
        PowerMockito.`when`(ds.getId()).thenReturn("ds-geoblock-include")
        PowerMockito.`when`(ds.createURIString(request, cache)).thenReturn(requestUrl)
        val tr: TrafficRouter? = PowerMockito.mock(TrafficRouter::class.java)
        ...
        val firstUrl: String? = "http://example.com/url1.m3u8"
        routeResult.addUrl(URL(firstUrl))
        enforce(tr, request, ds, cache, routeResult, track)
        MatcherAssert.assertThat(routeResult.getUrls().size, Matchers.equalTo(2))
        MatcherAssert.assertThat(routeResult.getUrls().get(1).toString(), Matchers.equalTo(requestUrl))
        MatcherAssert.assertThat(routeResult.getUrls().get(0).toString(), Matchers.equalTo(firstUrl))
    }
}