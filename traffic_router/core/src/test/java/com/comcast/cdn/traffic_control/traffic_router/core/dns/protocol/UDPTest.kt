...
class UDPTest constructor() {
    ...
    fun testGetMaxResponseLengthNoOPTQuery() {
        val name: Name? = Name.fromString("www.foo.com.")
    ...
    fun testGetMaxResponseLengthWithOPTQuery() {
        ...
        val name: Name? = Name.fromString("www.foo.com.")
        val question: Record? = Record.newRecord(name, Type.A, DClass.IN)
        val options: OPTRecord? = OPTRecord(size, 0, 0)
        val request: Message? = Message.newQuery(question)
    ...
    fun testUDPPacketHandler() {
        ...
        val aRecord: Record? = Record.newRecord(name, Type.A, DClass.IN, 3600)
        val response: Message? = Message.newQuery(question)
        response.getHeader().setFlag(Flags.QR.toInt())
        response.addRecord(aRecord, Section.ANSWER)
        val wireResponse: ByteArray? = response.toWire()
        val packet: DatagramPacket? = DatagramPacket(wireRequest, wireRequest.size, client, port)
        Mockito.`when`(
            nameServer.query(
                Matchers.any(
                    Message::class.java
        ...
        Mockito.doAnswer(Answer({ invocation: InvocationOnMock? ->
            val datagramPacket: DatagramPacket? = invocation.getArguments().get(0) as DatagramPacket?
            MatcherAssert.assertThat(datagramPacket.getData(), org.hamcrest.Matchers.equalTo(wireResponse))
            count.incrementAndGet()
            null
        })).`when`(datagramSocket).send(Matchers.any(DatagramPacket::class.java))
        val handler: UDPPacketHandler? = udp.UDPPacketHandler(packet)
        handler.run()
