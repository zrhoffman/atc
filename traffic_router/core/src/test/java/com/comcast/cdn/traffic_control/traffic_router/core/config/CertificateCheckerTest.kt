...
class CertificateCheckerTest constructor() {
    ...
    fun itReturnsFalseWhenDeliveryServiceNameIsNull() {
        val file: File? = File("src/test/resources/deliveryServices_missingDSName.json")
        val mapper: ObjectMapper? = ObjectMapper()
        deliveryServicesJson = mapper.readTree(file)
        val certificateChecker: CertificateChecker? = CertificateChecker()
    ...
    fun itReturnsTrueWhenAllHttpsDeliveryServicesHaveCertificates() {
        val file: File? = File("src/test/resources/deliveryServices.json")
        val mapper: ObjectMapper? = ObjectMapper()
        deliveryServicesJson = mapper.readTree(file)
        val certificateChecker: CertificateChecker? = CertificateChecker()
        MatcherAssert.assertThat(
    ...
    fun itReturnsFalseWhenAnyHttpsDeliveryServiceMissingCertificates() {
        val file: File? = File("src/test/resources/deliveryServices_missingCert.json")
        val mapper: ObjectMapper? = ObjectMapper()
        deliveryServicesJson = mapper.readTree(file)
        MatcherAssert.assertThat(
            CertificateChecker().certificatesAreValid(certificateDataList, deliveryServicesJson),
