...
class RRSetsBuilderTest constructor() {
    @Before
    ...
    fun findRRSet(rRsets: MutableList<RRset?>?, name: String?, type: Int): RRset? {
        val option: Optional<RRset?>? = rRsets.stream()
            .filter(Predicate({ rRset: RRset? -> (name == rRset.getName().toString()) && rRset.getType() == type }))
            .findFirst()
    ...
    fun itGroupsResourceRecordsAccordingToRfc4034() {
        val rRsets: MutableList<RRset?>? = RRSetsBuilder().build(ZoneTestRecords.records)
