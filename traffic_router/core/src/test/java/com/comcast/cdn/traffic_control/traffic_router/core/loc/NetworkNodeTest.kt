...
class NetworkNodeTest constructor() {
    ...
    private fun setUp(filename: String?, useDeep: Boolean): NetworkNode? {
        ...
        val file: File? = File(javaClass.getClassLoader().getResource(filename).toURI())
        ...
        val coverageZones: JsonNode? = jsonNode.get(czKey)
        val networkIter: MutableIterator<String?>? = coverageZones.fieldNames()
        while (networkIter.hasNext()) {
            val loc: String? = networkIter.next()
            val locData: JsonNode? = coverageZones.get(loc)
            for (networkType: String? in arrayOf<String?>("network", "network6")) {
                val networks: JsonNode? = locData.get(networkType)
                ...
                if (!testNetMap.containsKey(loc)) {
                    testNetMap.put(loc, ArrayList())
                }
                val addressList: MutableList<String?>? = testNetMap.get(loc)
    ...
    fun testIps() {
        try {
            for (location: String? in netMap.keys) {
                for (address: String? in netMap.get(location)) {
                    val nn: NetworkNode? = root.getNetwork(address)
                    Assert.assertNotNull(nn)
                    val loc: String? = nn.getLoc()
    ...
    private fun testNetworkNodePerformance(
        ...
        val nnTPS: Long = System.getProperty("nnTPS", "12000").toLong()
        for (i in 0 until iterations) {
            for (location: String? in testNetMap.keys) {
                try {
                    for (address: String? in testNetMap.get(location)) {
                        val nn: NetworkNode? = testRoot.getNetwork(address)
                    }
        ...
        val tps: Long = (iterations / runTime) * 1000
        MatcherAssert.assertThat(tps, Matchers.greaterThanOrEqualTo(nnTPS))
    }

    companion object {
        private val LOGGER: Logger? = Logger.getLogger(NetworkNodeTest::class.java)
    }
}