...
class GeoSortSteeringResultsTest constructor() {
    ...
    fun testClientGeolocationIsNull() {
        val steeringTarget: SteeringTarget? = SteeringTarget()
    ...
    fun testGeoSortingMixedWithNonGeoTargets() {
        val cache: Cache? = Cache("fake-id", "fake-hash-id", 1, clientLocation)
        var target: SteeringTarget?
        target = SteeringTarget()
        target.setOrder(-1)
        val resultNoGeoNegativeOrder: SteeringResult? = SteeringResult(target, deliveryService)
        resultNoGeoNegativeOrder.setCache(cache)
        steeringResults.add(resultNoGeoNegativeOrder)
        target = SteeringTarget()
        target.setOrder(1)
        val resultNoGeoPositiveOrder: SteeringResult? = SteeringResult(target, deliveryService)
        resultNoGeoPositiveOrder.setCache(cache)
        steeringResults.add(resultNoGeoPositiveOrder)
        target = SteeringTarget()
        target.setOrder(0)
        val resultNoGeoZeroOrder: SteeringResult? = SteeringResult(target, deliveryService)
        resultNoGeoZeroOrder.setCache(cache)
        steeringResults.add(resultNoGeoZeroOrder)
        target = SteeringTarget()
        target.setGeolocation(clientLocation)
        target.setOrder(0)
        val resultGeo: SteeringResult? = SteeringResult(target, deliveryService)
