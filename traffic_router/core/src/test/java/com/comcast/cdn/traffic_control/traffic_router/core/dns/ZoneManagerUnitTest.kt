...
class ZoneManagerUnitTest constructor() {
    var zoneManager: ZoneManager? = null
    @Before
    @Throws(Exception::class)
    fun before() {
        val trafficRouter: TrafficRouter? = Mockito.mock(TrafficRouter::class.java)
        ...
        val signatureManager: SignatureManager? = PowerMockito.mock(SignatureManager::class.java)
    ...
    fun itMarksResultTypeAndLocationInDNSAccessRecord() {
        val qname: Name? = Name.fromString("edge.www.google.com.")
        val client: InetAddress? = InetAddress.getByName("192.168.56.78")
        val setResponse: SetResponse? = Mockito.mock(SetResponse::class.java)
        Mockito.`when`(setResponse.isSuccessful()).thenReturn(false)
        val zone: Zone? = Mockito.mock(Zone::class.java)
        Mockito.`when`(zone.findRecords(Matchers.any(Name::class.java), Matchers.anyInt())).thenReturn(setResponse)
        Mockito.`when`(zone.getOrigin()).thenReturn(Name(qname, 1))
    ...
    fun testZonesAreEqual() {
        internal class TestCase constructor(
            var reason: String?,
            var r1: Array<Record?>?,
            var r2: Array<Record?>?,
        ...
        val testCases: Array<TestCase?>? = arrayOf(
        ...
        for (t: TestCase? in testCases) {
            val input1: MutableList<Record?>? = Arrays.asList(*t.r1)
            val input2: MutableList<Record?>? = Arrays.asList(*t.r2)
            val copy1: MutableList<Record?>? = Arrays.asList(*t.r1)
            val copy2: MutableList<Record?>? = Arrays.asList(*t.r2)
