...
object CacheRegisterBuilder {
    ...
    fun parseCacheConfig(contentServers: JsonNode?, cacheRegister: CacheRegister?) {
        ...
        val contentServersIter: MutableIterator<String?>? = contentServers.fieldNames()
        while (contentServersIter.hasNext()) {
            val node: String? = contentServersIter.next()
            ...
            if (loc != null) {
                ...
                val cache: Cache? = Cache(node, hashId, hashCount)
                ...
                if (jo.has("deliveryServices")) {
                    ...
                    while (dsIter.hasNext()) {
                        ...
                        if (dso.isArray()) {
                            var i: Int = 0
                            for (fqdn: JsonNode? in dso) {
                                val name: String? = fqdn.asText().toLowerCase()
                                if (i == 0) {
                                ...
                                if (name.contains(tld)) {
                                    val reName: String? = name.replace("^.*?\\.".toRegex(), "")
    ...
    fun parseDeliveryServiceConfig(deliveryServices: JsonNode?, cacheRegister: CacheRegister?) {
        ...
        val keyIter: MutableIterator<String?>? = deliveryServices.fieldNames()
        while (keyIter.hasNext()) {
            val dsId: String? = keyIter.next()
            val dsJo: JsonNode? = JsonUtils.getJsonNode(deliveryServices, dsId)
            val matchsets: JsonNode? = JsonUtils.getJsonNode(dsJo, "machsets")
            val ds: DeliveryService? = DeliveryService(dsId, dsJo)
            val isDns: Boolean = false
            dsMap.put(dsId, ds)
            for (matchset: JsonNode? in matchsets) {
                val protocol: String? = JsonUtils.getString(matchset, "protocol")
                val m: DeliveryServiceMatcher? = DeliveryServiceMatcher(ds)
                deliveryServiceMatchers.add(m)
                for (matchlist: JsonNode? in matchset.get("matchlist")) {
                    val type: DeliveryServiceMatcher.Type? =
                        DeliveryServiceMatcher.Type.valueOf(JsonUtils.getString(matchlist, "match-type"))
