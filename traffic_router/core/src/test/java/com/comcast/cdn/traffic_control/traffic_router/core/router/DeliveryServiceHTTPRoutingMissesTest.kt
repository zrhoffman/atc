...
class DeliveryServiceHTTPRoutingMissesTest constructor() {
    ...
    fun before() {
        ...
        val unusedByTest: JsonNode? = Mockito.mock(
            JsonNode::class.java
        )
        val ttls: JsonNode? = Mockito.mock(
            JsonNode::class.java
        )
        Mockito.`when`(unusedByTest.get("ttls")).thenReturn(ttls)
        Mockito.`when`(unusedByTest.has("routingName")).thenReturn(true)
        Mockito.`when`(unusedByTest.get("routingName")).thenReturn(mapper.readTree("\"edge\""))
        Mockito.`when`(unusedByTest.has("coverageZoneOnly")).thenReturn(true)
        Mockito.`when`(unusedByTest.get("coverageZoneOnly")).thenReturn(mapper.readTree("true"))
        Mockito.`when`(unusedByTest.has("deepCachingType")).thenReturn(true)
        Mockito.`when`(unusedByTest.get("deepCachingType")).thenReturn(mapper.readTree("\"NEVER\""))
    ...
    fun itSetsDetailsWhenNoHTTPBypass() {
        Mockito.`when`(bypassDestination.get("HTTP")).thenReturn(null)
        deliveryService.getFailureHttpResponse(httpRequest, track)
        MatcherAssert.assertThat(track.getResultDetails(), Matchers.equalTo(ResultDetails.DS_NO_BYPASS))
        MatcherAssert.assertThat(track.getResult(), Matchers.equalTo(ResultType.MISS))
    }
    ...
    fun itSetsDetailsWhenNoFQDNBypass() {
        ...
        Mockito.doReturn(null).`when`(httpJsonObject).get("fqdn")
        Mockito.`when`(bypassDestination.get("HTTP")).thenReturn(httpJsonObject)
        deliveryService.getFailureHttpResponse(httpRequest, track)
        Mockito.verify(httpJsonObject).get("fqdn")
        MatcherAssert.assertThat(track.getResultDetails(), Matchers.equalTo(ResultDetails.DS_NO_BYPASS))
        MatcherAssert.assertThat(track.getResult(), Matchers.equalTo(ResultType.MISS))
    }
}