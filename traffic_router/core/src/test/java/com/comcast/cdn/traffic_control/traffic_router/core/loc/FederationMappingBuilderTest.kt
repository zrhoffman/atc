...
class FederationMappingBuilderTest constructor() {
    @Test
    @Throws(Exception::class)
    fun itConsumesValidJSON() {
        val federationMappingBuilder: FederationMappingBuilder? = FederationMappingBuilder()
        val json: String? = ("{ " +
                "\"cname\" : \"cname1\", " +
                "\"ttl\" : \"86400\", " +
                ...
                "}")
        val federationMapping: FederationMapping? = federationMappingBuilder.fromJSON(json)
        MatcherAssert.assertThat(federationMapping, CoreMatchers.not(IsNull.nullValue()))
        MatcherAssert.assertThat(federationMapping.getCname(), Matchers.equalTo("cname1"))
        MatcherAssert.assertThat(federationMapping.getTtl(), Matchers.equalTo(86400))
        MatcherAssert.assertThat(
            federationMapping.getResolve4(),
            Matchers.containsInAnyOrder(
                CidrAddress.Companion.fromString("192.168.45.67/24"),
                CidrAddress.Companion.fromString("192.168.56.78/24")
            )
        )
        MatcherAssert.assertThat(
            federationMapping.getResolve6(),
            Matchers.containsInAnyOrder(
                CidrAddress.Companion.fromString("fd12:3456:789a:1::/64"),
                CidrAddress.Companion.fromString("fdfe:dcba:9876:5::/64")
            )
        )
    }
    ...
    fun itConsumesJSONWithoutResolvers() {
        ...
        MatcherAssert.assertThat(federationMapping.getCname(), Matchers.equalTo("cname1"))
        MatcherAssert.assertThat(federationMapping.getTtl(), Matchers.equalTo(86400))
        MatcherAssert.assertThat(federationMapping.getResolve4(), CoreMatchers.not(IsNull.nullValue()))
        MatcherAssert.assertThat(federationMapping.getResolve6(), CoreMatchers.not(IsNull.nullValue()))
    }
}