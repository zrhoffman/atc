 ...
 */
package org.apache.traffic_control.traffic_router.core.routerimport
...
class DNSRoutingMissesTest {
    ...
    fun before() {
        ...
        Mockito.`when`(federationRegistry.findInetRecords(ArgumentMatchers.anyString(), ArgumentMatchers.any(CidrAddress::class.java))).thenReturn(null)
        trafficRouter = Mockito.mock(TrafficRouter::class.java)
        Mockito.`when`(trafficRouter.getCacheRegister()).thenReturn(Mockito.mock(CacheRegister::class.java))
        ...
        track = PowerMockito.spy<StatTracker.Track?>(StatTracker.Companion.getTrack())
    ...
    fun itSetsDetailsWhenBypassDestination() {
        ...
        Mockito.`when`(deliveryService.isDns).thenReturn(true)
        Mockito.doReturn(deliveryService).`when`(trafficRouter).selectDeliveryService(request)
        val bypassDestination = Mockito.mock(JsonNode::class.java)
    ...
    fun itSetsDetailsAboutMissesWhenOnlyCoverageZoneSupported() {
        ...
        Mockito.doReturn(deliveryService).`when`(trafficRouter).selectDeliveryService(ArgumentMatchers.any(Request::class.java))
        trafficRouter.route(request, track)
        Mockito.verify(track).result = ResultType.MISS
    ...
    fun itSetsDetailsWhenCacheNotFoundByGeolocation() {
        PowerMockito.doCallRealMethod().`when`(trafficRouter).selectCachesByGeo(ArgumentMatchers.anyString(), ArgumentMatchers.any(DeliveryService::class.java), ArgumentMatchers.any(CacheLocation::class.java), ArgumentMatchers.any(StatTracker.Track::class.java), ArgumentMatchers.any(IPVersions::class.java))
        ...
        Mockito.`when`(deliveryService.isDns).thenReturn(true)
        Mockito.doReturn(deliveryService).`when`(trafficRouter).selectDeliveryService(request)
        Mockito.doReturn(cacheLocation).`when`(trafficRouter).getCoverageZoneCacheLocation("192.168.34.56", deliveryService, IPVersions.IPV4ONLY)
