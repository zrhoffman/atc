...
class HTTPAccessEventBuilderTest {
    ...
    fun before() {
        ...
        PowerMockito.whenNew(Date::class.java).withArguments(ArgumentMatchers.anyLong()).thenReturn(startDate)
    ...
    fun itGeneratesAccessEvents() {
        ...
        MatcherAssert.assertThat(httpAccessEvent, Matchers.equalTo("144140678.000 qtype=HTTP chi=192.168.7.6 rhi=- url=\"http://example.com/index.html?foo=bar\" cqhm=GET cqhv=HTTP/1.1 rtype=- rloc=\"-\" rdtl=- rerr=\"-\" rgb=\"-\" rurl=\"-\" rurls=\"-\" uas=\"null\" svc=\"-\" rh=\"-\""))
    }
    ...
    fun itAddsResponseData() {
        val nanoTimeAnswer: Answer<Long?> = object : Answer<Long?>() {
            val nanoTimes: LongArray? = longArrayOf(100111001L, 225111001L)
            var index = 0
        ...
        val httpAccessEvent = HTTPAccessEventBuilder.create(httpAccessRecord)
        MatcherAssert.assertThat(httpAccessEvent, Matchers.equalTo("144140678.000 qtype=HTTP chi=192.168.7.6 rhi=- url=\"http://example.com/index.html?foo=bar\" cqhm=GET cqhv=HTTP/1.1 rtype=CZ rloc=\"39.75,-104.99\" rdtl=- rerr=\"-\" rgb=\"-\" pssc=302 ttms=125.000 rurl=\"http://example.com/hereitis/index.html?foo=bar\" rurls=\"-\" uas=\"null\" svc=\"-\" rh=\"-\""))
    }
    ...
    fun itRoundsUpToNearestMicroSecond() {
        val nanoTimeAnswer: Answer<Long?> = object : Answer<Long?>() {
            val nanoTimes: LongArray? = longArrayOf(100111001L, 100234999L)
            var index = 0
            override fun answer(invocation: InvocationOnMock?): Long? {
                return nanoTimes.get(index++ % 2)
            }
        ...
        val httpAccessRecord = builder.build()
        val httpAccessEvent = HTTPAccessEventBuilder.create(httpAccessRecord)
        MatcherAssert.assertThat(httpAccessEvent, Matchers.equalTo("144140678.000 qtype=HTTP chi=192.168.7.6 rhi=- url=\"http://example.com/index.html?foo=bar\" cqhm=GET cqhv=HTTP/1.1 rtype=ERROR rloc=\"-\" rdtl=- rerr=\"-\" rgb=\"-\" pssc=302 ttms=0.124 rurl=\"http://example.com/hereitis/index.html?foo=bar\" rurls=\"-\" uas=\"null\" svc=\"-\" rh=\"-\""))
    }
    ...
    fun itRecordsTrafficRouterErrors() {
        val nanoTimeAnswer: Answer<Long?> = object : Answer<Long?>() {
            val nanoTimes: LongArray? = longArrayOf(111001L, 567002L)
            var index = 0
            override fun answer(invocation: InvocationOnMock?): Long? {
                return nanoTimes.get(index++ % 2)
            }
    ...
    fun itRecordsMissResultDetails() {
        val nanoTimeAnswer: Answer<Long?> = object : Answer<Long?>() {
            val nanoTimes: LongArray? = longArrayOf(100000101L, 100789000L)
            var index = 0
            override fun answer(invocation: InvocationOnMock?): Long? {
                return nanoTimes.get(index++ % 2)
            }
        ...
        val httpAccessRecord = builder.build()
        val httpAccessEvent = HTTPAccessEventBuilder.create(httpAccessRecord)
        MatcherAssert.assertThat(httpAccessEvent, Matchers.equalTo("144140678.000 qtype=HTTP chi=192.168.7.6 rhi=- url=\"http://example.com/index.html?foo=bar\" cqhm=GET cqhv=HTTP/1.1 rtype=MISS rloc=\"-\" rdtl=DS_NO_BYPASS rerr=\"-\" rgb=\"-\" pssc=503 ttms=0.789 rurl=\"-\" rurls=\"-\" uas=\"null\" svc=\"-\" rh=\"-\""))
    }
    ...
    fun itRecordsRequestHeaders() {
        ...
        val httpAccessRecord = builder.resultType(ResultType.CZ).build()
        val httpAccessEvent = HTTPAccessEventBuilder.create(httpAccessRecord)
        MatcherAssert.assertThat(httpAccessEvent, Matchers.not(Matchers.containsString(" rh=\"-\"")))
        MatcherAssert.assertThat(httpAccessEvent, Matchers.containsString("rh=\"If-Modified-Since: Thurs, 15 July 2010 12:00:00 UTC\""))
        MatcherAssert.assertThat(httpAccessEvent, Matchers.containsString("rh=\"Accept: text/*, text/html, text/html;level=1, */*\""))
        MatcherAssert.assertThat(httpAccessEvent, Matchers.containsString("rh=\"Arbitrary: The cow says 'moo'"))
    }

    @Test
    @Throws(Exception::class)
    fun itUsesXMmClientIpHeaderForChi() {
        Mockito.`when`(request.getHeader(HTTPRequest.Companion.X_MM_CLIENT_IP)).thenReturn("192.168.100.100")
        Mockito.`when`(request.getRemoteAddr()).thenReturn("12.34.56.78")
        val httpAccessRecord = HTTPAccessRecord.Builder(Date(144140678000L), request).build()
        val httpAccessEvent = HTTPAccessEventBuilder.create(httpAccessRecord)
        MatcherAssert.assertThat(httpAccessEvent, Matchers.equalTo("144140678.000 qtype=HTTP chi=192.168.100.100 rhi=12.34.56.78 url=\"http://example.com/index.html?foo=bar\" cqhm=GET cqhv=HTTP/1.1 rtype=- rloc=\"-\" rdtl=- rerr=\"-\" rgb=\"-\" rurl=\"-\" rurls=\"-\" uas=\"null\" svc=\"-\" rh=\"-\""))
    }
    ...
    fun itUsesFakeIpParameterForChi() {
        ...
        val httpAccessRecord = HTTPAccessRecord.Builder(Date(144140678000L), request).build()
        val httpAccessEvent = HTTPAccessEventBuilder.create(httpAccessRecord)
        MatcherAssert.assertThat(httpAccessEvent, Matchers.equalTo("144140678.000 qtype=HTTP chi=192.168.123.123 rhi=192.168.7.6 url=\"http://example.com/index.html?foo=bar\" cqhm=GET cqhv=HTTP/1.1 rtype=- rloc=\"-\" rdtl=- rerr=\"-\" rgb=\"-\" rurl=\"-\" rurls=\"-\" uas=\"null\" svc=\"-\" rh=\"-\""))
    }
    ...
    fun itUsesXMmClientIpHeaderOverFakeIpParameterForChi() {
        ...
        val httpAccessRecord = HTTPAccessRecord.Builder(Date(144140678000L), request).build()
        val httpAccessEvent = HTTPAccessEventBuilder.create(httpAccessRecord)
        MatcherAssert.assertThat(httpAccessEvent, Matchers.equalTo("144140678.000 qtype=HTTP chi=192.168.100.100 rhi=192.168.7.6 url=\"http://example.com/index.html?foo=bar\" cqhm=GET cqhv=HTTP/1.1 rtype=- rloc=\"-\" rdtl=- rerr=\"-\" rgb=\"-\" rurl=\"-\" rurls=\"-\" uas=\"null\" svc=\"-\" rh=\"-\""))
    }
    ...
    fun itUsesUserAgentHeaderString() {
        ...
        val httpAccessRecord = HTTPAccessRecord.Builder(Date(144140678000L), request).build()
        val httpAccessEvent = HTTPAccessEventBuilder.create(httpAccessRecord)
        MatcherAssert.assertThat(httpAccessEvent, Matchers.containsString("uas=\"Mozilla/5.0 Gecko/20100101 Firefox/68.0\""))
    }
}