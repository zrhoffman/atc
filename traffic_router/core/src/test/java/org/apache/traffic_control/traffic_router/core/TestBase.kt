 ...
 */
package org.apache.traffic_control.traffic_router.coreimport
...
object TestBase {
    ...
    private val testHttpServerPort = SocketUtils.findAvailableTcpPort()
    private val httpDataServer: HttpDataServer? = HttpDataServer(TestBase.testHttpServerPort)
    ...
    fun setupFakeServers() {
        ...
        if (TestBase.tmpDeployDir == null) {
            TestBase.tmpDeployDir = Files.createTempDirectory("ext-test-").toFile()
        }
        val TRAFFIC_MONITOR_BOOTSTRAP_LOCAL = "TRAFFIC_MONITOR_BOOTSTRAP_LOCAL"
        val TRAFFIC_MONITOR_HOSTS = "TRAFFIC_MONITOR_HOSTS"
        val FAKE_SERVER: String
        FAKE_SERVER = "localhost:" + TestBase.testHttpServerPort + ";"
        ...
        TestBase.addToEnv(additionalEnvironment)
        ...
        TestBase.httpDataServer.start(TestBase.testHttpServerPort)
        System.setProperty("testHttpServerPort", "" + TestBase.testHttpServerPort)
        ...
        File(TestBase.tmpDeployDir, "conf").mkdirs()
        println()
        println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
        println(">>>>>>>> Going to use tmp directory '" + TestBase.tmpDeployDir + "' as traffic router deploy directory")
        println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
        println()
        System.setProperty("deploy.dir", TestBase.tmpDeployDir.absolutePath)
        ...
        val dbDirectory = File(TestBase.tmpDeployDir, "db")
        dbDirectory.mkdir()
        ...
        LoggerContext.getContext().getLogger("").level = Level.WARN
        ...
        val consoleAppender = ConsoleAppender.newBuilder().setName("TestBase").setLayout(PatternLayout.newBuilder().withPattern("%d{ISO8601} [%-5p] %c{4}: %m%n").build()).build()
    ...
    @Throws(Exception::class)
    fun tearDownFakeServers() {
        TestBase.httpDataServer.stop()
        TestBase.tmpDeployDir.deleteOnExit()
        val platformMBeanServer = ManagementFactory.getPlatformMBeanServer()
        try {
            val objectName = ObjectName(DeliveryServiceCertificatesMBean.Companion.OBJECT_NAME)
    ...
    fun getContext(): ApplicationContext? {
        if (TestBase.context == null) {
            TestBase.LOGGER.warn("Initializing context before running integration tests")
            TestBase.context = FileSystemXmlApplicationContext("src/main/webapp/WEB-INF/applicationContext.xml")
            TestBase.LOGGER.warn("Context initialized integration tests will now start running")
        }
        return TestBase.context
    }
}