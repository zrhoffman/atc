...
object TrafficRouterStart {
    ...
    fun main(args: Array<String>) {
        ...
        config.getLoggerConfig("org.springframework").level = Level.WARN
        val rootAppenders = ctx.rootLogger.appenders.values
        for (a in rootAppenders) {
            ctx.rootLogger.removeAppender(a)
        }
        val consoleAppender = ConsoleAppender.newBuilder().setName("TrafficRouterStart").setLayout(PatternLayout.newBuilder().withPattern("%d{ISO8601} [%-5p] %c{4}: %m%n").build()).build()
        ctx.rootLogger.addAppender(consoleAppender)
        config.getLoggerConfig(LogManager.ROOT_LOGGER_NAME).level = Level.INFO
        ctx.updateLoggers()
