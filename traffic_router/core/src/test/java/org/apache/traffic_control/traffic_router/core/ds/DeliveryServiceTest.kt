...
class DeliveryServiceTest {
    ...
    fun itHandlesDuplicatesInConsistentHashQueryParams() {
        val mapper = ObjectMapper()
        val json = mapper.readTree("{\"routingName\":\"edge\",\"coverageZoneOnly\":false,\"consistentHashQueryParams\":[\"test\", \"quest\", \"test\"]}")
        val d = DeliveryService("test", json)
        assert(d.consistentHashQueryParams != null)
    ...
    fun itExtractsQueryParams() {
        val json = ObjectMapper().readTree("{\"routingName\":\"edge\",\"coverageZoneOnly\":false,\"consistentHashQueryParams\":[\"test\", \"quest\"]}")
        val r = HTTPRequest()
    ...
    fun itConfiguresRequestHeadersFromJSON() {
        val mapper = ObjectMapper()
        val jsonStr = "{\"routingName\":\"edge\",\"requestHeaders\":[\"Cookie\",\"Cache-Control\",\"If-Modified-Since\",\"Content-Type\"],\"coverageZoneOnly\":false}"
        val jsonConfiguration = mapper.readTree(jsonStr)
        val deliveryService = DeliveryService("a-delivery-service", jsonConfiguration)
        MatcherAssert.assertThat(deliveryService.requestHeaders, Matchers.containsInAnyOrder("Cache-Control", "Cookie", "Content-Type", "If-Modified-Since"))
    }
    ...
    fun itAddsRequiredCapabilities() {
        val mapper = ObjectMapper()
        val jsonConfiguration = mapper.readTree("{\"requiredCapabilities\":[\"all-read\",\"all-write\",\"cdn-read\"],\"routingName\":\"edge\",\"coverageZoneOnly\":false}")
        val deliveryService = DeliveryService("has-required-capabilities", jsonConfiguration)
        MatcherAssert.assertThat(Whitebox.getInternalState(deliveryService, "requiredCapabilities"), Matchers.containsInAnyOrder("all-read", "all-write", "cdn-read"))
    }
}