...
class FederationRegistryTest {
    ...
    fun itFindsMapping() {
        ...
        var inetRecords = federationRegistry.findInetRecords("kable-town-01", CidrAddress.Companion.fromString("192.168.10.11/24"))
        MatcherAssert.assertThat(inetRecords, Matchers.containsInAnyOrder(InetRecord("cname1", 1234)))
        inetRecords = federationRegistry.findInetRecords("kable-town-01", CidrAddress.Companion.fromString("192.168.10.11/16"))
        MatcherAssert.assertThat(inetRecords, Matchers.containsInAnyOrder(InetRecord("cname1", 1234)))
    }
}