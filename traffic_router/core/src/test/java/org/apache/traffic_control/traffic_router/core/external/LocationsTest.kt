...
class LocationsTest {
    ...
    fun itGetsAListOfCaches() {
        ...
        try {
            ...
            MatcherAssert.assertThat(cacheNode["cacheOnline"].asText(), AnyOf.anyOf(IsEqual.equalTo("true"), IsEqual.equalTo("false")))
            MatcherAssert.assertThat(cacheNode["lastUpdateHealthy"].asText(), AnyOf.anyOf(IsEqual.equalTo("true"), IsEqual.equalTo("false")))
        } finally {
            response?.close()
        }
    }
