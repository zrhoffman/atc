...
class SteeringRegistryTest {
    ...
    fun itConsumesValidJson() {
        ...
        MatcherAssert.assertThat(steeringRegistry["steering-1"].targets, Matchers.containsInAnyOrder(steeringTarget1, steeringTarget2))
        MatcherAssert.assertThat(steeringRegistry["steering-2"].targets[1].deliveryService, Matchers.equalTo("steering-target-02"))
        MatcherAssert.assertThat(steeringRegistry["steering-1"].filters[0].pattern, Matchers.equalTo(".*/force-to-one/.*"))
        MatcherAssert.assertThat(steeringRegistry["steering-1"].filters[0].deliveryService, Matchers.equalTo("steering-target-01"))
        MatcherAssert.assertThat(steeringRegistry["steering-1"].filters[1].pattern, Matchers.equalTo(".*/also-this/.*"))
        MatcherAssert.assertThat(steeringRegistry["steering-1"].filters[1].deliveryService, Matchers.equalTo("steering-target-01"))
        MatcherAssert.assertThat(steeringRegistry["steering-1"].getBypassDestination("/stuff/force-to-one/more/stuff"), Matchers.equalTo("steering-target-01"))
        MatcherAssert.assertThat(steeringRegistry["steering-1"].getBypassDestination("/should/not/match"), Matchers.nullValue())
