...
class CatalinaTrafficRouter(serverXmlPath: String?, appBase: String?) {
    ...
    init {
        ...
        val secureConnectorList = Arrays.stream(trafficRouterService.findConnectors()).filter { k: Connector? -> k.getAttribute("portAttribute") == "SecureApiPort" }.collect(Collectors.toList())
        val hasHttpsPort = secureConnectorList.size > 0
        val securePort = if (hasHttpsPort) secureConnectorList[0].getPort() else 0
        val apiPort = Arrays.stream(trafficRouterService.findConnectors()).filter { k: Connector? -> k.getAttribute("portAttribute") == "ApiPort" }.collect(Collectors.toList())[0].getPort()
        ...
        println("[" + System.currentTimeMillis() + "] >>>>>>>>>>>>>>>> Traffic Router listening on DNS port " + System.getProperty("dns.udp.port") + " udp")
        println("[" + System.currentTimeMillis() + "] >>>>>>>>>>>>>>>> Traffic Router listening on DNS port " + System.getProperty("dns.tcp.port") + " tcp")
