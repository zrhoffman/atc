...
class StatsTest {
    ...
    fun itGetsApplicationStats() {
        ...
        try {
            ...
            val data: MutableMap<String?, Any?>? = objectMapper.readValue(responseContent, object : TypeReference<HashMap<String?, Any?>?>() {})
            MatcherAssert.assertThat(data.keys, Matchers.containsInAnyOrder("app", "stats"))
            val appData = data.get("app") as MutableMap<String?, Any?>?
            MatcherAssert.assertThat(appData.keys, Matchers.containsInAnyOrder("buildTimestamp", "name", "deploy-dir", "git-revision", "version"))
            val statsData = data.get("stats") as MutableMap<String?, Any?>?
            MatcherAssert.assertThat(statsData.keys, Matchers.containsInAnyOrder("dnsMap", "httpMap", "totalDnsCount", "totalHttpCount", "totalDsMissCount", "appStartTime", "averageDnsTime", "averageHttpTime", "updateTracker"))
            val dnsStats = statsData.get("dnsMap") as MutableMap<String?, Any?>?
            val cacheDnsStats = dnsStats.values.iterator().next() as MutableMap<String?, Any?>?
            MatcherAssert.assertThat(cacheDnsStats.keys, Matchers.containsInAnyOrder("czCount", "geoCount", "missCount", "dsrCount", "errCount",
                    "deepCzCount", "staticRouteCount", "fedCount", "regionalDeniedCount", "regionalAlternateCount"))
            val httpStats = statsData.get("httpMap") as MutableMap<String?, Any?>?
            val cacheHttpStats = httpStats.values.iterator().next() as MutableMap<String?, Any?>?
            MatcherAssert.assertThat(cacheHttpStats.keys, Matchers.containsInAnyOrder("czCount", "geoCount", "missCount", "dsrCount", "errCount",
                    "deepCzCount", "staticRouteCount", "fedCount", "regionalDeniedCount", "regionalAlternateCount"))
            val updateTracker = statsData.get("updateTracker") as MutableMap<String?, Any?>?
            val keys = updateTracker.keys
            val expectedStats = Arrays.asList("lastCacheStateCheck", "lastCacheStateChange", "lastConfigCheck", "lastConfigChange")
    ...
    fun itGetsLocationsByIp() {
        ...
        try {
            ...
            val data: MutableMap<String?, Any?>? = ObjectMapper().readValue(actual, object : TypeReference<HashMap<String?, Any?>?>() {})
            MatcherAssert.assertThat(data.get("requestIp"), Matchers.equalTo("8.8.8.8"))
            MatcherAssert.assertThat(data.get("locationByFederation"), Matchers.equalTo("not found"))
            MatcherAssert.assertThat(data.get("locationByCoverageZone"), Matchers.equalTo("not found"))
            val locationByGeo = data.get("locationByGeo") as MutableMap<String?, Any?>?
            MatcherAssert.assertThat(locationByGeo.keys, Matchers.containsInAnyOrder("city", "countryCode", "latitude", "longitude", "postalCode", "countryName"))
        } finally {
            response?.close()
        }
    }
