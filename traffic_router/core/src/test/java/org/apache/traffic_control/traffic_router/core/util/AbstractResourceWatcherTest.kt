...
class AbstractResourceWatcherTest {
    ...
    fun setUp() {
        federationsWatcher = AbstractResourceWatcherTest.Companion.context.getBean("federationsWatcher") as FederationsWatcher
        steeringWatcher = AbstractResourceWatcherTest.Companion.context.getBean("steeringWatcher") as SteeringWatcher
        steeringRegistry = AbstractResourceWatcherTest.Companion.context.getBean("steeringRegistry") as SteeringRegistry
        trafficRouterManager = AbstractResourceWatcherTest.Companion.context.getBean("trafficRouterManager") as TrafficRouterManager
        trafficRouterManager.getTrafficRouter().setApplicationContext(AbstractResourceWatcherTest.Companion.context)
        ...
        while (!federationsWatcher.isLoaded()) {
            AbstractResourceWatcherTest.Companion.LOGGER.info("Waiting for a valid federations database before proceeding")
            Thread.sleep(1000)
        }
        while (!steeringWatcher.isLoaded()) {
            AbstractResourceWatcherTest.Companion.LOGGER.info("Waiting for a valid steering database before proceeding")
            Thread.sleep(1000)
        }
    }
    ...
    fun tearDown() {
        ...
        federationsWatcher.trafficOpsUtils.setConfig(config)
        federationsWatcher.configure(config)
        MatcherAssert.assertThat(federationsWatcher.getDataBaseURL(), Matchers.endsWith(FederationsWatcher.Companion.DEFAULT_FEDERATION_DATA_URL.split("api".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray().get(1)))
    }
    ...
    fun testWatchers() {
        ...
        MatcherAssert.assertThat(federationsWatcher.getDataBaseURL(), Matchers.endsWith(FederationsWatcher.Companion.DEFAULT_FEDERATION_DATA_URL.split("api".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray().get(1)))
        MatcherAssert.assertThat(steeringWatcher.getDataBaseURL(), Matchers.endsWith(SteeringWatcher.Companion.DEFAULT_STEERING_DATA_URL.split("api".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray().get(1)))
        ...
        MatcherAssert.assertThat(config[federationsWatcher.getWatcherConfigPrefix() + ".polling.url"].asText(), Matchers.endsWith("api/3.0/notAFederationsEndpoint"))
        MatcherAssert.assertThat(federationsWatcher.getDataBaseURL(), Matchers.endsWith("api/3.0/notAFederationsEndpoint"))
    }

    companion object {
        private val LOGGER = LogManager.getLogger(AbstractResourceWatcherTest::class.java)
        private val context: ApplicationContext? = null
        @BeforeClass
        @Throws(Exception::class)
        fun setUpBeforeClass() {
            TestBase.setupFakeServers()
            AbstractResourceWatcherTest.Companion.context = TestBase.getContext()
        }
