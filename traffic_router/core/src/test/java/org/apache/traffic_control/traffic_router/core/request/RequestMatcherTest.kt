...
class RequestMatcherTest {
    ...
    fun itDoesNotAllowHEADERmatchesWithoutHeaderName() {
        try {
            ...
            MatcherAssert.assertThat(iae.message, Matchers.equalTo("Request Header name must be supplied for type HEADER"))
        }
        try {
            RequestMatcher(DeliveryServiceMatcher.Type.HEADER, ".*kabletown.*", "")
            Assert.fail("Should have thrown IllegalArgumentException")
    ...
    fun itThrowsIllegalArgumentException() {
        try {
            ...
            MatcherAssert.assertThat(e.message, Matchers.equalTo("Request Header name must be supplied for type HEADER"))
        }
    }
