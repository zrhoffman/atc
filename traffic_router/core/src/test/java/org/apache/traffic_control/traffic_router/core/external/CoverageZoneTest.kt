...
class CoverageZoneTest {
    ...
    fun itGetsCacheLocation() {
        val httpGet = HttpGet("http://localhost:3333/crs/coveragezone/cachelocation?ip=100.3.3.123&deliveryServiceId=steering-target-1")
    ...
    @Throws(Exception::class)
    fun itGetsCaches() {
        val httpGet = HttpGet("http://localhost:3333/crs/coveragezone/caches?deliveryServiceId=steering-target-4&cacheLocationId=location-3")
        var response: CloseableHttpResponse? = null
        try {
            response = closeableHttpClient.execute(httpGet)
            ...
            Assert.assertThat(cacheNode["available"].asText(), AnyOf.anyOf(IsEqual.equalTo("true"), IsEqual.equalTo("false")))
        } finally {
            response?.close()
        }
    }
    ...
    fun itReturns404ForMissingDeliveryService() {
        val httpGet = HttpGet("http://localhost:3333/crs/coveragezone/caches?deliveryServiceId=ds-07&cacheLocationId=location-5")
        closeableHttpClient.execute(httpGet).use { response -> Assert.assertThat(response.statusLine.statusCode, IsEqual.equalTo(404)) }
    }

    fun isValidIpV4String(ip: String?): Boolean {
        val octets: Array<String?> = ip.split("\\.".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
        if (octets.size != 4) {
            return false
        }
        for (octet in octets) {
