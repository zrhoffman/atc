...
class GeoSortSteeringResultsTest {
    ...
    fun before() {
        ...
        steeringResults = Mockito.spy(ArrayList())
        clientLocation = Geolocation(47.0, -122.0)
        deliveryService = Mockito.mock(DeliveryService::class.java)
        Mockito.doCallRealMethod().`when`(trafficRouter).geoSortSteeringResults(ArgumentMatchers.anyList(), ArgumentMatchers.anyString(), ArgumentMatchers.any(DeliveryService::class.java))
        Mockito.`when`(trafficRouter.getClientLocationByCoverageZoneOrGeo(ArgumentMatchers.anyString(), ArgumentMatchers.any(DeliveryService::class.java))).thenReturn(clientLocation)
    }
    ...
    fun testClientGeolocationIsNull() {
        ...
        steeringResults.add(SteeringResult(steeringTarget, deliveryService))
        Mockito.`when`(trafficRouter.getClientLocationByCoverageZoneOrGeo(ArgumentMatchers.anyString(), ArgumentMatchers.any(DeliveryService::class.java))).thenReturn(null)
        trafficRouter.geoSortSteeringResults(steeringResults, "::1", deliveryService)
        Mockito.verify(steeringResults, Mockito.never()).sort(ArgumentMatchers.any<Comparator<in SteeringResult?>?>())
    }
    ...
    fun testGeoSortingMixedWithNonGeoTargets() {
        ...
        trafficRouter.geoSortSteeringResults(steeringResults, "::1", deliveryService)
        Assert.assertEquals(resultNoGeoNegativeOrder, steeringResults.get(0))
        Assert.assertEquals(resultGeo, steeringResults.get(1))
        Assert.assertEquals(resultNoGeoZeroOrder, steeringResults.get(2))
        Assert.assertEquals(resultNoGeoPositiveOrder, steeringResults.get(3))
    }
}