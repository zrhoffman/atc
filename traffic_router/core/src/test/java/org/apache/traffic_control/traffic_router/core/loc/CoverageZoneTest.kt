...
class CoverageZoneTest {
    ...
    fun before() {
        ...
        Mockito.`when`(trafficRouter.getCoverageZoneCacheLocation("12.23.34.45", "delivery-service-1", IPVersions.IPV4ONLY)).thenCallRealMethod()
        Mockito.`when`(trafficRouter.getCoverageZoneCacheLocation("12.23.34.45", "delivery-service-1", false, null, IPVersions.IPV4ONLY)).thenCallRealMethod()
        Mockito.`when`(trafficRouter.getCacheRegister()).thenReturn(cacheRegister)
        Mockito.`when`(trafficRouter.orderLocations(ArgumentMatchers.anyList(), ArgumentMatchers.any(Geolocation::class.java))).thenCallRealMethod()
        Mockito.`when`(trafficRouter.getSupportingCaches(ArgumentMatchers.anyList(), ArgumentMatchers.eq(deliveryService), ArgumentMatchers.any(IPVersions::class.java))).thenCallRealMethod()
        Mockito.`when`(trafficRouter.filterEnabledLocations(ArgumentMatchers.anyList(), ArgumentMatchers.any(LocalizationMethod::class.java))).thenCallRealMethod()
        PowerMockito.`when`<Any?>(trafficRouter, "getNetworkNode", "12.23.34.45").thenReturn(eastNetworkNode)
        PowerMockito.`when`<Any?>(trafficRouter, "getClosestCacheLocation", ArgumentMatchers.anyList<Any?>(), ArgumentMatchers.any(), ArgumentMatchers.any(), ArgumentMatchers.any()).thenCallRealMethod()
    }

    @Test
    @Throws(Exception::class)
    fun trafficRouterReturnsNearestCacheGroupForDeliveryService() {
        val cacheLocation = trafficRouter.getCoverageZoneCacheLocation("12.23.34.45", "delivery-service-1", IPVersions.IPV4ONLY)
        MatcherAssert.assertThat(cacheLocation.id, Matchers.equalTo("west-cache-group"))
