...
class StatelessTrafficRouterTest {
    ...
    fun setUp() {
        trafficRouterManager = StatelessTrafficRouterTest.Companion.context.getBean("trafficRouterManager") as TrafficRouterManager
        geolocationDatabaseUpdater = StatelessTrafficRouterTest.Companion.context.getBean("geolocationDatabaseUpdater") as GeolocationDatabaseUpdater
        networkUpdater = StatelessTrafficRouterTest.Companion.context.getBean("networkUpdater") as NetworkUpdater
        while (!networkUpdater.isLoaded()) {
            StatelessTrafficRouterTest.Companion.LOGGER.info("Waiting for a valid location database before proceeding")
            Thread.sleep(1000)
        }
        while (!geolocationDatabaseUpdater.isLoaded()) {
            StatelessTrafficRouterTest.Companion.LOGGER.info("Waiting for a valid Maxmind database before proceeding")
            Thread.sleep(1000)
        }
    }
    ...
    companion object {
        private val LOGGER = LogManager.getLogger(StatelessTrafficRouterTest::class.java)
        private val context: ApplicationContext? = null
        @BeforeClass
        @Throws(Exception::class)
        fun setUpBeforeClass() {
            TestBase.setupFakeServers()
            StatelessTrafficRouterTest.Companion.context = TestBase.getContext()
        }
