 ...
 */
package org.apache.traffic_control.traffic_router.core.hashingimport
...
import org.apache.traffic_control.traffic_router.core.hash.*
...
class ConsistentHasherTest {
    ...
    fun before() {
        ...
        Mockito.`when`(trafficRouter.buildPatternBasedHashString(ArgumentMatchers.anyString(), ArgumentMatchers.anyString())).thenCallRealMethod()
        Mockito.`when`(trafficRouter.buildPatternBasedHashString(ArgumentMatchers.any(DeliveryService::class.java), ArgumentMatchers.any(HTTPRequest::class.java))).thenCallRealMethod()
    ...
    fun itHashes() {
        ...
        val hashable = consistentHasher.selectHashable(hashables, Dispersion(mapper.createObjectNode()), "some-string")
        Assert.assertThat(hashable, Matchers.anyOf(IsEqual.equalTo(hashable1), IsEqual.equalTo(hashable2), IsEqual.equalTo(hashable3)))
        val nextHashable = consistentHasher.selectHashable(hashables, Dispersion(mapper.createObjectNode()), "some-string")
    ...
    fun itHashesMoreThanOne() {
        ...
        Assert.assertThat(results[0], Matchers.anyOf(IsEqual.equalTo(hashable1), IsEqual.equalTo(hashable2), IsEqual.equalTo(hashable3)))
        Assert.assertThat(results[1], Matchers.anyOf(IsEqual.equalTo(hashable1), IsEqual.equalTo(hashable2), IsEqual.equalTo(hashable3)))
    ...
    fun itemsMigrateFromSmallerToLargerBucket() {
        ...
        for (randomPath in randomPaths) {
            val hashable = consistentHasher.selectHashable(changedBuckets, Dispersion(mapper.createObjectNode()), randomPath)
            rehashedPaths[hashable].add(randomPath)
        }
        Assert.assertThat(rehashedPaths[grownBucket].size, Matchers.greaterThan(hashedPaths[smallerBucket].size))
        Assert.assertThat(rehashedPaths[shrunkBucket].size, Matchers.lessThan(hashedPaths[largerBucket].size))
    ...
    fun testPatternBasedHashing() {
        ...
        Assert.assertThat(hashableResult1, Matchers.allOf(IsEqual.equalTo(hashableResult2), IsEqual.equalTo(hashableResult3), IsEqual.equalTo(hashableResult4)))
    }

    @Test
    @Throws(Exception::class)
    fun itHashesQueryParams() {
        val j = ObjectMapper().readTree("{\"routingName\":\"edge\",\"coverageZoneOnly\":false,\"consistentHashQueryParams\":[\"test\", \"quest\"]}")
