...
object ZoneTestRecords {
    ...
    private fun recreateKeyPair(publicKey: String?, privateKey: String?): KeyPair? {
        val privateKeyCopy: PrivateKey? = BindPrivateKey().decode(String(Base64.getMimeDecoder().decode(privateKey)))
        val publicKeyCopy: PublicKey? =
            KeyFactory.getInstance("RSA").generatePublic(Pkcs1KeySpecDecoder().decode(publicKey))
    ...
    fun generateZoneRecords(makeNewKeyPairs: Boolean): MutableList<Record?>? {
        ...
        val tenYears: Duration? = Duration.ofDays(3650)
        val oneDay: Duration? = Duration.ofDays(1)
        val threeDays: Duration? = Duration.ofDays(3)
        val threeWeeks: Duration? = Duration.ofDays(21)
        val oneHour: Long = 3600
        val nameServer1: Name? = Name("ns1.example.com.")
        val nameServer2: Name? = Name("ns2.example.com.")
        val adminEmail: Name? = Name("admin.example.com.")
        val webServer: Name? = Name("www.example.com.")
        val ftpServer: Name? = Name("ftp.example.com.")
        val webMirror: Name? = Name("mirror.www.example.com.")
        val ftpMirror: Name? = Name("mirror.ftp.example.com.")
        val txtRecord: String? = String("dead0123456789")
        records = ArrayList<E?>(
            Arrays.asList(
                AAAARecord(webServer, DClass.IN, threeDays.getSeconds(), Inet6Address.getByName("2001:db8::5:6:7:8")),
                AAAARecord(
                    ftpServer,
                    DClass.IN,
                    threeDays.getSeconds(),
                    Inet6Address.getByName("2001:db8::12:34:56:78")
                ),
                NSRecord(origin, DClass.IN, tenYears.getSeconds(), nameServer1),
                NSRecord(origin, DClass.IN, tenYears.getSeconds(), nameServer2),
                ARecord(
                    webServer,
                    DClass.IN,
                    threeWeeks.getSeconds(),
                ...
                AAAARecord(webServer, DClass.IN, threeDays.getSeconds(), Inet6Address.getByName("2001:db8::4:3:2:1")),
                SOARecord(
                    origin,
                    DClass.IN,
                    tenYears.getSeconds(),
                ...
                AAAARecord(
                    ...
                    DClass.IN,
                    threeDays.getSeconds(),
                    Inet6Address.getByName("2001:db8::21:43:65:87")
                ),
                CNAMERecord(webMirror, DClass.IN, tenYears.getSeconds(), webServer),
                CNAMERecord(ftpMirror, DClass.IN, tenYears.getSeconds(), ftpServer),
                TXTRecord(webServer, DClass.IN, tenYears.getSeconds(), txtRecord)
            )
        )
        if (makeNewKeyPairs) {
            val keyPairs: MutableList<KeyPair?>? = generateKeyPairs()
            ksk1 = keyPairs.get(0)
            zsk1 = keyPairs.get(1)
            ksk2 = keyPairs.get(2)
            zsk2 = keyPairs.get(3)
