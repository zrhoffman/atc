...
class BindPrivateKey constructor() {
    ...
    private fun decodeBigIntegers(s: String?): MutableMap<String?, BigInteger?>? {
        val bigIntKeys: MutableList<String?>? = Arrays.asList(
        ...
        for (line: String? in s.split("\n").toTypedArray()) {
            val tokens: Array<String?>? = line.split(": ").toTypedArray()
            if (bigIntKeys.stream().filter(Predicate({ k: String? -> (k == tokens.get(0)) })).findFirst().isPresent()) {
                bigIntegerMap.put(tokens.get(0), decodeBigInt(tokens.get(1)))
            }
        }
    ...
    fun decode(data: String?): PrivateKey? {
        val map: MutableMap<String?, BigInteger?>? = decodeBigIntegers(data)
        val modulus: BigInteger? = map.get("Modulus")
        val publicExponent: BigInteger? = map.get("PublicExponent")
        val privateExponent: BigInteger? = map.get("PrivateExponent")
        val prime1: BigInteger? = map.get("Prime1")
        val prime2: BigInteger? = map.get("Prime2")
        val exp1: BigInteger? = map.get("Exponent1")
        val exp2: BigInteger? = map.get("Exponent2")
        val coeff: BigInteger? = map.get("Coefficient")
        val keySpec: RSAPrivateCrtKeySpec? =
            RSAPrivateCrtKeySpec(modulus, publicExponent, privateExponent, prime1, prime2, exp1, exp2, coeff)
        try {
    ...
    companion object {
        private val LOGGER: Logger? = Logger.getLogger(BindPrivateKey::class.java)
    }
}