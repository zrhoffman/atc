...
class Pkcs1KeySpecDecoder constructor() {
    ...
    fun decode(data: String?): KeySpec? {
        val pemData: String? =
            data.replace(Pkcs1.Companion.HEADER.toRegex(), "").replace(Pkcs1.Companion.FOOTER.toRegex(), "")
                .replace("\\s".toRegex(), "")
        val asn1Sequence: ASN1Sequence? = ASN1Sequence.getInstance(Base64.getDecoder().decode(pemData))
        val sequenceLength: Int = asn1Sequence.toArray().size
        ...
        if (asn1Sequence.toArray().size == PUBLIC_SEQUENCE_LENGTH) {
            val asn1Parser: ASN1SequenceParser? = asn1Sequence.parser()
            val n: BigInteger? = (asn1Parser.readObject() as ASN1Integer?).getValue()
            val e: BigInteger? = (asn1Parser.readObject() as ASN1Integer?).getValue()
        ...
        try {
            ...
            LOGGER.error(
                "Error converting to PKCS8 Encoded Key Spec " + e.javaClass.getCanonicalName() + ": " + e.message,
                e
            )
        }
        return null
    ...
    companion object {
        ...
        val PRIVATE_SEQUENCE_LENGTH: Int = 9
        val PUBLIC_SEQUENCE_LENGTH: Int = 2
        private val LOGGER: Logger? = Logger.getLogger(Pkcs1KeySpecDecoder::class.java)
    }
}