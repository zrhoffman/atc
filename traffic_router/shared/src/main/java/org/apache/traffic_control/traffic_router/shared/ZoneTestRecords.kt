...
object ZoneTestRecords {
    ...
    fun generateZoneRecords(makeNewKeyPairs: Boolean): MutableList<Record?>? {
        ZoneTestRecords.start = Date(System.currentTimeMillis() - 24 * 3600 * 1000)
        ZoneTestRecords.expiration = Date(System.currentTimeMillis() + 7 * 24 * 3600 * 1000)
        ZoneTestRecords.origin = Name("example.com.")
        ...
        val txtRecord = "dead0123456789"
        ZoneTestRecords.records = ArrayList<E?>(Arrays.asList(
                AAAARecord(webServer, DClass.IN, threeDays.seconds, Inet6Address.getByAddress(byteArrayOf(32, 1, 13, -72, 0, 0, 0, 0, 0, 5, 0, 6, 0, 7, 0, 8))),  // 2001:db8::5:6:7:8
                AAAARecord(ftpServer, DClass.IN, threeDays.seconds, Inet6Address.getByAddress(byteArrayOf(32, 1, 13, -72, 0, 0, 0, 0, 0, 18, 0, 52, 0, 86, 0, 120))),  // 2001:db8::12:34:56:78
                NSRecord(ZoneTestRecords.origin, DClass.IN, tenYears.seconds, nameServer1),
                NSRecord(ZoneTestRecords.origin, DClass.IN, tenYears.seconds, nameServer2),
                ARecord(webServer, DClass.IN, threeWeeks.seconds, InetAddress.getByAddress(byteArrayOf(11, 22, 33, 44))),
                ARecord(webServer, DClass.IN, threeWeeks.seconds, InetAddress.getByAddress(byteArrayOf(55, 66, 77, 88))),
                ARecord(ftpServer, DClass.IN, threeWeeks.seconds, InetAddress.getByAddress(byteArrayOf(12, 34, 56, 78))),
                ARecord(ftpServer, DClass.IN, threeWeeks.seconds, InetAddress.getByAddress(byteArrayOf(21, 43, 65, 87))),
                AAAARecord(webServer, DClass.IN, threeDays.seconds, Inet6Address.getByAddress(byteArrayOf(32, 1, 13, -72, 0, 0, 0, 0, 0, 4, 0, 3, 0, 2, 0, 1))),  // 2001:db8::4:3:2:1
                SOARecord(ZoneTestRecords.origin, DClass.IN, tenYears.seconds, nameServer1,
                        adminEmail, 2016091400L, oneDay.seconds, oneHour, threeWeeks.seconds, threeDays.seconds),
                AAAARecord(ftpServer, DClass.IN, threeDays.seconds, Inet6Address.getByAddress(byteArrayOf(32, 1, 13, -72, 0, 0, 0, 0, 0, 33, 0, 67, 0, 101, 0, -121))),  // 2001:db8::21:43:65:87
        ...
        if (makeNewKeyPairs) {
            val keyPairs = ZoneTestRecords.generateKeyPairs()
            ZoneTestRecords.ksk1 = keyPairs[0]
            ZoneTestRecords.zsk1 = keyPairs[1]
            ZoneTestRecords.ksk2 = keyPairs[2]
            ZoneTestRecords.zsk2 = keyPairs[3]
        } else {
            ZoneTestRecords.ksk1 = ZoneTestRecords.recreateKeyPair(SigningData.ksk1Public, SigningData.ksk1Private)
            ZoneTestRecords.zsk1 = ZoneTestRecords.recreateKeyPair(SigningData.zsk1Public, SigningData.zsk1Private)
            ZoneTestRecords.ksk2 = ZoneTestRecords.recreateKeyPair(SigningData.ksk2Public, SigningData.ksk2Private)
            ZoneTestRecords.zsk2 = ZoneTestRecords.recreateKeyPair(SigningData.zsk2Public, SigningData.zsk2Private)
        }
        ZoneTestRecords.zoneSigningKeyRecord = DNSKEYRecord(ZoneTestRecords.origin, DClass.IN, 31556952L,
                DNSKEYRecord.Flags.ZONE_KEY, DNSKEYRecord.Protocol.DNSSEC, DNSSEC.Algorithm.RSASHA1, ZoneTestRecords.zsk1.public.encoded)
        ZoneTestRecords.keySigningKeyRecord = DNSKEYRecord(ZoneTestRecords.origin, DClass.IN, 315569520L,
                DNSKEYRecord.Flags.ZONE_KEY or DNSKEYRecord.Flags.SEP_KEY, DNSKEYRecord.Protocol.DNSSEC, DNSSEC.Algorithm.RSASHA1, ZoneTestRecords.ksk1.public.encoded)
        return ZoneTestRecords.records
    }
}