...
class Pkcs1KeySpecDecoder {
    ...
    fun decode(data: String?): KeySpec? {
        val pemData: String = data.replace(Pkcs1.Companion.HEADER.toRegex(), "").replace(Pkcs1.Companion.FOOTER.toRegex(), "").replace("\\s".toRegex(), "")
        val asn1Sequence = ASN1Sequence.getInstance(Base64.getDecoder().decode(pemData))
        val sequenceLength = asn1Sequence.toArray().size
        if (sequenceLength != Pkcs1KeySpecDecoder.Companion.PUBLIC_SEQUENCE_LENGTH && sequenceLength != Pkcs1KeySpecDecoder.Companion.PRIVATE_SEQUENCE_LENGTH) {
        ...
        if (asn1Sequence.toArray().size == Pkcs1KeySpecDecoder.Companion.PUBLIC_SEQUENCE_LENGTH) {
        ...
        try {
            ...
            Pkcs1KeySpecDecoder.Companion.LOGGER.error("Error converting to PKCS8 Encoded Key Spec " + e.javaClass.canonicalName + ": " + e.message, e)
        }
        return null
    }
    ...
    companion object {
        ...
        private val LOGGER = LogManager.getLogger(Pkcs1KeySpecDecoder::class.java)
    }
}