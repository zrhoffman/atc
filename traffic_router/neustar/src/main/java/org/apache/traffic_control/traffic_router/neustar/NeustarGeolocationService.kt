...
class NeustarGeolocationService : GeolocationService {
    ...
    fun verifyDatabase(databaseDirectory: File?): Boolean {
        throw RuntimeException("verifyDatabase is no longer allowed, " + NeustarDatabaseUpdater::class.java.getSimpleName() + " is used for verification instead")
    }
    ...
    companion object {
        private val LOGGER: Logger? = LogManager.getLogger(NeustarGeolocationService::class.java)
    }
}