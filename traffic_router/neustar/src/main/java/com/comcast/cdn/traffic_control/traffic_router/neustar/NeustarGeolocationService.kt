...
@Component
class NeustarGeolocationService constructor() : GeolocationService {
    ...
    private fun createDatabaseReader(databaseDirectory: File?): GPDatabaseReader? {
        ...
        try {
            ...
            val path: String? = if (databaseDirectory != null) databaseDirectory.getAbsolutePath() else "NULL"
    ...
    private fun createGeolocation(response: GeoPointResponse?): Geolocation? {
        val geolocation: Geolocation? = Geolocation(response.getLatitude(), response.getLongitude())
