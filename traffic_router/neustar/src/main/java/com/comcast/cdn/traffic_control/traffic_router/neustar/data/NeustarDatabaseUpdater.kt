...
class NeustarDatabaseUpdater constructor() {
    ...
    fun update(): Boolean {
        ...
        val tmpDir: File? = createTmpDir(neustarDatabaseDirectory)
        if (tmpDir == null) {
            return false
        }
        try {
    ...
    fun getRemoteDataResponse(uri: URI?): CloseableHttpResponse? {
        val httpGet: HttpGet? = HttpGet(uri)
    ...
    fun getDatabaseBuildDate(): Date? {
        ...
        for (file: File? in neustarDatabaseFiles) {
            if (file.isDirectory()) {
                continue
            }
