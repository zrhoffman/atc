...
class NeustarDatabaseUpdaterTest {
    @Mock
    ...
    fun itRetrievesRemoteFileContents() {
        ...
        whenNew(GPDatabaseReader.Builder::class.java).withArguments(any(File::class.java)).thenReturn(mock(GPDatabaseReader.Builder::class.java))
        ...
        `when`(filesMover.updateCurrent(eq(neustarDatabaseDirectory), any(File::class.java), eq(neustarOldDatabaseDirectory))).thenReturn(true)
