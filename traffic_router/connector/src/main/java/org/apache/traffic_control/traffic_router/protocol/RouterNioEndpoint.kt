...
class RouterNioEndpoint : NioEndpoint() {
        ...
        for (alias in aliases) {
            ...
            if (sslHostConfig.hostName != lastHostName) {
                try {
                    ...
                    RouterNioEndpoint.Companion.LOGGER.error("In RouterNioEndpoint.replaceSSLHosts, sslHostConfig and certs did not get replaced " +
    ...
    fun reloadSSLHosts(cr: MutableMap<String?, HandshakeData?>?): MutableList<String?>? {
        ...
        for (alias in cr.keys) {
            try {
                ...
                RouterNioEndpoint.Companion.LOGGER.error("In RouterNioEndpoint could not create new SSLContext for cert " + alias +
    ...
    override fun getSSLHostConfig(sniHostName: String?): SSLHostConfig? {
        return super.getSSLHostConfig(sniHostName?.lowercase(Locale.getDefault()))
    }
    ...
    companion object {
        private val LOGGER = LogManager.getLogger(RouterNioEndpoint::class.java)
    }
}