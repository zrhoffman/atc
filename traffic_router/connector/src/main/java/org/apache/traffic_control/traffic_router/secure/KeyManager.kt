...
class KeyManager : X509ExtendedKeyManager(), X509KeyManager {
    ...
    private fun chooseServerAlias(sslSession: ExtendedSSLSession?): String? {
        ...
        for (requestedName in requestedNames) {
            ...
            val partialAliasMatches = certificateRegistry.getAliases().stream().filter { s: String? -> sniString.contains(s) }.collect(Collectors.toList())
            ...
            alias = partialAliasMatches.stream().filter { cs: String? -> sniStringTrimmed.contentEquals(cs) }.findFirst()
            if (alias.isPresent) {
                return alias.get()
            }
        }
    ...
    companion object {
        private val log = LogManager.getLogger(KeyManager::class.java)
    }
}