...
class CertificateDecoder {
    ...
    fun toCertificate(encodedCertificate: String?): X509Certificate? {
        val encodedBytes: ByteArray = Base64.getDecoder().decode(encodedCertificate.replace(CertificateDecoder.Companion.CRT_HEADER.toRegex(), "").replace(CertificateDecoder.Companion.CRT_FOOTER.toRegex(), ""))
        try {
            ByteArrayInputStream(encodedBytes).use { stream -> return CertificateFactory.getInstance("X.509").generateCertificate(stream) as X509Certificate }
        } catch (e: Exception) {
    ...
    companion object {
        private val log = LogManager.getLogger(CertificateDecoder::class.java)
