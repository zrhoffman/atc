...
class CertificateDataConverter {
    ...
    fun toHandshakeData(certificateData: CertificateData?): HandshakeData? {
        try {
            ...
            if (hostMatch && modMatch) {
                return HandshakeData(certificateData.getDeliveryservice(), certificateData.getHostname(),
            ...
            CertificateDataConverter.Companion.log.warn("Failed to convert certificate data for delivery service = " + certificateData.getHostname()
            ...
            CertificateDataConverter.Companion.log.warn("Failed to convert certificate data (delivery service = " + certificateData.getDeliveryservice()
    ...
    fun verifySubject(certificate: X509Certificate?, hostAlias: String?): Boolean {
        ...
        try {
            ...
            CertificateDataConverter.Companion.log.error("Encountered an error while validating the certificate subject for service: " + hostAlias + ", " +
    ...
    private fun hostCompare(hostAlias: String?, subject: String?): Boolean {
        ...
        if (subjectName.contains("CN=")) {
            val chopped: Array<String?> = subjectName.split("CN=".toRegex(), limit = 2).toTypedArray()
            if (chopped != null && chopped.size > 1) {
                val chop = chopped[1]
                subjectName = chop.split(",".toRegex(), limit = 2).toTypedArray()[0]
            }
        }
    ...
    fun verifyModulus(privateKey: PrivateKey?, certificate: X509Certificate?): Boolean {
        ...
        if (publicKey is RSAPublicKeySpec) {
            ...
            val keyparts: Array<String?> = publicKey.toString().split(System.getProperty("line.separator").toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
    ...
    companion object {
        private val log = LogManager.getLogger(CertificateDataConverter::class.java)
    }
}