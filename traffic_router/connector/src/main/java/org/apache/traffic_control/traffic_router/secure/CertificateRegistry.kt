...
class CertificateRegistry private constructor() {
    ...
    private fun createApiDefaultSsl(): HandshakeData? {
        try {
            ...
            if (File(selfSignedKeystoreFile).exists()) {
                ...
                FileInputStream(selfSignedKeystoreFile).use { readStream -> ks.load(readStream, password.toCharArray()) }
                ...
                return HandshakeData(defaultAlias, defaultAlias,
    ...
    fun importCertificateDataList(certificateDataList: MutableList<CertificateData?>?) {
        ...
        for (certificateData in certificateDataList) {
            try {
                ...
                if (!master.containsKey(alias)) {
                    ...
                    CertificateRegistry.Companion.log.error("An TLS certificate already exists in the registry for host: " + alias + " There can be " +
                            "only one!")
                }
            } catch (e: Exception) {
        ...
        if (!master.containsKey(CertificateRegistry.Companion.DEFAULT_SSL_KEY)) {
            ...
            if (handshakeDataMap.containsKey(CertificateRegistry.Companion.DEFAULT_SSL_KEY)) {
                master[CertificateRegistry.Companion.DEFAULT_SSL_KEY] = handshakeDataMap.get(CertificateRegistry.Companion.DEFAULT_SSL_KEY)
            } else {
                // create a new default certificate
                val defaultHd: HandshakeData = CertificateRegistry.Companion.createDefaultSsl()
                if (defaultHd == null) {
                    CertificateRegistry.Companion.log.error("Failed to initialize the CertificateRegistry because of a problem with the 'default' " +
    ...
    companion object {
        ...
        private val log = LogManager.getLogger(CertificateRegistry::class.java)
        ...
        private fun createDefaultSsl(): HandshakeData? {
            return try {
                ...
                val dnName = X500Name("C=US; ST=CO; L=Denver; " +
                ...
                certBuilder.addExtension(Extension.extendedKeyUsage, true, ExtendedKeyUsage(arrayOf(
                ...
                HandshakeData(CertificateRegistry.Companion.DEFAULT_SSL_KEY, CertificateRegistry.Companion.DEFAULT_SSL_KEY, chain, keyPair.private)
            } catch (e: Exception) {
