...
class CertificateDataListener constructor() : NotificationListener {
    public override fun handleNotification(notification: Notification?, handback: Any?) {
        if (!(notification is AttributeChangeNotification)) {
            return
        }
        var certificateDataList: MutableList<CertificateData?>? = ArrayList()
        val newValue: Any? = (notification as AttributeChangeNotification?).getNewValue()
        if (certificateDataList.javaClass.isInstance(newValue)) {
            ...
            try {
                ...
                log.warn("Failed importing certificate data list into registry " + t.javaClass.getSimpleName(), t)
            }
        }
    }

    companion object {
        private val log: Logger? = Logger.getLogger(CertificateDataListener::class.java)
    }
}