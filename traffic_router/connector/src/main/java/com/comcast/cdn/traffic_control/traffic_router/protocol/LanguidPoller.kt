...
class LanguidPoller constructor(private val protocolHandler: RouterProtocolHandler?) : Thread() {
    public override fun run() {
        ...
        while (true) {
            try {
                ...
                val languidState: ObjectName? = ObjectName(protocolHandler.getMbeanPath())
                val readyValue: Any? = mbs.getAttribute(languidState, protocolHandler.getReadyAttribute())
                val portValue: Any? = mbs.getAttribute(languidState, protocolHandler.getPortAttribute())
                val ready: Boolean = java.lang.Boolean.parseBoolean(readyValue.toString())
                val port: Int = portValue.toString().toInt()
                if (firstTime) {
    ...
    companion object {
        private val log: Logger? = Logger.getLogger(LanguidPoller::class.java)
    }
}