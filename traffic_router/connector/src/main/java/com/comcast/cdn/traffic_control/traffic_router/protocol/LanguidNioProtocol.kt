...
class LanguidNioProtocol constructor() : AbstractHttp11JsseProtocol<NioChannel?>(RouterNioEndpoint()),
    RouterProtocolHandler {
    ...
    companion object {
        protected var log: Log? = LogFactory.getLog(LanguidNioProtocol::class.java)
        ...
        init {
            log.warn("Adding BouncyCastle provider")
            Security.addProvider(BouncyCastleProvider())
        }
    }

    public override fun setSslImplementationName(sslClassName: String?) {
        try {
            Class.forName(sslClassName)
            Companion.log.info("setSslImplementation: " + sslClassName)
            super.setSslImplementationName(sslClassName)
        } catch (e: ClassNotFoundException) {
            Companion.log.error("LanguidNIOProtocol: Failed to set SSL implementation to " + sslClassName + " class was not found, defaulting to OpenSSL")
        }
    }
    ...
    public override fun init() {
        if (!isReady()) {
            Companion.log.info("Init called; creating thread to monitor the state of Traffic Router")
            LanguidPoller(this).start()
            return
        }
        Companion.log.info("Traffic Router SSL Protocol is ready; calling super.init()")
    ...
    public override fun start() {
        Companion.log.info("LanguidNioProtocol Handler Start called; waiting for initialization to occur")
        while (!isInitialized()) {
            try {
                Thread.sleep(100)
            } catch (e: InterruptedException) {
                Companion.log.info("interrupted waiting for initialization")
            }
        }
        Companion.log.info("LanguidNioProtocol Handler Initialization complete; calling super.start()")
        super.start()
    }
    ...
    public override fun setReady(isReady: Boolean) {
        ready = isReady
    }
    ...
    public override fun setInitialized(isInitialized: Boolean) {
        initialized = isInitialized
    }

    public override fun getMbeanPath(): String? {
        return mbeanPath
    }

    public override fun setMbeanPath(mbeanPath: String?) {
        this.mbeanPath = mbeanPath
    }

    public override fun getReadyAttribute(): String? {
        return readyAttribute
    }

    public override fun setReadyAttribute(readyAttribute: String?) {
        this.readyAttribute = readyAttribute
    }

    public override fun getPortAttribute(): String? {
        return portAttribute
    }

    public override fun setPortAttribute(portAttribute: String?) {
    ...
    init {
        Companion.log.warn("Serving wildcard certs for multiple domains")
    }
}