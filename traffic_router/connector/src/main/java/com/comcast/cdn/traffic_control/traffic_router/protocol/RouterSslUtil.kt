...
class RouterSslUtil constructor(certificate: SSLHostConfigCertificate?) : SSLUtilBase(certificate) {
    ...
    @Throws(Exception::class)
    public override fun getKeyManagers(): Array<KeyManager?>? {
        return arrayOf(com.comcast.cdn.traffic_control.traffic_router.secure.KeyManager())
    }

    @Throws(Exception::class)
    public override fun getTrustManagers(): Array<TrustManager?>? {
        return null
    }

    public override fun configureSessionContext(sslSessionContext: SSLSessionContext?) {}

    companion object {
        private val log: Log? = LogFactory.getLog(RouterSslUtil::class.java)
    }
}