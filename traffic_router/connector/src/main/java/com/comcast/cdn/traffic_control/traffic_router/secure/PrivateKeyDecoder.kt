...
class PrivateKeyDecoder constructor() {
    @Throws(IOException::class, GeneralSecurityException::class)
    fun decode(data: String?): PrivateKey? {
        val decodedData: String? = String(Base64.getMimeDecoder().decode(data.toByteArray()))
        return if (decodedData.contains(Pkcs1.Companion.HEADER)) Pkcs1(decodedData).getPrivateKey() else Pkcs8(
            decodedData
        ).getPrivateKey()
    }
}