...
class CertificateDecoder constructor() {
    ...
    fun toCertificate(encodedCertificate: String?): X509Certificate? {
        val encodedBytes: ByteArray? = Base64.getDecoder().decode(
            encodedCertificate.replace(CRT_HEADER.toRegex(), "").replace(
                CRT_FOOTER.toRegex(), ""
            )
        )
        try {
            ...
            val message: String? =
                "Failed to decode certificate data to X509! " + e.javaClass.getSimpleName() + ": " + e.message
            log.error(message, e)
    ...
    companion object {
        private val log: Logger? = Logger.getLogger(CertificateDecoder::class.java)
