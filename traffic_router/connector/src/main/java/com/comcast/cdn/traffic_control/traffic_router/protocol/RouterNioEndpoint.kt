...
class RouterNioEndpoint constructor() : NioEndpoint() {
    ...
    override fun initialiseSsl() {
        if (isSSLEnabled()) {
            ...
            val certificateRegistry: CertificateRegistry? = keyManager.getCertificateRegistry()
    ...
    private fun replaceSSLHosts(sslHostsData: MutableMap<String?, HandshakeData?>?): MutableList<String?>? {
        val aliases: MutableSet<String?>? = sslHostsData.keys
        var lastHostName: String? = ""
        val failedUpdates: MutableList<String?>? = ArrayList()
        for (alias: String? in aliases) {
            val sslHostConfig: SSLHostConfig? = SSLHostConfig()
            val cert: SSLHostConfigCertificate? =
            ...
            if (!(sslHostConfig.getHostName() == lastHostName)) {
                try {
    ...
    fun reloadSSLHosts(cr: MutableMap<String?, HandshakeData?>?): MutableList<String?>? {
        val failedUpdates: MutableList<String?>? = replaceSSLHosts(cr)
        if (!failedUpdates.isEmpty()) {
            failedUpdates.forEach(Consumer({ alias: String? -> cr.remove(alias) }))
        }
        val failedContextUpdates: MutableList<String?>? = ArrayList()
        for (alias: String? in cr.keys) {
            try {
                val data: HandshakeData? = cr.get(alias)
                val sslHostConfig: SSLHostConfig? = sslHostConfigs.get(data.getHostname())
                sslHostConfig.setSslProtocol(sslHostConfig.getSslProtocol())
    ...
    override fun getSSLHostConfig(sniHostName: String?): SSLHostConfig? {
        return super.getSSLHostConfig(if (sniHostName == null) null else sniHostName.toLowerCase())
    }
     ...
     */
    protected inner class RouterSocketProcessor constructor(
        socketWrapper: SocketWrapperBase<NioChannel?>?,
        event: SocketEvent?
		...
		*/
        override fun doRun() {
            val localWrapper: SocketWrapperBase<NioChannel?>? = socketWrapper
            val socket: NioChannel? = localWrapper.getSocket()
            super.doRun()
            if (!socket.isOpen()) {
    ...
    companion object {
        private val LOGGER: Logger? = Logger.getLogger(RouterNioEndpoint::class.java)
    }
}