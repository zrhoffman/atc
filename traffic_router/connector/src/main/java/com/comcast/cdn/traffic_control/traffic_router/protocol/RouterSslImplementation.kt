...
class RouterSslImplementation constructor() : SSLImplementation() {
    public override fun getSSLSupport(session: SSLSession?): SSLSupport? {
        return JSSESupport(session)
    }

    public override fun getSSLUtil(certificate: SSLHostConfigCertificate?): SSLUtil? {
        return RouterSslUtil(certificate)
    }

    public override fun isAlpnSupported(): Boolean {
        return true
    }
}