...
class LanguidProtocol constructor() : Http11NioProtocol(), RouterProtocolHandler {
    ...
    public override fun init() {
        if (!isReady()) {
            ...
            Companion.log.info("Traffic Router is ready; calling super.init()")
            super.init()
            setInitialized(true)
        }
    }
    ...
    public override fun start() {
        Companion.log.info("Start called; waiting for initialization to occur")
        while (!isInitialized()) {
            Thread.sleep(100)
        }
        Companion.log.info("Initialization complete; calling super.start()")
        super.start()
    }
    ...
    companion object {
        private val log: Logger? = Logger.getLogger(LanguidProtocol::class.java)
    }
}