...
class HttpsProperties constructor() {
    ...
    companion object {
        private val log: Logger? = Logger.getLogger(HttpsProperties::class.java)
                ...
                Files.readAllLines(Paths.get(HTTPS_PROPERTIES_FILE)).forEach(
                    Consumer({ propString: String? ->
                        if (!propString.startsWith("#")) { // Ignores comments in properties file
                            val prop: Array<String?>? = propString.split("=").toTypedArray()
                            httpsProperties.put(prop.get(0), prop.get(1))
                        }
                    })
                )
