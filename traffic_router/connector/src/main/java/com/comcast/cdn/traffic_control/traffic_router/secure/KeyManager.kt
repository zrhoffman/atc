...
class KeyManager constructor() : X509ExtendedKeyManager(), X509KeyManager {
    ...
    public override fun chooseClientAlias(
    ...
    // To date this method is not getting exercised while running the router
    public override fun getServerAliases(s: String?, principals: Array<Principal?>?): Array<String?>? {
        return certificateRegistry.getAliases().toTypedArray()
    }

    // To date this method is not getting exercised while running the router
    public override fun getClientAliases(s: String?, principals: Array<Principal?>?): Array<String?>? {
        throw UnsupportedOperationException("Traffic Router KeyManager does not support getting a list of Client Aliases")
    }

    public override fun chooseEngineServerAlias(
        keyType: String?,
        ...
        val sslSession: ExtendedSSLSession? = engine.getHandshakeSession() as ExtendedSSLSession?
        return chooseServerAlias(sslSession)
    }

    public override fun chooseServerAlias(keyType: String?, principals: Array<Principal?>?, socket: Socket?): String? {
        if (keyType == null || socket == null) {
            return null
        }
        val sslSocket: SSLSocket? = socket as SSLSocket?
        val sslSession: ExtendedSSLSession? = sslSocket.getHandshakeSession() as ExtendedSSLSession?
        return chooseServerAlias(sslSession)
    }

    private fun chooseServerAlias(sslSession: ExtendedSSLSession?): String? {
        val requestedNames: MutableList<SNIServerName?>? = sslSession.getRequestedServerNames()
        val stringBuilder: StringBuilder? = StringBuilder()
        for (requestedName: SNIServerName? in requestedNames) {
            if (stringBuilder.length > 0) {
                stringBuilder.append(", ")
            }
            val sniString: String? = String(requestedName.getEncoded())
            stringBuilder.append(sniString)
            val partialAliasMatches: MutableList<String?>? = certificateRegistry.getAliases().stream().filter(
                Predicate({ s: String? -> sniString.contains(s) })
            ).collect(Collectors.toList())
            var alias: Optional<String?>? =
                partialAliasMatches.stream().filter(Predicate({ cs: String? -> sniString.contentEquals(cs) }))
                    .findFirst()
            if (alias.isPresent()) {
                return alias.get()
            }
            ...
            val sniStringTrimmed: String? = sniString.substring(sniString.indexOf('.') + 1)
            alias =
                partialAliasMatches.stream().filter(Predicate({ cs: String? -> sniStringTrimmed.contentEquals(cs) }))
                    .findFirst()
            if (alias.isPresent()) {
                return alias.get()
            }
    ...
    public override fun getCertificateChain(alias: String?): Array<X509Certificate?>? {
        val handshakeData: HandshakeData? = certificateRegistry.getHandshakeData(alias)
    ...
    public override fun getPrivateKey(alias: String?): PrivateKey? {
        val handshakeData: HandshakeData? = certificateRegistry.getHandshakeData(alias)
        if (handshakeData != null) {
    ...
    companion object {
        private val log: Logger? = Logger.getLogger(KeyManager::class.java)
    }
}