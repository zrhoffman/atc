...
class TomcatLifecycleListener constructor() : LifecycleListener {
    private var certificateDataListener: CertificateDataListener? = CertificateDataListener()
    public override fun lifecycleEvent(event: LifecycleEvent?) {
        if (!(Lifecycle.AFTER_INIT_EVENT == event.getType())) {
            return
        }
        try {
            log.info("Registering delivery service certificates mbean")
            val objectName: ObjectName? = ObjectName(DeliveryServiceCertificatesMBean.Companion.OBJECT_NAME)
            val platformMBeanServer: MBeanServer? = ManagementFactory.getPlatformMBeanServer()
            ...
            throw RuntimeException(
                "Failed to register MBean " + DeliveryServiceCertificatesMBean.Companion.OBJECT_NAME + " " + e.javaClass.getSimpleName() + ": " + e.message,
                e
            )
        }
    }
    ...
    companion object {
        private val log: Logger? = Logger.getLogger(LifecycleListener::class.java)
    }
}