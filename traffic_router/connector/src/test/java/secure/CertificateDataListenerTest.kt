...
class CertificateDataListenerTest {
    ...
    fun itImportsCertificateDataToRegistry() {
        ...
        val notification: Notification = AttributeChangeNotification(notifier, 1L, System.currentTimeMillis(),
    ...
    fun itIgnoresBadInput() {
        ...
        val certificateDataListener = CertificateDataListener()
        certificateDataListener.handleNotification(notification, null)
        Mockito.verify(certificateRegistry, Mockito.times(0)).importCertificateDataList(ArgumentMatchers.any())
        val badData = Arrays.asList("foo", "bar", "baz")
        notification = AttributeChangeNotification("notifier", 1L, System.currentTimeMillis(),
