...
class CertificateDataListenerTest constructor() {
    ...
    fun itImportsCertificateDataToRegistry() {
        ...
        val certificateDataListener: CertificateDataListener? = CertificateDataListener()
    ...
    fun itIgnoresBadInput() {
        var notification: Notification? = Notification("notifier", "source", 1L, "hello world")
        val certificateDataListener: CertificateDataListener? = CertificateDataListener()
        certificateDataListener.handleNotification(notification, null)
        Mockito.verify(certificateRegistry, Mockito.times(0)).importCertificateDataList(Matchers.any())
        val badData: MutableList<String?>? = Arrays.asList("foo", "bar", "baz")
