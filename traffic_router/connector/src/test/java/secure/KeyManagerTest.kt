...
import org.apache.traffic_control.traffic_router.secure.*
...
class KeyManagerTest {
    ...
    fun before() {
        ...
        val certificateRegistry = Mockito.mock(CertificateRegistry::class.java)
        Mockito.`when`(certificateRegistry.aliases).thenReturn(Arrays.asList(
                "deliveryservice3.cdn2.example.com",
                "deliveryservice2.cdn2.example.com"
        ))
        PowerMockito.mockStatic(CertificateRegistry::class.java)
        Mockito.`when`<CertificateRegistry?>(CertificateRegistry.Companion.getInstance()).thenReturn(certificateRegistry)
        Mockito.`when`(certificateRegistry.getHandshakeData("deliveryservice2.cdn2.example.com")).thenReturn(handshakeData)
    ...
    fun itSelectsServerAlias() {
        ...
        val sslExtendedSession = Mockito.mock(ExtendedSSLSession::class.java)
    ...
    fun itGetsCertFromRegistry() {
        MatcherAssert.assertThat(keyManager.getCertificateChain("deliveryservice2.cdn2.example.com")[0], Matchers.equalTo(x509Certificate))
    }

    @Test
    fun itGetsKeyFromRegistry() {
        MatcherAssert.assertThat(keyManager.getPrivateKey("deliveryservice2.cdn2.example.com"), Matchers.equalTo(privateKey))
    }
