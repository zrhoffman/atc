...
class KeyManagerTest constructor() {
    ...
    fun before() {
        ...
        val handshakeData: HandshakeData? = Mockito.mock(HandshakeData::class.java)
        Mockito.`when`(handshakeData.getCertificateChain()).thenReturn(x509Certificates)
        Mockito.`when`(handshakeData.getPrivateKey()).thenReturn(privateKey)
        val certificateRegistry: CertificateRegistry? = Mockito.mock(
            CertificateRegistry::class.java
        )
        Mockito.`when`(certificateRegistry.getAliases()).thenReturn(
            Arrays.asList(
    ...
    fun itSelectsServerAlias() {
        ...
        val sslExtendedSession: ExtendedSSLSession? = Mockito.mock(
            ExtendedSSLSession::class.java
        )
        Mockito.`when`(sslExtendedSession.getRequestedServerNames()).thenReturn(sniServerNames)
        val sslSocket: SSLSocket? = Mockito.mock(SSLSocket::class.java)
        Mockito.`when`(sslSocket.getHandshakeSession()).thenReturn(sslExtendedSession)
        val serverAlias: String? = keyManager.chooseServerAlias("RSA", null, sslSocket)
    ...
    fun itGetsCertFromRegistry() {
        MatcherAssert.assertThat(
            keyManager.getCertificateChain("deliveryservice2.cdn2.example.com").get(0),
    ...
    internal inner class TestSNIServerName constructor(type: Int, name: String?) :
        SNIServerName(type, name.toByteArray())
}