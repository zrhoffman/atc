...
class CertificateRegistryTest {
    ...
    fun itImportsCertificateData() {
        ...
        MatcherAssert.assertThat(certificateRegistry.getHandshakeData("ds-1.some-cdn.example.com"), Matchers.equalTo(handshakeData1))
        MatcherAssert.assertThat(certificateRegistry.getHandshakeData("ds-2.some-cdn.example.com"), Matchers.equalTo(handshakeData2))
        MatcherAssert.assertThat(certificateRegistry.getHandshakeData("ds-3.some-cdn.example.com"), Matchers.equalTo(handshakeData3))
        ...
        MatcherAssert.assertThat(certificateRegistry.getAliases(),
                Matchers.containsInAnyOrder<String?>(CertificateRegistry.Companion.DEFAULT_SSL_KEY, "ds-1.some-cdn.example.com",
    ...
    fun itRetrysCertificateDataOnEndpointFail() {
        ...
        Mockito.`when`(endpoint.reloadSSLHosts(ArgumentMatchers.anyMap())).thenReturn(failist)
        certificateRegistry.setEndPoint(endpoint)
        certificateRegistry.importCertificateDataList(certificateDataList)
        MatcherAssert.assertThat(certificateRegistry.getHandshakeData("ds-1.some-cdn.example.com"), Matchers.equalTo(handshakeData1))
        MatcherAssert.assertThat(certificateRegistry.getHandshakeData("ds-2.some-cdn.example.com"), Matchers.equalTo(handshakeData2))
        MatcherAssert.assertThat(certificateRegistry.getHandshakeData("ds-3.some-cdn.example.com"), Matchers.equalTo(handshakeData3mod))
        Mockito.verify(certificateDataConverter).toHandshakeData(certificateData1)
        Mockito.verify(certificateDataConverter).toHandshakeData(certificateData2)
        Mockito.verify(certificateDataConverter).toHandshakeData(certificateData3mod)
        Mockito.verify(endpoint).reloadSSLHosts(ArgumentMatchers.anyMap())
        MatcherAssert.assertThat(certificateRegistry.getAliases(),
                Matchers.containsInAnyOrder<String?>(CertificateRegistry.Companion.DEFAULT_SSL_KEY, "ds-1.some-cdn.example.com", "ds-2.some-cdn.example.com", "ds-3.some-cdn.example.com"))

        // try again
        // we should see that reloadSSLHosts gets called again even though none of the inputs have changed
        Mockito.`when`(endpoint.reloadSSLHosts(ArgumentMatchers.anyMap())).thenReturn(ArrayList())
        certificateRegistry.importCertificateDataList(certificateDataList)
        MatcherAssert.assertThat(certificateRegistry.getHandshakeData("ds-1.some-cdn.example.com"), Matchers.equalTo(handshakeData1))
        MatcherAssert.assertThat(certificateRegistry.getHandshakeData("ds-2.some-cdn.example.com"), Matchers.equalTo(handshakeData2))
        MatcherAssert.assertThat(certificateRegistry.getHandshakeData("ds-3.some-cdn.example.com"), Matchers.equalTo(handshakeData3mod))
        ...
        Mockito.verify(endpoint, Mockito.times(2)).reloadSSLHosts(ArgumentMatchers.anyMap())
        MatcherAssert.assertThat(certificateRegistry.getAliases(),
                Matchers.containsInAnyOrder<String?>(CertificateRegistry.Companion.DEFAULT_SSL_KEY, "ds-1.some-cdn.example.com", "ds-2.some-cdn.example.com", "ds-3.some-cdn.example.com"))
    }
}