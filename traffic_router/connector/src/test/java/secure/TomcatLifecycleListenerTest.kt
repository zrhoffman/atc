...
class TomcatLifecycleListenerTest constructor() {
    @Before
    fun before() {
    ...
    fun itIgnoresNonInitEvents() {
        ...
        val lifecycle: Lifecycle? = Mockito.mock(Lifecycle::class.java)
        val tomcatLifecycleListener: TomcatLifecycleListener? = TomcatLifecycleListener()
        Arrays.asList(
    ...
    fun itRegistersBeanAndAddsListenerOnInit() {
        val mBeanServer: MBeanServer? = Mockito.mock(MBeanServer::class.java)
        Mockito.`when`(ManagementFactory.getPlatformMBeanServer()).thenAnswer(
            Answer<Any?>({ invocationOnMock: InvocationOnMock? -> mBeanServer })
        )
        val certificateDataListener: CertificateDataListener? = Mockito.mock(
            CertificateDataListener::class.java
        )
        val tomcatLifecycleListener: TomcatLifecycleListener? = TomcatLifecycleListener()
        tomcatLifecycleListener.setCertificateDataListener(certificateDataListener)
        val lifecycleEvent: LifecycleEvent? = PowerMockito.mock(LifecycleEvent::class.java)
        ...
        val name: ObjectName? = ObjectName(DeliveryServiceCertificatesMBean.Companion.OBJECT_NAME)
