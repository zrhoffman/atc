...
class TomcatLifecycleListenerTest {
    ...
    fun itIgnoresNonInitEvents() {
        Mockito.`when`(ManagementFactory.getPlatformMBeanServer()).thenThrow(RuntimeException("invoked getPlatformMBeanServer"))
        ...
        ).forEach(Consumer { s: String? -> tomcatLifecycleListener.lifecycleEvent(LifecycleEvent(lifecycle, s, Any())) })
    }
    ...
    fun itRegistersBeanAndAddsListenerOnInit() {
        ...
        Mockito.`when`(ManagementFactory.getPlatformMBeanServer()).thenAnswer { invocationOnMock: InvocationOnMock? -> mBeanServer }
        val certificateDataListener = Mockito.mock(CertificateDataListener::class.java)
        val tomcatLifecycleListener = TomcatLifecycleListener()
