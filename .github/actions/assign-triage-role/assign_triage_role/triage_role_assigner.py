...
from assign_triage_role.constants import SINCE_DAYS_AGO, TRIAGE_USER_MINIMUM_COMMITS, GH_TIMELINE_EVENT_TYPE_CROSS_REFERENCE, ENV_GITHUB_TOKEN, ENV_GITHUB_REPOSITORY, ASF_YAML_FILE, APACHE_LICENSE_YAML, ENV_GITHUB_REF_NAME, GIT_AUTHOR_EMAIL_TEMPLATE, ENV_GIT_AUTHOR_NAME, SINGLE_PR_TEMPLATE_FILE, SINGLE_CONTRIBUTOR_TEMPLATE_FILE, PR_TEMPLATE_FILE, ENV_GITHUB_REPOSITORY_OWNER
...
class TriageRoleAssigner:
	...
	def get_pr_body(self, prs_by_contributor: dict[str, list[(Issue, Issue)]], since_day: date, today: date) -> str:
		...
		pr_body: str = pr_template.format(CONTRIB_LIST_LIST=contrib_list_list, MONTH=today.strftime('%B'), ISSUE_THRESHOLD=TRIAGE_USER_MINIMUM_COMMITS, SINCE_DAYS_AGO=SINCE_DAYS_AGO, SINCE_DAY=since_day, TODAY=today)
		# If on a fork, do not ping users or reference Issues or Pull Requests
		if self.repo.parent is not None:
			pr_body = pr_body.replace('@', '＠').replace('#', '⌗')
	...
	def create_pr(self, prs_by_contributor: dict[str, list[(Issue, Issue)]], commit_message: str, owner: str,
		...
		try:
			...
			pr.add_to_labels(collaborators_label)
